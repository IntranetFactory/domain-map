...
All additive; no migrations needed. Apply via `semantius` field-creation tooling, same pattern as [plan-domain-fact-sheets.md § 3](plan-domain-fact-sheets.md).
...
|---|---|---|---|
| `cross_domain_handoffs` | `source_domain_module_id` | nullable FK → `domain_modules` | Handoffs really live between modules; the domain pair is a rollup. Nullable for backfill. |
| `cross_domain_handoffs` | `target_domain_module_id` | nullable FK → `domain_modules` | Same. |
...
- **Baseline tiers:** `<domain_module_code>:read`, `<domain_module_code>:manage`, `<domain_module_code>:admin` (mirrors today's `<slug>:read|manage|admin` from [plan-domain-fact-sheets.md § 4.1](plan-domain-fact-sheets.md), just module-scoped instead of domain-scoped).
...
Permissions are derived from `data_object_lifecycle_states` (workflow gates) and `data_objects` pattern flags (`has_personal_content`, `has_submit_lock`, `has_single_approver`) per the existing [plan-domain-fact-sheets.md § 8](plan-domain-fact-sheets.md) rules. Modules are the **namespace** in which derivation happens, not the source of the permissions themselves. The lifecycle state lives in some module (`data_object_lifecycle_states.domain_module_id`); the derived permission code uses that module's `domain_module_code` as prefix.
...
- `cross_domain_handoffs` keeps `source_domain_id` / `target_domain_id`; new columns are additive.
...
| TODO | When |
|---|---|
| **Rename `cross_domain_handoffs` → `handoffs`.** The table becomes misnamed once handoffs primarily live at module granularity (some still domain-only during backfill). Rename when cross-module backfill is complete catalog-wide. | After all 88 domains modularized OR explicitly decline to ever rename if backfill stays partial — decide at the post-20-domain review point. |
...
| **Semantic-architect skill flow change.** Today the architect asks the user to design entities from scratch; post-modularization the question becomes "which modules do you want to install?" The architect skill in `C:\dev\semantius-agent\.claude\skills\semantius-architect` needs a Stage 0 update analogous to the existing fact-sheet load (per [plan-domain-fact-sheets.md § 1](plan-domain-fact-sheets.md)) but selecting modules from the domain instead of taking the whole domain. | After ATS + ITSM modularized and fact sheet emitter produces module-aware output. |
...
- [ ] §11 step 7 — Per-domain rollout (track each separately):
  ...
  - [ ] PSA
  - [ ] S2P
  - [ ] (continue list per [plan-domain-fact-sheets.md § 6.3](plan-domain-fact-sheets.md) order)
...
- [ ] D3 — `cross_domain_handoffs` → `handoffs` rename (only if cross-module backfill catalog-wide complete)
