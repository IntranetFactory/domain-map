...
- `personas.json` and `processes.json` are gated to **released** domains only (a domain whose `catalog_release` is set and on or before the emit date); `domain-map.json` lists every domain regardless.
