...
**sources/** — Drop zone for files awaiting ingestion. Files here are processed by the batch ingest command and tracked in the log by filename.
...
- `sources/` directory (with `.gitkeep`)
...
**Single source** — A URL or file path is provided as the argument (e.g., `ingest https://...` or `ingest path/to/file.md`). Process that one source.

**Batch from sources/** — No argument is provided, or the argument is `sources`. List all files in `sources/`. Cross-reference `wiki/log.md` to identify which filenames have already been ingested (look for lines matching `- Source: sources/<filename>`). Process only the unprocessed files, in order. If all files are already logged, report that nothing is new and stop.
...
3. Save as `raw/<topic>/YYYY-MM-DD-descriptive-slug.md`.
   ...
   - Include metadata header: source URL or source file path, collected date, published date.
   - Preserve original text. Clean formatting noise (e.g., strip base64-encoded images). Do not rewrite opinions.
...
Append to `wiki/log.md` — one entry per source processed:

```
## [YYYY-MM-DD] ingest | <primary article title>
- Source: sources/<filename> → raw/<topic>/<raw-filename>
- Updated: <cascade-updated article title>
- Updated: <another cascade-updated article title>
```

For URL ingests, use `- Source: <url> → raw/<topic>/<raw-filename>` instead.
