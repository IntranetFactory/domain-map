...
**`record_status` changes ONLY on an explicit, direct, standalone user request that names the rows and the target status. There is no other path, and it is NEVER a side effect of anything else.** Not of answering a `q-`/`a-` file question, not of a renamed `a-` file, not of finishing or "passing" an audit, not of re-pointing / re-tagging / fixing a row, not of an "approve the batch" option folded into a multi-part answer, and never inferred because a row "looks reviewed". A `q-`/`a-` answer may authorize *structural* work (additive/destructive, per Rule #21 / Rule #22) but it can NEVER carry a `record_status` flip — that flip is out of band and needs its own explicit request. This holds in both directions: do not set `approved` / `pending` / `rejected` **and do not revert an existing value** without the user asking for that specific change. If you believe rows are ready, you ASK in plain chat, name exactly which rows and which status, and wait for the user to say yes to that change. When in doubt, leave it `new` (the DB default) and move on. An unrequested `record_status` write is a Rule #1 incident: log it to [references/skill-changelog.md](references/skill-changelog.md) and revert only once the user confirms the revert.
...
**Write tier by `entity_type` (B2).** Beyond lifecycle, `entity_type` also selects which existing module baseline governs an entity's WRITES (read is uniformly `:read`). This is DERIVED at emit time by `deriveWriteTier` in `scripts/generate_blueprints.ts` and rendered as the section-3 "write tier" column; no new permission is minted and nothing is stored on `data_objects`.
...
**`has_personal_content` hint** still needs to be considered for any master with `entity_type='operational_workflow'`: set it true when the entity holds owner-scoped rows, which signals that a row-scoped ABAC rule (JsonLogic `select_rule` / `validation_rules`) should be authored for it. The flag enforces nothing on its own. Auto-populating `notes` to explain it remains forbidden (Rule #15). If the reasoning is non-obvious, surface in chat so the user can decide whether and how to record it. (The former `has_submit_lock` / `has_single_approver` flags were dropped 2026-06-26 — those are workflow-state concerns that live in the lifecycle / RACI layer, not on the entity.)
...
**Cross-cutting modules.** Every module has exactly one home domain, `domain_modules.domain_id` (REQUIRED, `input_type='required'`). The catalog does NOT share one module across markets: cross-cutting concepts are modeled as **separate per-domain modules that share the same data_object** (e.g. "Knowledge" is `ITSM-KNOWLEDGE` + `HRSD-KNOWLEDGE` + `CSM-KNOWLEDGE`, each with its own role on `knowledge_articles`; "KNOWLEDGE-MGMT spanning ITSM/CSM/HRSD/LSD" is a CAPABILITY via `capability_domains`, not a module). A genuinely-cross-domain STARTER is promoted to its own `domain_kind='bundle'` domain (Rule #2) and homes there; its touched markets are DERIVED from its `embedded_master` rows. There is no host-junction table (`domain_module_host_domains` is REMOVED, see [references/deprecations.md](references/deprecations.md)). See [references/modules.md](references/modules.md) for the full mechanics.
...
### 17. Every domain with masters has exactly one `skill_type='domain'` skill, anchored at the domain grain, deriving its toolset from the domain's modules.

A domain's `domain` skill is the single self-contained agent surface for that domain. **Skills are domain-grain ONLY** (there is no module grain; `skills.domain_module_id` does not exist): one `skills` row with `skill_type='domain'`, `domain_id=<id>` (convention `skill_name='<domain_code_lower>'`, e.g. `grc`, `itsm`, `ats`; no suffix). Its routing prose is `skills.description`; its trigger keywords are the comma-separated `skills.trigger_keywords`, which the emitter appends to the description as "Use for: ..." (deduped against words already in the prose). **Both `skills.description` and `skills.trigger_keywords` are REQUIRED and co-authored in one pass:** the emitter errors (and the F8 audit band fails) if either is empty. The skill **derives** its toolset as the **union** of the domain's modules' `domain_module_tools` (each master's query / mutate, the lifecycle-gate mutations, the `notify_person` / `notify_team` abstractions, the handoff-fired cross-domain tools) and **discovers at runtime** which of the domain's modules and tools are installed in the deploying tenant, scoping itself to that subset. It stores no tools of its own (`skill_tools` is retired; tool requirements live on the modules). Without the skill the domain has no agent surface, and the Semantius score, the metric the entire tools/skills layer exists to compute, is uncomputable for that domain.
...
- **One `domain` skill per domain.** `skills.skill_type='domain'` AND `skills.domain_id=<id>`. Authoring more than one domain skill for a domain is a rule violation. **There is no module-anchored skill.** A single-domain starter (`module_kind='starter'` homing on a real market, e.g. `HIRING-STARTER` on ATS) gets NO skill of its own: a starter deployment ships the host domain's skill, which discovers and scopes itself to the installed subset. A genuinely cross-domain starter is promoted to its own `domain_kind='bundle'` domain (Rule #2) and gets one ordinary domain skill on that bundle domain (e.g. `it-ops-starter` on the IT-OPS-STARTER bundle). FULL modules get **no** skill. F1 cleans up any skill that is not cleanly domain-anchored.
- **That skill derives ≥1 tool.** Its toolset is the union of `domain_module_tools` over the domain's PRIMARY (full) modules; typical shape is 8–40 tools (required + optional). Required tools are the irreducible set the agent needs for the domain's primary workflows; optional tools cover degraded modes (lower-tier ML, alternate channels). The practical `required` floor is ≥1 `query`, ≥1 `mutate`, and ≥1 workflow gate **across the domain's modules**. (A cross-domain value stream is a `process` skill instead, linked via `skills.process_id` and deriving from `process_tools`.)
...
**This is a Phase-A obligation, not Phase E**, parallel to Rule #14. When loading a new market, the domain skill (with its `description` + `trigger_keywords`) + the modules' `tools` + `domain_module_tools` ship in the same load as `domains` + `capabilities` + `domain_modules` + `solutions`. The Phase-S step in the workflow (§ "Workflow for any research task") is the canonical authoring procedure.

**Audit blockers.** A domain with zero or >1 domain skills fails F2; any skill that is not cleanly domain-anchored fails F1; a domain whose modules carry zero `domain_module_tools` fails F3; a tool with an invalid `operation_kind` ↔ `data_object_id` pairing fails F4; an uncomputable Semantius score is the F5 rollup; a domain skill with an empty `description` or empty `trigger_keywords` fails F8. Each blocks the per-domain audit until cured. See F1–F8 in the per-domain completeness checklist.

**Why this rule exists:** before F2-F5 became positive-existence checks, the per-domain audit could tick green on a domain with zero domain skills (the F-band only carried a legacy-cleanup check). The Semantius score was then uncomputable but the gap went silent. F2-F5 close that.
...
- `data_objects` — `data_object_name`, `singular_label`, `plural_label`, `description`, `catalog_description`, `notes`.
...
Starter kits used to be an editorial junction (`domain_starter_modules`) recommending an install order over full modules. That shape did not work: installing a starter still meant installing N full modules with everything they carry (data_objects, lifecycle states, workflow-gate permissions, domain skills), which is too heavy for a small org and offers no "lite" path. The new shape: starter kits are deployable units themselves, distinguished from full modules by a `module_kind` discriminator on `domain_modules`.
...
- **Persona / use-case bundles.** Cross-domain personas like `REAL-ESTATE-AGENT` (CRM + CLM + light project tracking) that don't belong to any single market. These are promoted to their own `domain_kind='bundle'` domain (Rule #2): the starter module's REQUIRED `domain_id` points at that bundle-domain. Its touched markets (CRM, CLM, RE-BROKERAGE) are DERIVED from its `embedded_master` rows, not authored; the `domain_module_host_domains` table is REMOVED ([references/deprecations.md](references/deprecations.md)).
...
6. **No skill of its own (Rule #17: skills are domain-grain only).** A starter does NOT carry a `skills` row (`skills.domain_module_id` no longer exists). A **single-domain** starter (homing on a real market, e.g. `HIRING-STARTER` on ATS) is served by the host domain's skill, which discovers the installed modules and scopes itself to the starter's subset at runtime. A **cross-domain** starter is promoted to its own `domain_kind='bundle'` domain (Rule #2) and gets one ordinary domain skill on that bundle domain (`skill_type='domain'`, `domain_id=<bundle>`, `skill_name='<bundle_code_lower>'`), deriving its toolset from the starter module's `domain_module_tools`. Either way the starter's `domain_module_tools` are still authored (floor: ≥1 row, `query_<entity>` for each embedded master plus light mutates and the standalone-reachable re-prefixed workflow-gate tools its embedded entities expose); they roll up into whichever domain skill serves the starter.

**Specification requirements (optional, prose).** A starter / bundle whose promise depends on a field its embedded masters do not carry on their own MAY record that as prose in `domain_modules.specification_requirements`. Motivating case: a starter that collapses a multi-entity pattern from the full module into a flat field on a surviving shell (showback-lite cost: the full SMP/ITAM modules carry spend across separate entities like `smp_spend_allocations` / `journal_entries` that the starter omits, so it must denormalize a flat `renewal_cost` / `annual_spend` onto its shells, or its renewal-and-cost view cannot resolve). The emitter renders it as a dedicated `## Additional Requirements Specification` section at the overview-to-detail boundary (after §2 Entity summary and its diagram, before §3 Entities catalog; unnumbered, so it does not renumber the derived §1-§9 schema; conditional, emitted only when non-empty). It is specialist detail for the architect / analyst, so it heads the detailed sections rather than interrupting the high-level overview; the modeler / deployer honors it at spec time. This is the architect's directive to the modeler, NOT a field-contract copy (the catalog still models no fields, see the Phase-B field-boundary note). Author it ONLY for starters / bundles where the gap is real; leave empty on full modules. First write into the empty field is free; never overwrite a reviewed value without explicit user approval (Rule #20 shape). Not an audit blocker, and NOT part of regular per-domain research, it is reached only on the starter / bundle authoring path.

**This field is PUBLISHED** (it renders into the blueprint), so two hard constraints on its content: (1) NO vendor / competitor product names, ever (Rule #17, same as every other emitted field, a violation already shipped once here). (2) Keep it architect-actionable: state WHAT to implement and the coexistence / upgrade caveat the modeler can act on (e.g. "standalone-only field, deduplicate when the full module joins"); do NOT include derivation backstory that assumes knowledge of other modules' internals, the architect reading a standalone blueprint does not have it and cannot act on it.
...
**Upgrade behavior.** When a tenant later installs the full module whose data_objects the starter embedded, the embedded shells deterministically demote via the existing rule for `embedded_master` rows with a canonical master elsewhere. No tenant-side data migration (same `data_object_id`, same table). The starter's baseline permissions stick around after upgrade; its re-prefixed workflow gates (invariant #4) are superseded by the full module's canonical-prefixed gates once it installs (provisional cleanup, revisit after first real starter ships). The catalog ships no starter-specific skill: a single-domain starter is served by the host domain's skill, and a cross-domain starter by its bundle-domain skill (the FULL module carries no skill of its own either; its tools roll up into the host domain's skill via `domain_module_tools`).

**Audit blocker.** A starter row violating any of the six invariants fails the per-domain checklist on F2 / F3 (domain skill / toolset) or a Rule-#11 / Rule-#12 cross-check depending on which invariant is violated. The platform-side validation_rule throws on the master-role violation independently of the audit.
...
**Backfill empty fields by WRITING them at `record_status='new'` AND mirroring the written copy into the `q-` file for confirmation.** Catalog UX copy is buyer-facing marketing. On any domain audit where A4 fails (empty `domains.catalog_tagline` / `domains.catalog_description`) or M8 fails (empty `domain_modules.catalog_tagline` / `domain_modules.catalog_description`), author the buyer-voice copy and **write it into the empty column at `record_status='new'`**, AND surface that exact written text in the domain's `q-` file as a catalog-copy confirmation block (one per written field, per row: the domain itself and each hosted `domain_modules` row). Writing the column is fine and expected (Rule #21 corrective-additive); the part that is NOT optional is the `q-` file confirmation surface, the user must be able to read the copy you wrote and confirm it (or hand back a correction, which is the approved overwrite). Empty catalog copy is a **BLOCKER**: a domain with any empty `catalog_tagline` / `catalog_description` on itself or any hosted module is not "finished", and the site emitter refuses to publish it (`scripts/emit_domain_map.ts` hard-stops on any released domain/module with empty catalog copy). Copy that is written but not yet confirmed keeps the domain `feedback_needed`: the confirmation stays an OPEN `b1b` item in `state.yaml` (blocked on user confirmation) and is re-rendered in the `q-` file every audit until the `a-` file answers it, so it survives re-audits even though the column is now non-empty. Apply an empty-guard per field on the WRITE: only an empty column is written; a non-empty column is governed by the overwrite rule below (but its confirmation item, if still open, persists). This applies to subagents too: a subagent writes the field AND hands back the written copy for the `q-` file.
...
- Leaving an empty `catalog_tagline` / `catalog_description` empty and moving on; or writing the copy into the column WITHOUT mirroring it into the `q-` file for confirmation; or burying it in `history.md` / a chat message. Empty catalog copy is a blocker: write it at `record_status='new'` and surface the written text in the `q-` file for confirmation (see the backfill rule above); only non-empty values are overwrite-protected.
...
**When a review ends "waiting on the user" on any market-shape decision, the vendor research that grounds that decision (Phase 0) is part of the SAME review pass that produces the q-file, never a step deferred until the user objects (Rule #22 "Market-shape decisions require Phase 0 FIRST"). Surfacing a market-shape decision to the user without the named-vendor evidence already in hand is the review failing, not the user being consulted.**
...
- **Corrective-additive work the agent executes without asking** — and ONLY *corrective* gap-fill on the domain's ALREADY-SCOPED footprint: fill an empty field, classify `entity_type`, author `record_status='new'` content (catalog text per Rule #20, aliases, lifecycle states, trigger_events, personas + RACI, APQC tags) on entities/modules that already exist, backfill module FKs. Everything lands at `record_status='new'`; the user reviews the result in the records, never via a chat gate. Rule #1 still holds: never stamp `approved`. **Catalog UX copy (`catalog_tagline` / `catalog_description`) carries one extra obligation on top of being written:** the written text is ALSO mirrored into the `q-` file for the user to confirm (Rule #20, Rule #22), and an unconfirmed write keeps the domain `feedback_needed` (open `b1b`) rather than "finished".
- **Expansive additions the agent NEVER writes before a `q-` file (the research carve-out):** net-new market *structure* — new `data_objects` / masters, new `domain_modules`, new `regulations` + `domain_regulations`, new `capabilities`, new domains, or a master moved between modules — whether it came from research, a `b3` idea, or a market audit. These are market-shape decisions: surface them in a `q-` file FIRST (Rule #22, with the Phase 0 named-vendor evidence already in hand) and write them to the catalog ONLY after the `a-` file approves them. **A greenlight to "research X" / "add the ones that hold up" authorizes producing the analysis, the Phase 0 report, and the `q-` file. It is NEVER a license to write the new structure to the live platform before that `q-` file is answered** (`b3` is "never auto-execute" for exactly this reason; "draft, don't load" / workflow step 3 is the same gate). The corrective-additive license in the bullet above does NOT reach expansive additions: writing research-derived entities straight to the catalog at `record_status='new'` is a Rule #21 violation, not a shortcut. On catching it, revert the unapproved writes (or surface a one-question keep/revert gate) and produce the `q-` file. The rule is mood-independent: a prior "stop asking me" never collapses the research → `q-` file → approve → load sequence into a direct write.
...
**Grounding travels WITH the decision, never in a decoupled section (the structural rule that makes a q-file answerable).** A q-file is a faithful projection of `state.yaml`, so a q-question is only as grounded as the `b2` item it is rendered from. Therefore every market-shape `b2` item (module split/count, scope, what a domain masters vs consumes, promote-to-domain, where an entity is mastered) MUST carry an `evidence` field holding the named-vendor grounding for THAT specific decision (which vendors package it which way, by name; what each masters vs consumes), and the q-file's `Recommended:` reason is **rendered by copying that `evidence` field**, never re-derived at q-file-write time and never left to point at a "Vendor surface basis" paragraph in `history.md`. The failure this prevents: the vendor surface is authored once as a `history.md` section, the `b2` item and the q-question carry only the question + options, and the agent writing the q-file (a later pass, research context gone) fills the reason with the cheap default ("maps cleanly onto the capabilities / cleanest / unblocks the build"). If the `b2.evidence` field is empty, the q-question CANNOT be written: run Phase 0 and populate it first (see "Market-shape decisions require Phase 0 FIRST" below). The evidence field is the single carrier from Phase 0 → `b2` item → q-file; nothing downstream re-discovers it.

**`state.yaml` hygiene: OPEN items only, no disposition tombstones.** Every row in `state.yaml` is something still open. The instant an item is **resolved** (executed), **closed** (decided won't-do), **routed** to another domain, or **superseded** (made moot by a rule/plan change), DELETE it from `state.yaml` and write a one-line dated note in `history.md` in the **same** pass. "Dispositioning" an item means moving it OUT, never tagging it in place: **there is no `disposition` / `status` / `route_to` / `held_out` / `closed` field on a state item**, and inventing one to keep a decided row in-file is the exact bug that bloats `state.yaml`. Only the schema fields, the documented optional fields ([audits/README.md](../../../../audits/README.md) "Standard optional fields"), and `extra_`-prefixed extras may appear. Two carve-outs: **(a) a `postpone` / defer answer keeps the item OPEN** — postpone is deferred-not-done, so the item STAYS (as `b1b` with `blocked_by: {type: user_decision, ...}` for a deferred action, or `b3` for a parked additive idea); never delete a postponed item. **(b) Routing an entity to a neighbor domain** resolves it HERE (one-line `history.md` note) AND is added to the neighbor's backlog so it is not lost. End every audit by re-reading `state.yaml`: if any row is not genuinely open, it belongs in `history.md`. A clean `state.yaml` answers exactly "what is still open on this domain?" and nothing else.
...
**What goes in the `q-` file.** Only items that need the user: every `b2` decision, every pending destructive approval (Rule #21), and every **catalog-copy confirmation** (buyer-voice `catalog_tagline` / `catalog_description` the agent wrote at `record_status='new'` for the domain or a hosted module, surfaced verbatim for the user to confirm or correct, Rule #20). A catalog-copy block shows the written wording and asks the user to confirm it (or supply a correction, which is an approved overwrite). Open `b3` ideas go last under an "Optional (will not hold up the build)" heading. Nothing else (no `b1a`; no `b1b` other than these catalog-copy confirmations). **`record_status` approval is NOT a q-file item** (distinct from catalog-copy confirmation, which IS): approving / rejecting records (setting any non-`new` `record_status`) is out of band, it never rides the `q-`/`a-` mechanism and requires the separate explicit, direct user request of Rule #1. Confirming the catalog COPY authorizes nothing more than keeping the text the agent already wrote (a correction overwrites it); it is never a `record_status` flip. Do not list `record_status` flips as q-file questions, and never treat an a-file answer (including a "go" rename) as authorization for one.

**Market-shape decisions require Phase 0 FIRST (the forcing step, non-negotiable).** Any `b2` that decides market shape (module split or count, what is in scope, what a domain masters vs consumes, whether to promote a sub-market to its own domain, where an entity is mastered) MUST be backed by a CURRENT Phase 0 vendor-surface report ([references/vendor-research-protocol.md](references/vendor-research-protocol.md)) saved to `.tmp_deploy/<DOMAIN>-phase0-<date>.md`, produced **as part of this review, before the q-file is written**. Researching the market is a step of the review, not something done after the user pushes back. The sequence is fixed: **run Phase 0 → write the q-file from its evidence → surface to the user.** A review that surfaces a market-shape `b2` without having run Phase 0 this pass is incomplete: run Phase 0, then write the q-file. This holds even when a prior audit already "described the vendor surface": a narrative basis is not a Phase 0 report; the report (named flagship vendors, a vendor-by-entity surface matrix, per-decision verdicts) is what each recommendation must cite. Each affected recommendation embeds the named-vendor evidence **inline** in the `q-` file (which vendors package it which way, by name, what each masters vs consumes), not merely a pointer to the report, so the user can decide from facts in front of them. A recommendation grounded in "cleanest / unblocks the build / what is already built" instead of vendor reality is a Rule-#22 violation **even when a report exists** (the report has to actually drive the call). When fresh Phase 0 contradicts a recommendation already sitting in a q-file, the fresh evidence wins: reverse the recommendation and regenerate the q-file. This applies to UNBUILT domains too: an unbuilt domain whose entire build is gated on market-shape `b2` calls is exactly the case that needs Phase 0 run before the user is asked anything.
...
- `---`, then one block per question. Each block, in order, with a blank line between every part:
  ...
  - `Recommended: <answer>. <reason>` on its own line. **For a market-shape question the `<reason>` is rendered by copying the `b2` item's `evidence` field (the named-vendor grounding); it is NOT re-authored from scratch at q-file-write time.** The reason must be grounded in evidence, not in build-convenience. For any decision about market shape (module splits, scope, ownership, what entity belongs where), cite what flagship vendors actually do, by name: how specific vendors package the domain into products / editions, or which model the entity vs not (vendor names are allowed here; Rule #18 bars them only from catalog text fields, not audit files). Surface the genuine trade-off honestly. NEVER recommend from "the minimal shape", "what unblocks the build", or "what is already built": those are reasons about us, not the market, and are not acceptable rationale. A market-shape recommendation with no vendor/market evidence is an incomplete q-file.
- **Grounding gate (transcript + lint, non-skippable when a q-file is written or refreshed).** Before declaring a domain `feedback_needed`, run `bun run scripts/analytics/qfile_grounding_lint.ts <DOMAIN_CODE>` and confirm it exits clean. The lint flags any market-shape q-question whose `Recommended:` line carries a build-convenience phrase ("cleanest", "maps cleanly onto", "minimal shape", "unblocks the build", "small enough to own", "what is already built") with no named-vendor token nearby. A flagged question is an incomplete q-file: pull the grounding from the `b2.evidence` field (populating it from Phase 0 if empty) and rewrite the `Recommended:` line. If the lint output for this domain is not in the audit transcript, the q-file is not done.
...
2. Map each answer to its decision ID via the footer, then execute under Rule #21: additive/corrective decisions applied at `record_status='new'` without asking; destructive *structural* decisions performed because this file IS the approval for them. The a-file is NEVER approval for a `record_status` change: it authorizes the structural steps it enumerates, but any `approved` / `pending` / `rejected` flip stays out of band and needs the separate explicit request of Rule #1 (never inferred from an a-file answer, never from the rename itself).
...
| Table | Holds | Hierarchical? |
|---|---|---|
| `domain_modules` | Autonomous deployable units inside a domain (`ATS-CANDIDATE-CRM`, `ITSM-INCIDENT-MGMT`, `KNOWLEDGE-MGMT`). Natural key `domain_module_code`. `domain_id` is the REQUIRED and ONLY home domain; a genuinely-cross-domain starter homes on its own `domain_kind='bundle'` domain. There is no additional-hosts table (a starter's touched markets are DERIVED from its `embedded_master` rows). | no |

### Junctions with qualifiers (12 entities)
...
| `domain_module_data_objects` | domain_modules ↔ data_objects | role (same 5-value enum as `domain_data_objects`) + necessity + `notes`. Once a domain has modules, `domain_data_objects` is a **derived rollup** from this junction (group by data_object_id, strongest role wins). The single-master rule applies at this layer: exactly one row per `data_object_id` may have `role=master`; the blueprint emitter throws if it sees more. `notes` is empty by default — see Rule #15. Also the source from which a starter's **host markets are derived** (the domains mastering its `embedded_master` entities), replacing the removed `domain_module_host_domains` table. |
...
| `skills` | Agent skills (domain / process / role). A `domain` skill is the agent surface for a **domain**: one per domain (`skill_type='domain'`, `domain_id` set) that **derives** its toolset from the domain's modules. Skills are domain-grain ONLY; there is no module-anchored skill. FULL modules get no skill of their own, and starters carry no skill: a single-domain starter is served by its host domain's skill, a cross-domain starter is promoted to a `domain_kind='bundle'` domain and served by that bundle domain's skill (Rule #17 / Rule #19 #6). A `process` skill wraps a cross-domain value stream (`process_id` set) and derives from `process_tools`; `role` wraps a user-role workflow. | `skill_type` enum (`domain` / `process` / `role`, default `domain`) + `domain_id` (domain skill, the only grain) / `process_id` (process). A skill stores **no** tools; it derives them from the unit it represents |
...
| `domain_module_tools` | N:M between `domain_modules` and `tools` - which atomic tools a module requires | `requirement_level` (`required` / `optional`). `notes` off-limits without user-approved wording per Rule #15. Computed unique key `<domain_module_id>.<tool_id>`. A domain's `domain` skill derives its toolset by rolling these up over the domain's PRIMARY (full) modules |
...
The domain's tool set is the union of `domain_module_tools` over the domain's **PRIMARY modules** (`domain_modules.domain_id = X`), deduped by `tool_id`. A starter that merely embeds one of this domain's entities is a CONSUMER of the market (Rule #19: starters never master, only embed), not a contributor to it, so its tools are NOT this market's tool needs and are NOT folded in — the score is a property of the market, not of whatever persona bundle borrows one of its entities. A process unit substitutes `process_tools` keyed on `processes.id`.
...
- For the **per-domain** score, union the numerators and denominators of `domain_module_tools` across the domain's **PRIMARY modules** (`domain_modules.domain_id`), then divide. Starters that embed one of the domain's entities are NOT included (their tools are not the market's needs; see the tool-set rule above). **NO fallback:** a domain whose modules carry no `domain_module_tools` reads as incomplete (band F3), not as 0-of-0 covered. Do not average per-module scores - that biases toward small modules.
...
5. **Load via the script idiom — in multiple phases, each surfaced separately.** Even for ~20 rows, prefer extending [scripts/loaders/load_research.ts](scripts/loaders/load_research.ts) over one-off CLI calls. The script is idempotent (safe to re-run), chunked (no Windows command-line issues), and produces a row-count summary you can paste back to the user. Dated drafts in `.tmp_deploy/` are gitignored, so iteration is free; reusable patterns graduate to `scripts/loaders/`.
   ...
   Surface each phase as a **separate draft** before loading — reviewers check Phase 0 (vendor surface enumeration), Phase A (vendors/capabilities/modules), Phase B (data-object footprint), Phase C (functional ownership), and Phase S (domain skills + tools) with different mental models. Bundling them into one preview means one of them gets a shallower review.
   ...
   **Phase A — Market shape** (load first):
   ...
   - **`domain_modules`** — at least 1 row per Rule #14. For domains with ≥3 capabilities: ≥2 full modules (`module_kind='full'`). For domains with <3 capabilities: exactly 1 full module. Every module has exactly one home domain (`domain_modules.domain_id`); a cross-domain starter homes on its own `domain_kind='bundle'` domain and its touched markets derive from its `embedded_master` rows (no host-junction table). Starter kits (`module_kind='starter'`, per Rule #19) are optional and not subject to the floor.
   ...
   **Phase B — Data-object footprint** (load second, against the same domain). The Phase-B contract ships seven deliverables:
   ...
   1. **`data_objects`** that the domain masters (3–8 noun-plural snake_case names; apply the "what would a flagship vendor build their schema around?" test). Each row populates `singular_label`, `plural_label`, and — per Rule #12 — the `has_personal_content` hint where the master entity holds owner-scoped rows. Apply Rule #9 (naming arbitration) before insert. Also author a short `catalog_description` (1 to 2 sentences, **plural voice matching the plural Name, no leading article** — "Contracts with counterparties...", not "A contract with...") on every master: it is the reader-facing surface the §2 Entity summary renders, kept distinct from the analyst-facing `description` (which feeds the agent skill spec and the B13 keyword heuristic). Keep it short, especially when the `data_object_name` is already multi-word and self-describing. Fill the empty field freely; never overwrite a non-empty value without explicit user approval (Rule #20 shape). Field shape in [references/module-shape.md](references/module-shape.md).
   ...
   5. **`data_object_aliases`** — ≥1 alias per non-self-explanatory master (industry synonyms, vendor-specific labels). The blueprint generator embeds these directly.
   ...
   Field-level constraints on embedded shells (which columns the local copy must include when a module embedded_masters another domain's data_object) are NOT a domain-map concern. The deployer validates embedded shells against the canonical entity's `fields` metadata at deploy time. The catalog does not duplicate that field-level contract. The one prose escape hatch is `domain_modules.specification_requirements` (Rule #19): a starter / bundle MAY record, as a directive to the modeler, a field its promise needs that the canonical master does not carry (e.g. a flat cost field the starter denormalizes after dropping the full module's cost entities). That is authored intent, not a field-contract copy; the catalog still models no fields.
   ...
   **Phase S - Domain skill + tools** (load fourth, against the same domain). Under Rule #17 every domain needs exactly one domain-grain `skill_type='domain'` skill that **derives** its toolset from the domain's modules' `domain_module_tools`. Skills are domain-grain only: a single-domain starter is served by the host domain's skill, a cross-domain starter by its bundle-domain skill; a FULL module gets no skill but carries its tool requirements as `domain_module_tools`. Phase S delivers:

   1. **`skills`** - one row per **domain** with `skill_type='domain'`, `domain_id` set, and `skill_name='<domain_code_lower>'` (e.g. `crm`, `ats`; no suffix). Author its `description` (trigger-shaped routing prose: what the agent can DO, in the natural-language phrasing a user would say) AND `trigger_keywords` (comma-separated search terms, the complement of the prose) **together in one pass, deduped against each other**. Both are REQUIRED: the emitter errors and F8 fails if either is empty. If an existing `description` is internal / meta-shaped rather than trigger-shaped, rewrite it in the same pass (a non-empty overwrite, surfaced per Rule #1 / #21). Starters and FULL modules get no skill row of their own. The skill encodes what an agent can do across the domain's data and lifecycle; it derives its toolset and stores no tools.
   ...
   **What `description` + `trigger_keywords` must (and must NOT) contain.** The `description` is the AGENT-ROUTING surface, not buyer copy. Write it as the **jobs a user would ask the agent to do, in their words** ("capture and qualify leads, schedule showings, request e-signatures, carry a deal from offer to closing"). EXCLUDE everything that does not help an agent decide to load the skill: buyer-segment / company-size framing ("solo / small firm" is buyer copy, it lives in `catalog_tagline` / `catalog_description`, NOT the routing description), the deployment ("in this Semantius deployment"), and the raw entity / workflow inventory ("Workflows: Contract Repository, Account and Contact Management ..."). For `trigger_keywords`, the trigger match is **semantic** (an LLM reads the prose), so a keyword earns its place ONLY when the prose does not already evoke it. A near-synonym of the domain name, or a word already in the prose, is dead weight: with "real estate agent" in the prose, "real estate broker" and "realtor" add nothing (the skill already triggers on them). Keep ONLY genuinely different vocabulary a user would actually type that the prose does not use: domain jargon (`MLS`, `escrow`, `open house`, `closing`), adjacent intents, alternative phrasings. The generator's substring dedup is only a floor for exact repeats; **semantic dedup against the prose is the author's judgment** (drop a keyword if the model would already trigger on it from the prose). Size the keyword set to what survives that test, not a fixed count.

   Phase S is **not optional** per Rule #17. The killer hypothesis of the entire tools/skills layer (*"how many of the loaded domains have a domain skill where every required tool is Semantius-covered?"*) is uncomputable without it. Reference Phase-S loader pattern: the ATS / SMP / TALENT-MGMT / EMP-EXP loads set the shape; mirror them when authoring for other domains.
...
**Blueprint emission is not part of this checklist.** The [blueprint generator](../../../scripts/generate_blueprints.ts) is an explicit, user-triggered step — see § "Blueprints (explicit step, not part of any sequence)" below. The audit reads live state from PostgREST, not from on-disk blueprints, so the rendered files can sit stale through any audit pass and that's fine.
...
- Expected-non-zero call (the schema doesn't carry this; it follows from catalog rules):
  ...
  - Routinely zero: `domains.parent_domain_id` (only when this domain has sub-domains).
...
For every `domain_modules` row of this domain (its primary modules, `domain_modules.domain_id = <id>`), count `domain_module_capabilities` and `domain_module_data_objects`. Surface as:
...
- Fix: author the buyer-voice copy, write it into the empty field(s) at `record_status='new'`, AND surface that written text in the domain's `q-` file as a catalog-copy confirmation block (Rule #20, Rule #22). Empty `catalog_tagline` / `catalog_description` is a BLOCKER; copy written but not yet confirmed keeps the domain `feedback_needed` (the confirmation stays an open `b1b` item in `state.yaml` until the `a-` file answers it). Never overwrite a non-empty value without explicit per-row user approval; a user correction returned via the `a-` file IS that approval; marketing may have fine-tuned the original.
...
- Query: `/domain_modules?domain_id=eq.<id>&select=id,domain_module_code,domain_module_name` (a domain's modules are its primary modules; `domain_modules.domain_id` is the one and only home domain).
- Pass: result ≥1.
...
- Module count query: same as M1 (the domain's primary modules).
...
- Fix: link the orphan capability to whichever module it best belongs to, OR drop the capability if it shouldn't be in the catalog. An orphan capability is a Phase-A gap that propagates into the domain-skill derivation.
...
- Module set query: `/domain_modules?domain_id=eq.<id>&select=id,domain_module_code,catalog_tagline,catalog_description` (the domain's primary modules).
...
- Fix: author each empty module's buyer-voice copy, write it at `record_status='new'`, AND surface that written text in the domain's `q-` file as a catalog-copy confirmation block, one per written field per module (Rule #20, Rule #22). Any empty module `catalog_tagline` / `catalog_description` is a BLOCKER; copy written but not yet confirmed keeps the domain `feedback_needed` (the confirmation stays an open `b1b` item in `state.yaml` until the `a-` file answers it). Never overwrite a non-empty value without explicit per-row user approval; a user correction returned via the `a-` file IS that approval; marketing may have fine-tuned the original.
...
- Fix: **embed** (`embedded_master`, carry a local shell) is the structural fix and restores self-containment on its own: the canonical owner becomes required-if-present (plan-4) and is never dragged in, so an `embedded_master + required` row is already self-contained (embedding does NOT require relaxing the FK). Relaxing the existing row to `necessity=optional` (it degrades gracefully without the master) is the NARROWER alternative, valid only for a pure read the module can lose entirely when the master is absent; it is not a co-equal of embedding. Do NOT "document" the dependency: a hard prerequisite is a modeling failure, not a deploy fact. (This replaces the abandoned "deployable closure / required modules" idea: related modules are an informational hint in the blueprint front-matter `related_modules`, never a requirement.)
- Surfacing (Rule #22): this rewrites existing rows, so it is destructive and surfaced for sign-off, but do NOT frame it as a flat co-equal "embed (a) vs optional (b)". Recommend **embed**, then make `required` vs `optional` a SEPARATE per-`data_object` necessity call, naming each flagged entity and its recommended necessity (required-if-present where the module always models the record, optional where many real deployments skip it). Defaulting the whole set to optional is wrong: `necessity=required` feeds the coverage math (Signal 1 excludes optional) and required-if-present already keeps required embedded rows self-contained.
...
**M10. Every module name is self-explaining standalone (no weak / generic name). HARD BLOCKER.**

`domain_module_name` is the published label: the emitter renders it as the blueprint front-matter `system_name`, and it is the title a buyer reads on a per-module catalog card. It is read WITHOUT its `domain_module_code` and WITHOUT the surrounding domain, so a name that only makes sense once you know the code prefix is a defect. The signal: `domain_module_code` always carries the domain (`ATS-OFFERS`, `INTRANET-GOV-INVENTORY`), but `domain_module_name` often drops it (`Offers`, `Inventory and Sources`), leaving a bare generic noun that reads as belonging to several unrelated markets.

- Query: `/domain_modules?domain_id=eq.<id>&select=id,domain_module_code,domain_module_name` (the domain's primary modules; same module set as M1 / M8).
- Pass (JUDGMENT, grounded in this domain's review, like B13, NOT a mechanical string predicate): read each `domain_module_name` in isolation, with the code prefix and the domain hidden. It passes when a reader can tell roughly what market it belongs to: the self-explaining shape is **qualifier + noun** (`HR Knowledge`, `Contract Repository`, `Supplier Performance Management`) or an inherently domain-specific phrase. It FAILS when the name is a bare generic noun phrase that plausibly fits several unrelated domains (`Offers`, `Interviews`, `Knowledge Management`, `Renewal Management`, `Inventory and Sources`, `Request Intake`, `Agent Workspace and Reporting`). Industry-standard terms that are unambiguous within their discipline (`Incident Management`, `Problem Management`) pass.
- Fix: renaming `domain_module_name` is a DESTRUCTIVE overwrite of a non-empty value (Rule #1 / Rule #21), so it is NEVER auto-applied. For each weak name, draft a disambiguated replacement (prepend the missing domain qualifier to the existing noun, e.g. `Offers` -> `Job Offers`, `Knowledge Management` -> `IT Knowledge Management`, `Inventory and Sources` -> `Intranet Content Inventory and Sources`) and surface it in the domain's `q-` file as a yes/no rename approval per weak name (Rule #22). A user "this short name is intentional" answer WAIVES the finding for that module.
- Hard blocker: a domain with any weak module name on itself or a hosted module is NOT "finished" until every weak name is either renamed (on the `a-` file approval) or explicitly waived. Because the only fixes are an approved overwrite or a waiver, an open M10 finding keeps the domain `feedback_needed` (the rename approval / waiver is an open `b2` in `state.yaml`, surfaced in the `q-` file every audit until the `a-` file answers it), exactly like a pending destructive approval.
- Re-emit: after any approved rename, the module's blueprint `system_name` is stale; flag it for the next explicit blueprint emit (Rule: emission is a separate user-triggered step, never folded into the audit).
...
**B4. `has_personal_content` hint considered on every master.** (Rule #12.)
- Query: `/data_objects?id=in.(<masters>)&select=data_object_name,has_personal_content`
- Pass: every master row exists (the hint defaults to false; `false` is a valid considered answer). What the checklist actually verifies is whether it is true OR a notes/audit trail confirms the consideration. **An audit MUST positively re-evaluate** — false-by-default is not the same as false-after-review.
- Fix: PATCH the hint to `true` where applicable. Record the audit pass in the gap report / PR description / chat exchange, **not** as a `notes` annotation (Rule #15).
...
**B9d. Handoff payload realization: BIDIRECTIONAL, every boundary processed both directions (SEMANTIC, not structural). Non-skippable.**

The structural bands (B9 existence, B10b module FKs) confirm the handoff ROW and its FKs. They never check that the handoff's APQC payload (`handoff_processes.process_id`) resolves to REAL work. A handoff can be structurally perfect and tagged with a PCF process that nobody realizes, that sits at the wrong hierarchy level, or that belongs to an unrelated process category. That is semantic drift between the handoff layer and the realization layer (gated `data_object_lifecycle_states.process_id` + `process_raci`), the same two-unjoined-models class B9c guards for `trigger_events`. **A handoff review is NEVER complete on the structural bands alone. This band is the semantic half and is non-skippable: a handoff connects two domains that can drift, so its payload must be reconciled against realized work, not just confirmed to exist.**

- **NOT outbound-only. Process the full boundary, both directions.** The structural bands B8/B9/B10 are outbound-only for ROW AUTHORSHIP only: you never fabricate an inbound handoff ROW on behalf of a domain you have not audited (no provenance). B9d authors no handoff rows. It reads handoffs that already exist on this domain's boundaries and asks whether each payload is realized work, which is true or false regardless of which side published the handoff. So B9d processes BOTH directions on every boundary. Collapsing B9d into the outbound-only row-authorship rule is the split-brain bug that made this band do nothing: when each side calls the other side's payloads "report-only", a process owed across the boundary gets realized by neither. When auditing this domain, enumerate every related domain (every neighbor reached by a handoff in either direction) and classify EVERY payload on each boundary, `this->neighbor` and `neighbor->this`.
- Inputs:
  - Payload tags BOTH directions: `/handoff_processes?select=process_id,record_status,handoffs!inner(source_domain_id,target_domain_id)&handoffs.source_domain_id=eq.<id>` for this-domain-as-source, and the symmetric `handoffs.target_domain_id=eq.<id>` for this-domain-as-target. Process both; the inbound half is NOT report-only.
  - Realized set + WHERE realized: `/data_object_lifecycle_states?process_id=not.is.null&select=process_id,domain_module_id` joined to `/processes?select=id,process_code,process_key`. The `domain_module_id` maps each realized code to its realizing domain — the ORPHAN owner signal.
  - RACI personas: `/process_raci?select=process_id,raci,actor_role_id` joined to `/domain_roles?select=id,role_code,role_name` — gives each realized process its R/A/C/I personas, and is the pool an ORPHAN's candidate persona is drawn from.
  - Each endpoint's persona pool: `/domain_roles` reachable via `/role_modules?select=role_id,domain_module_id` to that endpoint's modules. Needed to decide which endpoint owns an ORPHAN's subtree and to pick its candidate R/A.
  - Payload `process_code` + `hierarchy_level` from `/processes`.
- Classify each payload on the boundary (BOTH directions) by its APQC code C **in this order; first match wins** (hierarchy = `process_code` dotted-prefix; category = first segment). Realized = a process whose `process_id` is the gated `data_object_lifecycle_states.process_id` AND that carries `process_raci`:
  1. **RESOLVED** — the **exact** code C is realized: C's own `process_id` is the gated process AND C carries `process_raci` with ≥1 R and ≥1 A. RACI on C without C being the gated process is NOT resolved (a parent with roll-up RACI whose gate sits on a child is ROLL-UP, never RESOLVED). No action.
  2. **ROLL-UP** — C is not exactly realized, but a strict ancestor or descendant of C is the realized code. The work IS realized, at a different grain. Fix: re-point `handoff_processes.process_id` to the realized code (`record_status='new'`). Mechanical, no new authoring, NOT a coverage gap.
  3. **MIS-TAG** — C is unrealized AND its top-level APQC category is in **neither** endpoint's family (read each endpoint's family from the categories it actually realizes; e.g. HR domains → category 7). A wrong tag, not missing work (e.g. a category-2 product process on an HR boundary; the `10145`-subscription class, see [discover-cross-domain-processes.md](references/discover-cross-domain-processes.md) Pass 1). **The sender owns it** (it authored the tag): surface for arbitration; re-point or delete the `handoff_processes` row on sign-off. **This is checked BEFORE ORPHAN — a wrong-category code is a mis-tag, never a coverage gap. (If ORPHAN ran first it would swallow every mis-tag, since both are "unrealized"; order is load-bearing.)**
  4. **ORPHAN**: C is unrealized but its category DOES fit an endpoint: real missing work, i.e. **a process with no persona** (the original symptom this whole band exists for). Resolve by **OWNER, on either side, written down NOW**:
     - **Owner** = the domain that **masters the carried entity** (its `master` / `embedded_master` `domain_module_data_objects` row). This is the **primary, authoritative, unbuilt-safe** signal (it works when the owner has no personas yet). The realized-nearest-sibling cross-check is **corroborating only and is SKIPPED when the owner is unbuilt**: an unbuilt owner realizes nothing, so the absence of a realized sibling must NEVER downgrade a correctly-tagged ORPHAN. One process can carry entities mastered by DIFFERENT domains, so the finding grain is per `(process, owner)`, not per process. If the carried entity has no master row anywhere, do not silently drop it: surface it as an unowned dependency on the sender.
     - **Detect the persona.** R and A come from the OWNER's persona pool (the Responsible persona must reach the module where C is gated, so only the realizer's persona can be R). Suggest the candidate by mirroring the owner's nearest already-realized process in the same subtree (e.g. an unrealized HR-case process inherits the owner's existing case process's R/A); the other endpoint's persona is C/I only. If the owner has no realized sibling and no persona pool yet (cold-start, e.g. a domain with 0 personas like SWP), say so in the item and make the persona itself part of the question, do not skip the ORPHAN.
     - **Write it into the OWNER's audit artifacts in THIS pass, whichever side the owner is.** Add a durable `b2` item with id `B2-B9D-OWN-<process_id>` to `audits/<OWNER>/state.yaml` AND surface the question in `audits/<OWNER>/q-<OWNER>.md`, worded plainly per the q-wording contract (lead with the business problem, options as role names, recommended + reasoning, codes only in the agent-map footer). Model it as a **`b2`** (a decision the owner must make), NOT a `b1b`: a `b1b`-backed q is illegal under the q-file content rule (§ "What goes in the `q-` file"), and a `b2` is q-file-legal by construction and forces the options + Recommended the wording rules require. This is the SAME mechanism entity routing uses to write into a neighbor's backlog (§ state.yaml hygiene carve-out (b)), and it holds whether the owner is THIS domain or the neighbor: when the owner is the neighbor you ARE writing into the neighbor's `state.yaml` + q-file during this audit, which is correct and required. You are only surfacing a question and (on the answer) authoring additive `record_status='new'` realization rows on the owner's side; you never overwrite the neighbor's data and never flip `record_status`. There is NO "latent", NO "derived, re-derive it later", NO cross-pointer hole: the item is recorded on the owner the moment it is found.
     - **Both sides close in one pass.** Because B9d walks both directions and writes to whichever side owns each payload, auditing EITHER domain on a boundary reconciles that boundary on both sides. No second audit is needed to collect the other half. A payload that has been realized and given a persona classifies RESOLVED on the next run and the item drops off.
- **Direction governs ONLY the row-level TAG edit, never the realization.** Re-pointing or deleting a `handoff_processes` row edits the handoff the SOURCE authored, so that one edit is recorded against the source's audit. The realization, the persona, and the q always follow the OWNER, on either side. There is no "inbound payload" B9d skips.
- Pass: every payload on every boundary (both directions) is RESOLVED, ROLL-UP-repointed on the source, realized-with-a-persona-q on the owner (ORPHAN), or surfaced for arbitration on the source (MIS-TAG). No payload is left as "report-only".
- Rule #1: every re-point, new `process_raci`, and realize-here write lands at `record_status='new'`; a `handoff_processes` deletion (mis-tag) is destructive and needs explicit user sign-off.
- **Run the committed resolver.** This band is executed by `scripts/analytics/b9d_resolver.ts`: `bun run scripts/analytics/b9d_resolver.ts <DOMAIN_CODE> --dry-run` to preview, `--write` to apply the additive owner-side `b2` + q edits. Destructive re-points / mis-tag fixes and the one-time `--migrate-legacy` sweep are surfaced for sign-off, never applied unprompted. It is the per-domain instance of the catalog-wide resolver. `validate cross-domain` (mode b2) is now the **structural substrate check only**; B9d ownership routing lives in `b9d_resolver.ts`, not in that deferring validator. **Transcript gate:** if the resolver's both-directions classification output for this domain is not in the audit transcript, the audit is INCOMPLETE. See [discover-cross-domain-processes.md](references/discover-cross-domain-processes.md).

**B9e. Per-function RACI reveal: Phase E must author the `process_raci` the function view reads; an empty function is unauthored RACI, not a finished domain. Non-skippable on a released, persona-bearing domain.**

B9d realizes a handoff's payload as `process_raci` R/A on the OWNER. B9e is the function-centric VIEW of that same `process_raci` (R/A plus the C/I rows): `catalog/domain-map/business-functions.json` (emitted by `bun run scripts/emit_domain_map.ts`) carries a `raci[]` per business function, one self-describing row per `(domain, responsibility, persona, persona_via, process, RACI-letter)`, so a reader can say WHO is R/A/C/I for each market a function owns, contributes to, or consumes WITHOUT joining `personas.json`. The reveal is only as complete as the domain's authored RACI: a domain that finished Phase E with personas but no `process_raci` (or with the lifecycle gates left unwired) shows its functions with `personas[]` (members) but empty `raci[]`. **This band is the check that this did not happen: for a released domain, re-emit and confirm its owner/contributor/consumer functions carry non-empty `raci[]` in the right market. An empty cell means RACI was not authored or the gate was not wired, NOT a finished domain.**

- **What the reveal reads (author all of it in Phase C/E):** `business_function_domains` (the market edges, Phase C), `domain_roles.business_function_id` (members), `role_modules` (released reach), `process_raci` (the R/A/C/I rows), and `data_object_lifecycle_states.process_id` (the gate that places each process in its market). A `process_raci` row whose process has no `process_id`-wired gate falls to handoff attribution or `(undeclared)` in the reveal, the SAME unwired-gate defect roles.md §5 / §8 flag for the derived grant: wire the gate.
- **Market attribution is GATE-DOMINANT.** A process's market is the domain that GATES it; a handoff-only process (gated nowhere) falls back to its handoff domain. So a process gated in domain X is not bled into a neighbor Y it is merely handed off to. Mirrors the gate-wins-over-handoff rule the per-domain `processes[]` uses for `via`.
- **The cross-functional seam is OWNER-ANCHORED.** A NULL-`business_function_id` persona (e.g. Hiring Manager) appears under the function whose subtree OWNS the domain its work is gated in, never under that domain's consumers or contributors. So a consumer function that owns no released domain reads honestly empty, not falsely realized by a borrowed actor.
- **Released-only, so NO catalog-wide state.yaml seed (unlike B9d).** The reveal, like all four emitted artifacts, is gated to released domains, and every released domain already carries complete RACI. RACI is authored per domain in its own Phase E (before release), so B9e rides the normal review and needs no one-time b1a seed across every `state.yaml`. B9e is also NOT universal: only operational domains with personas have RACI (about 1 in 9 domains today), so never manufacture `process_raci` for a domain with no operational persona.
- **Relationship to B9d.** B9d authors the R/A realization across a boundary; B9e is the per-function, per-market presentation of the full R/A/C/I and the check that Phase E authored it. B9e never authors `process_raci` itself; it verifies E and B9d did.
- **Transcript gate:** if a RELEASED domain's audit does not show its functions populating the reveal (re-emit and confirm), the audit is INCOMPLETE for B9e.

**B10. Inbound `handoffs`: ROW AUTHORSHIP ONLY (payload realization is B9d, never report-only).**
- **Scope of this band.** B10 is ONLY about not fabricating inbound handoff ROWS for a source domain you have not audited. **Payload realization on inbound edges is B9d's job and is NEVER report-only** (see the B9d band): an unrealized payload owned across the boundary is authored onto the OWNER in this pass, whichever side the owner is.
...
- Why this matters: without per-module attribution, blueprints attribute outbound events to every module in the source domain and inbound events to every module in the target domain — a single outbound event then appears duplicated across every module page in the source domain. The columns exist precisely so this can't happen.
...
**B13. `entity_type` classified on every master. Non-skippable; grounded in THIS domain's review.** (Rule #12.)
...
- Fix: classify each unclassified master to the correct value per Rule #12's enum (`operational_workflow` / `operational_record` / `catalog` / `junction` / `computed`) as a JUDGMENT made with the domain's full review context (the master's description, its lifecycle states, its handoffs, the Phase 0 vendor surface), surfaced in the domain's `q-` file alongside the rest of the review. NEVER classify `entity_type` from a context-free catalog-wide table: "is `agency_jobs` computed?" is undecidable without knowing what the domain is, so the grounding context is the domain review itself. The strongest single signal is already in scope: a master that carries `data_object_lifecycle_states` is `operational_workflow` (a thing with a state machine is a workflow); fold the B12 lifecycle read into this call. PATCH lands additive (`record_status` untouched, Rule #1); a row that genuinely fits no value is surfaced for the user, not force-classified.
- **Transcript gate.** If this domain holds an unclassified master and the audit transcript does not show the B13 query plus a grounded class for each one, the audit is INCOMPLETE. This is why the catalog accumulated a large unclassified backlog: handoff-focused partial audits ran B9d / B10b and skipped B13 with no trace, so the gap stayed invisible. To see every domain that still owes a B13 pass, run the query UNSCOPED (`/data_objects?entity_type=eq.unclassified&select=id,data_object_name,domain_module_data_objects(role,domain_modules(domain_module_code))`, keep rows with a `master` role); each clears on its next review, in context. The per-domain helper `scripts/analytics/b13_resolver.ts <DOMAIN_CODE>` proposes a first-pass class per master from the lifecycle + name signals for the reviewing agent to confirm or correct against the domain context, never a blind catalog-wide apply.
...
**B15. No `has_personal_content` on a `catalog` / `junction` / `computed` master.** (Rule #12; authoring-time gate for the emitter's M6 invariant.)
- Query: `/data_objects?id=in.(<masters>)&entity_type=in.(catalog,junction,computed)&has_personal_content=eq.true&select=id,data_object_name,entity_type,has_personal_content`
- Pass: empty result. `has_personal_content` is an operational-write concern and is only meaningful on `operational_workflow` / `operational_record`. The hint set on a `catalog` / `junction` / `computed` master is a self-contradiction: the emitter suppresses the override and prints `⚠ M6` at emit time (`deriveWriteTier` gives these classes an admin / neighbor / read-only tier with no per-row override surface). This band is the authoring-time gate so the contradiction is caught by the audit checklist, not left to a console warning that a checklist-only review walks past. B12 / B13 do NOT cover it (B13 only flags `unclassified`; B12 only flags `operational_workflow` missing lifecycle; B4 only verifies the hint was *considered*, not that it *contradicts* the class).
- Fix: **clear the offending `has_personal_content`; keep `entity_type` as-is.** The flag is ALWAYS the error here, never the class. It is an operational-write override with no meaning on `catalog` / `junction` / `computed` (the emitter already suppresses it via M6), so a hint on one of these classes is a stray setting to clear, not a signal to reclassify. **Having a lifecycle does NOT promote the entity to `operational_workflow`:** per Rule #12, `catalog` (config / definitions with publishing flows, e.g. `offer_letter_templates`) and `junction` (qualifier-carrying links, e.g. `talent_pool_memberships`) legitimately carry lifecycle states, and their approval gate is emitted from the lifecycle regardless of class. Who may approve is handled by the lifecycle gate + RACI; the entity stays admin-tier (`catalog`) / neighbor-tier (`junction`) / read-only (`computed`) for base writes, which is correct for configuration data. Clearing a non-empty flag is a DESTRUCTIVE overwrite (Rule #21 / Rule #1): surface the rows and apply only on sign-off. Never auto-PATCH, never stamp `approved`. (Reclassifying to `operational_workflow` would be wrong: it flips base writes `:admin → :manage`, handing edit rights on configuration data to managers, and it contradicts Rule #12.)
- History: the 2026-06-11 catalog-wide scan found 14 such rows, all resolved by clearing the stray flag (NO reclassification) per the decision recorded in [references/skill-changelog.md](references/skill-changelog.md). At the time the predicate spanned three flags (`has_personal_content` / `has_submit_lock` / `has_single_approver`); `has_submit_lock` and `has_single_approver` were dropped 2026-06-26, so the check is now the single `has_personal_content=eq.true` predicate above. (The 2026-06-11 reference loader `.tmp_deploy/clear_b15_pattern_flags_2026_06_11.ts` predates the removals and is historical.)

**B16. No British-English spellings in any authored text. American English only.** (CLAUDE.md "American English only" — project-wide, enforced here.)
- The catalog is American-English only. British spellings (`-ise`/`-isation`, `-our`, `-re`, `-ence`, `-logue`, BE double-`l` like `cancelled`/`modelling`, `programme`, `fulfilment`, `enrolment`, `judgement`, `behaviour`, `licence`, `centre`, `analogue`, etc.) are forbidden in every authored text column: `description`, `*_label`, `*_name` display names, `catalog_tagline` / `catalog_description`, `business_logic`, `specification_requirements`, `trigger_keywords`, `notes`. Like B15 this is a **catalog-wide predicate**, not per-domain: one tool finds every violation across all domains at once, so it can be swept globally rather than waiting for each domain's turn.
- Check: `bun run scripts/analytics/be_spelling_sweep.ts` (dry-run). It uses a CURATED BE→AE map (never a blanket `-ise→-ize`, which would corrupt AE-native words like `enterprise` / `comprise` / `surprise` / `customer`). It AUTOFIXES the authored-prose columns and REPORTS (never auto-changes) natural-key / code columns (renaming cascades to FKs) and externally-sourced names (APQC `processes`, statutes, vendor / solution proper names, which may legitimately carry a BE brand like the ERP named "Fulfil").
- Pass: the dry-run reports zero AUTOFIX hits for this domain's rows.
- Fix: `bun run scripts/analytics/be_spelling_sweep.ts --write` applies the AUTOFIX changes (spelling only; `record_status` untouched, Rule #1). REPORT-only hits (identifiers, vendor/solution copy) are surfaced for manual judgment — a BE token inside a product brand name is left alone; a BE token in our own prose about it is fixed by hand. The same AE rule binds every new write at author time, so a clean catalog stays clean and B16 is a regression guard, not a recurring cleanup.
...
> Blueprint emission used to live here as D1 / D2 of the checklist. It has been pulled out of every sequence (load, audit, fix-loop) and is now an explicit, user-triggered step. See § "Blueprints (explicit step, not part of any sequence)" below the audit recipe.
...
**E5. (retired 2026-06-26 — entity-level approver flag dropped.)**
- Accountability for an approval is now governed solely by `process_raci` (exactly one Accountable per gated process, already covered by E4). The former `has_single_approver` boolean on `data_objects` was removed because it duplicated RACI's single Accountable; there is no per-entity flag left to reconcile. Band number retained so the `E1–E6` range stays stable.
...
**F1. Every skill is cleanly domain-anchored; the module grain is retired.**
- Query: `/skills?skill_type=eq.domain&domain_id=is.null&select=id,skill_name`.
- Pass: empty. A `skill_type='domain'` row with NULL `domain_id` is an orphan (the `domain_required_when_skill_type_is_domain` validation rule blocks it platform-side; F1 audits the read path). The former module grain is fully retired: `skills.domain_module_id` has been dropped, single-domain starters use the host domain skill, and cross-domain starters use their bundle-domain skill (Rule #17 / Rule #19 #6).
- Fix: set the missing `domain_id` on the orphan, or delete it.

**F2. The domain has exactly one `skill_type='domain'` skill, anchored to the domain.** (Rule #17.)
- Query: `/skills?domain_id=eq.<id>&skill_type=eq.domain&select=id,skill_name`.
- Pass: exactly one row. Zero is a Phase-S gap; >1 is a rule violation (keep one, delete the extras; their tools already live on `domain_module_tools`). **`domain_kind='bundle'` is NOT exempt:** a cross-domain starter is promoted to a bundle domain (Rule #2) and carries exactly one ordinary domain skill there (the re-anchored former starter skill), so a bundle domain returns exactly one row here like any other domain.
- Fix: author the missing domain skill per Phase S in the workflow. Use `skill_name='<domain_code_lower>'` (e.g. `grc`, `ats`), `skill_type='domain'`, `domain_id=<id>`, plus its `description` (routing prose) and `trigger_keywords` (comma-separated trigger terms); load alongside the modules' tools and `domain_module_tools` in the same loader.

**F3. The domain's modules carry ≥1 `domain_module_tools` (the derived toolset is non-empty).** (Rule #17.)
- Query: over the domain's PRIMARY modules (`domain_modules?domain_id=eq.<id>&select=id`), then `/domain_module_tools?domain_module_id=in.(<modIds>)&select=domain_module_id,tool_id,requirement_level`. Starters that embed one of the domain's entities are NOT included (their tools are not the market's needs).
...
- Derivation: per the formulas in § "Semantius score", computed over `domain_module_tools` across the domain's PRIMARY modules (deduped by `tool_id`; starters that merely embed a domain entity are excluded): `strict_score = count(distinct platform-tier tools) / count(distinct tools)` and `operational_score = count(distinct platform+integration tools) / count(distinct tools)`. A per-module coverage view (which deployable modules are 100% platform-covered) is a separate rollup over each module's own `domain_module_tools`, computed from `module -> its tools` independently of the skill grain.
...
**F8. The domain skill carries a non-empty `description` AND non-empty `trigger_keywords`.** (Rule #17; generator-enforced.)
- Query: `/skills?domain_id=eq.<id>&skill_type=eq.domain&select=id,skill_name,description,trigger_keywords`.
- Pass: the one domain skill row has a non-empty `description` AND a non-empty `trigger_keywords`, and BOTH clear these gates (the emitter's `skillDescriptionFrom` ERRORS only on EMPTY, so the two content gates below are audit-enforced, not yet emitter-enforced):
  - **No third-party names (HARD fail, Rule #18).** Neither `description` nor `trigger_keywords` may contain a vendor / product / brand / trademark token (e.g. ServiceNow, Jira, Zendesk, Freshservice, Workday, Asana, DocuSign, Zylo). These two columns are NOT on Rule #18's commerce-shaped allow-list (`vendors` / `solutions` / `data_object_aliases` / `tool_solutions`, plus statutory names on `regulations`), so brand names are forbidden here exactly as in any other description field. Run the same proper-noun scan Rule #18 prescribes. Statutory / standards-body names (ITIL, HIPAA, SOX, WCAG, SCORM) are allowed; commercial product names are not.
  - **`description` is pure task prose (HARD fail).** It must NOT contain a `Use for:` segment or a trailing comma-run keyword list. The rendered `Use for:` tail is produced SOLELY by the emitter from `trigger_keywords`; an embedded tail in the description double-renders (two `Use for:` tails in the package) and fails.
  - **`description` is trigger-shaped (soft fail, rewrite).** Trigger-shaped task language ("track candidates from sourcing to offer; schedule interviews; send offers"), NOT internal / meta-shaped ("derives its toolset from the `domain_module_tools` union ..."), NOT led by buyer-segment / company-size framing, NOT a raw workflow inventory.
  - **`trigger_keywords` is additive and generic (soft fail, rewrite).** Comma-separated terms that are the semantic complement of the prose: generic category names, industry jargon, acronyms, synonyms, and adjacent terms a user might type that are NOT already words in the prose and are NOT brand names (`help desk`, `ticketing`, `ITIL`, `MTTR` yes; `ServiceNow`, `Zendesk` no). Padding with near-synonyms of the domain name or terms the prose already evokes is dead weight.
  - **Exempt for `domain_kind='bundle'`? No** - bundle domains carry a domain skill (F2) and are held to F8 like any other domain.
- Fix (ONE pass, per Phase S): author `skills.description` as pure trigger-shaped task prose (NO `Use for:` tail, NO embedded keyword list) and `skills.trigger_keywords` as the deduped, brand-free complement (generic category terms + jargon only), with both in view so no keyword repeats a word already in the prose and no vendor / product name appears in either. Backfilling an EMPTY field is additive (`record_status='new'`, no chat gate, like the Rule #20 catalog-UX backfill); rewriting a NON-empty `description` (meta-shaped, or carrying brand names or an embedded `Use for:`) is a destructive overwrite surfaced per Rule #1 / #21. **On failure the audit writes a `state.yaml` `b1a` item** with id `B1A-SKILL-COPY-<DOMAIN>` ("author the domain skill's `description` + `trigger_keywords` in one pass: brand-free, deduped, description as pure prose with no `Use for:` tail") so the next agent pass regenerates both; it is in-scope, agent-doable additive work, and clears on re-audit once both pass the gates above.
- Why this is here: `trigger_keywords` defaults empty and `description` is often seeded as internal prose, so without F8 the skill ships an empty or non-triggering routing surface while F1-F5 (which only check the skill row's existence + tools) tick green. F8 closes that, parallel to how A4 / M8 gate the buyer-copy fields. The brand-name and embedded-`Use for:` gates are HARD fails because a skill-copy pass that names competitor products in `description` / `trigger_keywords` violates Rule #18 (these columns are not commerce-shaped), and an embedded `Use for:` tail double-renders in the package. Both should also be added as an emitter backstop in `skillDescriptionFrom` (strip any embedded `Use for:`, reject brand tokens) so a human-review miss cannot ship them.
...
3. Run every **in-scope** band check (A / M / B / **B9d** / C / D / E / **B9e** / F / **H**) in order. Skip A5 unless the user has explicitly asked for a vendor-ownership refresh. **H1 is NOT optional**: audits that skip the H-band are incomplete; do not surface the gap report until H1 has been worked. **B9d is NOT optional either** and is run via `scripts/analytics/b9d_resolver.ts` in BOTH directions; an audit whose transcript lacks the resolver's classification output is incomplete. **B9e** (the per-function RACI reveal) is verified after Phase E: for a RELEASED domain, re-emit `business-functions.json` (`bun run scripts/emit_domain_map.ts`) and confirm its owner/contributor/consumer functions carry non-empty `raci[]`; an empty cell means Phase-E `process_raci` was not authored or its gates were not wired.
4. Classify each result:
   - **Structural gate** — M1–M7 failures block every downstream concern. A domain with no modules (or with capability-orphaned modules) can't be modeled in Phase B/E correctly until the M-band is clean. Resolve M-band first. (M8 is a content-quality check and M10 is a name-quality hard blocker; neither is a structural gate that blocks downstream modeling.)
   - **In-scope fix** — this domain can fix locally (S1–S3 zero-row anomalies, A1–A4, M1–M8, B1–B9, B9b, **B9d**, B10b, B11–B16, C1–C2, D1, E1–E6, **B9e**, F1–F5, F7, F8, **H1**). Goes into the **gap report** as actionable. (M9 self-containment and M10 weak-name renames are hard blockers whose only fixes are a destructive rewrite or a decision, so they surface for sign-off in the `q-` file rather than auto-loading at `record_status='new'`; they still block "finished".) Blueprint emission is **not** an audit step — see § "Blueprints" below. (B15, the pattern-flag-on-a-catalog/junction/computed-master check, and B16, the British-English spelling check, are both pure catalog-wide predicates: one query / one tool finds every violation across all domains at once, independent of which domain is being audited, so they can be swept globally rather than waiting for each domain's turn.)
   - **Report-only follow-up** — the symmetric side is owned by another domain (B8 inbound direction, all of B10). Goes into a separate **"report-only follow-ups"** subsection of the report, naming the source domain + the missing check ID on that side (e.g. "HCM B9 owes outbound on `hcm_positions`"). **Do not author fixes for these from this domain's audit. EXCEPT B9d payload realization: that is ALWAYS authored into the OWNER's files in this pass (run `scripts/analytics/b9d_resolver.ts`), on whichever side owns it, and is never report-only.** These items NEVER block the audited domain's green status; they are observations the user can act on by scheduling audits of the source domains.
...
A well-run audit produces two artifacts: the gap report (with in-scope and report-only subsections) and, if the user agrees to load, the fix drafts. Blueprint emission is not part of the audit deliverable; if the user wants refreshed blueprints after fixes land, that's a separate explicit step (see § "Blueprints" below).
...
1. Pull current state from live tables — `domains`, `domain_modules`, `domain_module_data_objects`. Never from blueprints, never from deploy scripts (Rule: live state only).
...
> **The per-neighbor pass plus boundary reconciliation (passes 3 and 4 of Validate b1; see [README.md](../../../README.md) § "Validate a domain").** Each boundary is reconciled in TWO PHASES below: Phase 1 (structural gate) and Phase 2 (semantic reconciliation), and only Phase 2 decides pass/fail. The neighbor set for domain X is **auto-discovered** from the catalog, not user-supplied. Query `handoffs?source_domain_id=eq.<X-id>` UNION `handoffs?target_domain_id=eq.<X-id>` UNION `domain_module_data_objects` rows on X's modules whose `data_object_id` is mastered in another domain. Rank by edge weight (number of handoffs + dependency count). Run both phases against every neighbor; the only allowed condensation is the light-neighbor rule under Phase 2 (a light neighbor still gets a Phase-2 verdict on every payload, never a structure-only summary). **Manual bilateral form** (user names two specific domain codes) skips neighbor discovery (pass 3) and runs the two phases directly on the named pair: triggers *"validate `<A>` ↔ `<B>` boundary"*, *"reconcile `<A>` and `<B>`"*, *"check the handoff boundary between `<A>` and `<B>`"*. Either form runs the same two phases below.

**What "reconciled" means — read this first; it has regressed FIVE times.** A boundary is RECONCILED only by the **Phase-2 Pass test** below (every payload RESOLVED, or ROLL-UP re-pointed AND that re-point approved; zero ORPHAN/MIS-TAG). Structural greenness is NEVER that test. Confirming the handoff rows exist and are wired (master, trigger event, module FKs, consumer DMDO, relationship mirror) is **structural validation, NOT reconciliation**: an edge can be fully wired and still carry a payload nobody realizes, a mis-categorized PCF code, or a parent code whose work lives on a child. If your output contains only structural findings, you VALIDATED the pair, you did not RECONCILE it. Phase 1 is the GATE; Phase 2 is the RECONCILIATION and alone decides pass/fail.
...
#### Phase 1 — Precondition gate (structural; NECESSARY, NOT reconciliation)

You cannot reconcile a payload on a broken edge, so structure runs first as a GATE. **Passing Phase 1 means the boundary is ELIGIBLE to be reconciled, nothing more** (Discover Pass 0 makes the same move catalog-wide). Phase 1 never declares a boundary reconciled.

**The four structural legs of every cross-domain handoff A→B** (and symmetrically B→A):
...
3. **Gate diff sections** (per direction) — every one is a GATE check, NEVER the verdict (the verdict is Phase 2):
   ...
   - **Section 5 — Cross-domain `data_object_relationships`.** These are the structural mirror of cross-domain handoffs: a handoff says *"event X fires from A to B"*; a relationship row says *"A's master has a verb to B's master"* (e.g. `job_offers spawns onboarding_journeys`). The relationship graph drives the mermaid renderer in blueprints and the navigation hints in the architect view — missing relationships leave both silently incomplete.
     ...
     - *MISSING-RELATIONSHIP / ORPHAN-RELATIONSHIP are Phase-1 gate items (structural), never the Phase-2 verdict. Do not confuse a Section-5 ORPHAN-RELATIONSHIP with a Phase-2 ORPHAN payload.*
4. **Produce the gate diff** (Phase 1 output). For the auto-discovery form (per-neighbor pass inside a single-domain Validate), per-neighbor findings append to the host domain's `audits/<DOMAIN>.md` under "Bucket 1 — Boundary findings per neighbor". For the manual bilateral form, draft in `.tmp_deploy/reconcile_<A>_<B>_<YYYY_MM_DD>.md`, then on user approval **append to BOTH `audits/<A>.md` AND `audits/<B>.md`** (each file gets the direction it owns: A→B in A's file, B→A in B's file) plus a shared summary count.

#### Phase 2 — Reconciliation (semantic; THIS is the reconciliation, and it alone decides pass/fail)

For **every** handoff on the boundary (both directions) that cleared Phase 1, classify its APQC payload against the realized set, where **realized = a process whose `process_id` is the gated `data_object_lifecycle_states.process_id` AND that carries `process_raci`**. Hierarchy is read from `process_code` (dotted prefix: `7.2.3` is the parent of `7.2.3.2`); category is the first code segment. This is the B9d resolver run per boundary; B9d is its per-domain entry point. Run as a TypeScript loader, never Python.

Resolver inputs: realized ids `/data_object_lifecycle_states?process_id=not.is.null&select=process_id` joined to `/processes` for `process_code`; `/process_raci?process_id=in.(<payloads + their hierarchy relatives>)&select=process_id,raci,actor_role_id`; each payload's `process_code` + `hierarchy_level`.

Classify each payload by its code C **in this order; first match wins** (the order is load-bearing: it is the parent-grain trap that has slipped through before):

1. **RESOLVED** — the **exact** code C is realized: C's own `process_id` is the gated `data_object_lifecycle_states.process_id` AND C carries `process_raci` with ≥1 R and ≥1 A. RACI on C **without C itself being the gated process** is NOT resolved: a parent code often carries roll-up RACI while the real gate sits on a child, and that is ROLL-UP, never RESOLVED.
2. **ROLL-UP** — C is not exactly realized, but a strict ancestor or descendant of C **is** the realized (gated + RACI'd) code. The work exists at a different grain. Fix: re-point `handoff_processes.process_id` to the realized code (`record_status='new'`). Reconcilable, not a coverage gap.
3. **MIS-TAG** — C is unrealized AND its first code segment is in **neither** endpoint's APQC category family (read each endpoint's family from its domain: HR → 7; sales/CRM → 3; product/engineering → 2; finance → 9; supply chain → 4; etc.; when unsure, treat as MIS-TAG and surface, do not pass it). A wrong tag, not missing work (the `10145`-subscription class). **Checked BEFORE ORPHAN — both are "unrealized", so if ORPHAN ran first it would swallow every mis-tag; the category test is the only thing separating them.** The sender owns it: on sign-off, re-point or delete the `handoff_processes` row.
4. **ORPHAN**: C is unrealized but its category DOES fit an endpoint: real missing work, a **process with no persona**. Resolve by **OWNER, on either side, recorded NOW**. The owner is the domain that **masters the carried entity** (its `master`/`embedded_master` row): primary, authoritative, unbuilt-safe; the realized-nearest-sibling check is corroborating only and is SKIPPED when the owner is unbuilt. One process can carry entities mastered by different domains, so the finding grain is per `(process, owner)`. If the owner is built, suggest candidate R/A by mirroring the owner's nearest already-realized process on the SAME carried entity. Write a durable **`b2`** item (id `B2-B9D-OWN-<process_id>`) into `audits/<OWNER>/state.yaml` AND the question into `audits/<OWNER>/q-<OWNER>.md` in THIS pass, whether the owner is the domain under audit or the neighbor (the same way entity routing writes into a neighbor's backlog, § state.yaml hygiene carve-out (b)). Model it as a `b2`, never a `b1b` (a `b1b`-backed q is illegal under the q-file content rule). Not latent, not derived-later. **This is executed by `scripts/analytics/b9d_resolver.ts`, not by hand.**

**Pass — the ONLY definition of reconciled, and RECONCILABLE is not RECONCILED.**

- A boundary with any open Phase-2 ROLL-UP, ORPHAN, or MIS-TAG is **RECONCILABLE, not RECONCILED**: it has been analyzed and fixes are proposed, but it is not yet clean.
- A boundary is **RECONCILED** iff **every** payload is RESOLVED, AND every ROLL-UP re-point has been **user-approved and applied** (`record_status` is no longer `new`), AND every ORPHAN and MIS-TAG is closed. Authoring `record_status='new'` re-point rows does NOT make the boundary reconciled: approval does (Rule #1).
- **Any Phase-2 ORPHAN or MIS-TAG means NOT reconciled, no matter how green Phase 1 is.** A report whose findings are all Phase-1 structural-gate items (Section 1-5), with Phase 2 unrun or empty, is an INCOMPLETE reconciliation: re-run Phase 2 before claiming the boundary reconciled.

**Light-neighbor rule (the ONLY allowed condensation).** A neighbor with edge weight <3 may get a condensed one-line-per-payload Phase-2 verdict table instead of the full write-up, but **Phase 2 still runs on every payload**. A light neighbor with any ORPHAN or MIS-TAG escalates to the full procedure. "Phase 1 only" is never an acceptable output for any neighbor that has ≥1 handoff payload.

**Output discipline.** The user approves loads one side at a time. Phase-2 fixes route by ownership: ROLL-UP re-points on the source; ORPHAN writes a durable `state.yaml` item + q into the OWNER's audit files on either side and realizes on the answer (never "owed by, someone else's problem"); MIS-TAG to the user on the source; structural gate items (Section 1-5) on the side that owes them. Each accepted fix loads as a normal Phase-B / Phase-M update on the owing side. Rule #1: every write lands `record_status='new'`; a `handoff_processes` deletion (mis-tag) needs explicit sign-off. **Never bulk-patch both sides of the TAG rows from a single invocation** (that erases the source-vs-target provenance the tag edits depend on); writing each owner's realization item into that owner's own audit files is the opposite, and is required.

**Anti-patterns.**

- ❌ **Calling a boundary "reconciled" because Phase 1 is green.** Phase 1 is the gate, not the verdict; this is the failure that has recurred five times. Reconciled requires the Phase-2 Pass test: zero ORPHAN/MIS-TAG and every ROLL-UP re-point approved.
- ❌ **Producing a reconciliation report with no Phase-2 section.** If you did not classify every payload, you validated the pair, you did not reconcile it.
- ❌ **Classifying a parent code as RESOLVED because it carries RACI.** If the gated process is a child (or any other code), it is ROLL-UP. The classification order exists precisely for this trap.
- ❌ **Declaring reconciled on `record_status='new'` re-points.** Authoring the fix is not approving it (Rule #1); the boundary is RECONCILABLE until the re-points are approved and the ORPHAN/MIS-TAG are closed.
- ❌ Running Phase 2 on an edge that failed Phase 1 (resolving payloads on broken plumbing).
...
### Blueprints (explicit step, not part of any sequence)

Blueprint emission is a deliberate, user-triggered action. **Do not run it as part of any load, audit, fix-loop, or "verify and share" step.** The audit reads live state via PostgREST; the on-disk blueprints can sit stale across many audit passes and that's the intended state.
...
6. **Surface descriptions for review.** Always show `data_object_name` + `singular_label` (+ `plural_label`) + `description` + `catalog_description` in a single table before loading. These prose columns are where AI research goes wrong silently; reviewers need to see them.
...
- **The overlay test:** *Does this domain persist any record that no other domain canonically masters?* If yes → it is **master-bearing**: author those masters in Phase B, give it modules and a domain skill like any other domain. If it persists nothing and recomputes its entire output at query time from other domains' masters → it is a **genuine derive/overlay domain**.
...
- **`opportunity` (rank 190):** 9 handoffs, 10 domains, 10 functions, 3 high-friction. The lead-to-cash motion — CRM → SALES-ENG → CPQ → CLM → FIN. Lower domain count than `employee` but comparable friction. Classic revenue process.
...
semantius call crud postgrestRequest '{"method":"GET","path":"/skills?select=skill_name,domain_id&order=domain_id.asc"}'  # who already has a domain skill
...
2. **The domain's `contributor` / `consumer` `data_objects`** — query tools for the cross-domain reads the workflow can't function without (e.g. PA `query_employees` reads HCM; EPM `query_journal_entries` reads FIN). Mark `required` only when the workflow demonstrably needs the read, not for every consumer relationship the catalog records.
3. **The domain's outbound `handoffs`** — when a handoff says "this domain triggers an event that creates a record in another domain", the domain skill needs the mutate tool on the receiving side. Canonical examples: CMDB → ITSM `create_incident` for `ci.unauthorized_change_detected`; SWP → ATS `create_candidate` for `headcount.approved`; AUDIT → GRC `close_follow_up_action` for finding-closure cascades. These are `required`.

### The 100%-Semantius hypothesis test

For each domain skill the killer-hypothesis rollup is: `% = (required tools with coverage_tier='platform') / (total required tools)`. **100% means every workflow primitive the skill needs is delivered by the platform OOTB — no external connector required.**
...
The reliable predictor of *not* being 100% Semantius: the domain's core workflow involves **(a)** generating narrative/explanation text the user reads, **(b)** distributing artefacts to recipients via external channels (email, SMS, mailers), or **(c)** running ML/statistical scoring beyond what computed_fields express. PA hits (a) + (b). When you encounter a domain-skill draft that looks like it needs LLM generation or external comms in the *required* set, that's the signal to expect a sub-100% result — and the signal that the domain genuinely benefits from non-Semantius tool delivery.

### Cross-tranche external-tool patterns

After running 22 domain-skill drafts across the catalog (12 hypothesis-candidates + 10 obvious-non-Semantius), the external tools that appear in REQUIRED sets cluster into recognisable shapes:
...
### When a domain has no domain skill

A domain has no domain skill ONLY when it is a genuine derive/overlay domain (per § "Master-bearing vs derive/overlay domains") that masters nothing AND has no actionable consumed/derived surface to operate on. This is rare. Most analytical-feeling domains are master-bearing and DO get a domain skill once their masters are loaded.

**There is no fixed "leadership-tier" list of skill-less domains.** The earlier version of this rule named 16 "verified zero-master" domains (REV-INTEL, SALES-PERF, GTM-PLAN, ACCT-PLAN, PRM, OP-RES, BCM, SECOPS, SOAR, THREAT-INTEL, TPRM, VULN-MGMT, PRIV-MGMT, FINOPS, INTRANET, COLLAB-GOV) and told you to NOT scaffold Phase-B for them. That was wrong: ~14 of those persist real records by their own descriptions and `crud_percentage` (e.g. partner deal registrations, vulnerabilities, DSARs, quotas, account plans, deal scores, forecasts) and are simply unbuilt. Do NOT treat a domain as skill-less because it appears on an old list. Apply the overlay test instead. EPM is the canonical illustration: it reads FIN journal entries and SWP cost projections but **masters** `financial_plans` / `budgets` / `forecasts` / `variance_analyses` / `financial_scenarios`, so it is master-bearing and gets a domain skill.

Quick check before drafting any domain skill: `semantius call crud postgrestRequest '{"method":"GET","path":"/domain_data_objects?domain_id=eq.<id>&role=eq.master"}'`. Zero masters means one of two things, and you must decide which via the overlay test: (a) the domain is a genuine overlay (masters nothing, recomputes at query time) - skip the domain skill but still author its `consumer`/`derived` footprint and module; or (b) the domain is master-bearing but **unbuilt** - that's a Phase-B gap; run Phase B and author the masters, then draft the skill. Don't paper over a Phase-B gap by inventing `domain_module_tools` that reference data_objects from other domains - that produces a derived toolset whose entire required set is consumer-reads, which is not a domain skill.

### Anti-patterns specific to domain-skill derivation
...
2. **Auto-derive query tools** — every master `data_object` across `query_domains` contributes its existing `query_<master>` tool as REQUIRED. The catalog already has these from the domain-skill derivation pass; no new query tools needed. Dedupe by `tool_name`.
...
5. **Apply the coverage rollup** ([references/semantius-coverage-rollup.md](references/semantius-coverage-rollup.md)) — process skills sit at 92-97% by design (every workflow needs at least `send_email`). 100% is rare and usually means the skill is mis-modeled as a process skill when it's actually a domain skill.
...
Blueprint emitter (explicit, user-triggered only): [scripts/generate_blueprints.ts](../../../scripts/generate_blueprints.ts). See § "Blueprints (explicit step, not part of any sequence)".
