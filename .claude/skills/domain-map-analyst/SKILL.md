...
**B15. No pattern flag on a `catalog` / `junction` / `computed` master.** (Rule #12; authoring-time gate for the emitter's M6 invariant.)
- Query: `/data_objects?id=in.(<masters>)&entity_type=in.(catalog,junction,computed)&or=(has_personal_content.eq.true,has_submit_lock.eq.true,has_single_approver.eq.true)&select=id,data_object_name,entity_type,has_personal_content,has_submit_lock,has_single_approver`
- Pass: empty result. The three pattern flags are an operational-write concern and are only meaningful on `operational_workflow` / `operational_record`. A flag set on a `catalog` / `junction` / `computed` master is a self-contradiction: the emitter suppresses the override and prints `⚠ M6` at emit time (`deriveWriteTier` gives these classes an admin / neighbor / read-only tier with no per-row override surface). This band is the authoring-time gate so the contradiction is caught by the audit checklist, not left to a console warning that a checklist-only review walks past. B12 / B13 do NOT cover it (B13 only flags `unclassified`; B12 only flags `operational_workflow` missing lifecycle; B4 only verifies a flag was *considered*, not that it *contradicts* the class).
- Fix: **clear the offending pattern flag(s); keep `entity_type` as-is.** The flag is ALWAYS the error here, never the class. The three flags are operational-write overrides with no meaning on `catalog` / `junction` / `computed` (the emitter already suppresses them via M6), so a flag on one of these classes is a stray setting to clear, not a signal to reclassify. **Having a lifecycle does NOT promote the entity to `operational_workflow`:** per Rule #12, `catalog` (config / definitions with publishing flows, e.g. `offer_letter_templates`) and `junction` (qualifier-carrying links, e.g. `talent_pool_memberships`) legitimately carry lifecycle states, and their approval gate is emitted from the lifecycle regardless of class. Who may approve is handled by the lifecycle gate + RACI; the entity stays admin-tier (`catalog`) / neighbor-tier (`junction`) / read-only (`computed`) for base writes, which is correct for configuration data. Clearing a non-empty flag is a DESTRUCTIVE overwrite (Rule #21 / Rule #1): surface the rows and apply only on sign-off. Never auto-PATCH, never stamp `approved`. (Reclassifying to `operational_workflow` would be wrong: it flips base writes `:admin → :manage`, handing edit rights on configuration data to managers, and it contradicts Rule #12.)
- Reference loader for the bulk fix: [.tmp_deploy/clear_b15_pattern_flags_2026_06_11.ts](../../../.tmp_deploy/clear_b15_pattern_flags_2026_06_11.ts) (reads the violating set, clears only the flags that are set, re-verifies the catalog-wide predicate returns zero). The 2026-06-11 catalog-wide scan found 14 such rows, all resolved by clearing the stray flag (NO reclassification) per the decision recorded in [references/skill-changelog.md](references/skill-changelog.md): `contract_clauses` + `contract_templates` (CLM-AUTHORING), `skill_taxonomies` + `competency_models` (SKILLS-MGMT-TAXONOMY), `certification_definitions` + `credential_badges` (LMS-CREDENTIALS), `offer_letter_templates` (ATS-OFFERS), `job_profiles` (HCM-ORG-POSITIONS) cleared `has_submit_lock` / `has_single_approver`; `recruiting_event_attendances` (ATS-CANDIDATE-CRM), `talent_pool_memberships` (ATS-TALENT-POOLS), `smp_app_owners` (SMP-DISCOVERY), `smp_license_seat_assignments` (SMP-OPTIMIZATION), `relationship_records` (INV-CRM-RELATIONSHIP-GRAPH), `work_user_workloads` (WORK-MGMT-TASK-EXEC) cleared `has_personal_content`. The B15 `or=(…)` across all three flags is what surfaces the full set; do not narrow it to one flag.
