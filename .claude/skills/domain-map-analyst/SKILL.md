...
4. Classify each result:
   ...
   - **In-scope fix** — this domain can fix locally (S1–S3 zero-row anomalies, A1–A4, M1–M8, B1–B9, B9b, **B9d**, B10b, B11–B15, C1–C2, D1, E1–E6, **B9e**, F1–F5, F7, F8, **H1**). Goes into the **gap report** as actionable. Blueprint emission is **not** an audit step — see § "Blueprints" below. (B15, the pattern-flag-on-a-catalog/junction/computed-master check, is also a pure catalog-wide predicate: one query finds every violation across all domains at once, independent of which domain is being audited, so it can be swept globally rather than waiting for each domain's turn.)
