...
| `domain_module_tools` | N:M between `domain_modules` and `tools` - which atomic tools a module requires | `requirement_level` (`required` / `optional`). `notes` off-limits without user-approved wording per Rule #15. Computed unique key `<domain_module_id>.<tool_id>`. A domain's `domain` skill derives its toolset by rolling these up over the domain's primary + host modules |
...
- Query: `/domain_modules?domain_id=eq.<id>&select=id,domain_module_code,domain_module_name` UNION the cross-cutting modules hosted here via `/domain_module_host_domains?domain_id=eq.<id>&select=domain_module:domain_modules(id,domain_module_code,domain_module_name)` (same module set as M1 / M8).
