...
5. **Load via the script idiom — in multiple phases, each surfaced separately.** Even for ~20 rows, prefer extending [scripts/loaders/load_research.ts](scripts/loaders/load_research.ts) over one-off CLI calls. The script is idempotent (safe to re-run), chunked (no Windows command-line issues), and produces a row-count summary you can paste back to the user. Dated drafts in `.tmp_deploy/` are gitignored, so iteration is free; reusable patterns graduate to `scripts/loaders/`.
   ...
   **Phase S - Domain skill + tools** (load fourth, against the same domain). Under Rule #17 every domain needs exactly one domain-grain `skill_type='domain'` skill that **derives** its toolset from the domain's modules' `domain_module_tools`. Skills are domain-grain only: a single-domain starter is served by the host domain's skill, a cross-domain starter by its bundle-domain skill; a FULL module gets no skill but carries its tool requirements as `domain_module_tools`. Phase S delivers:
   ...
   1. **`skills`** - one row per **domain** with `skill_type='domain'`, `domain_id` set, and `skill_name='<domain_code_lower>'` (e.g. `crm`, `ats`; no suffix). Author its `description` (the routing prose the SKILL.md renders) and `trigger_keywords` (comma-separated trigger terms, the complement of the prose). Starters and FULL modules get no skill row of their own. The skill encodes what an agent can do across the domain's data and lifecycle; it derives its toolset and stores no tools.
