...
- Fix: decided by whether the entity has (or warrants) a gated lifecycle state machine (the Rule #12 `operational_workflow` ⟺ gated-state-machine boundary):
  - **(a) has a real `draft → … → terminal` gated lifecycle** → it was mis-classified; the correct value is `operational_workflow` and the flag is valid there. This is the near-universal case ("template / definition with an authoring-and-approval lifecycle" filed as `catalog`). Re-class `entity_type` to `operational_workflow`.
  - **(b) genuinely reference / config data, no gated lifecycle** → the flag is the error; clear the flag and keep `catalog`.
  Either fix is a DESTRUCTIVE overwrite of a non-empty value (Rule #21 / Rule #1): surface each row to the user with the proposed value AND the write-tier consequence (`catalog → operational_workflow` flips the tier `:admin → :manage`; governance is then carried by the lifecycle's approval gate, e.g. `approve_x`, not by the catalog admin tier), and apply only on sign-off. Never auto-PATCH, never stamp `approved`.
- Catalog-wide scan on 2026-06-11 found 14 such rows across 10 modules:
  - **Case (a), 8 `catalog` masters** that each carry a 3–6 state `draft → … → terminal` lifecycle, so all resolve to `operational_workflow`: `contract_clauses` + `contract_templates` (CLM-AUTHORING), `skill_taxonomies` + `competency_models` (SKILLS-MGMT-TAXONOMY), `certification_definitions` + `credential_badges` (LMS-CREDENTIALS), `offer_letter_templates` (ATS-OFFERS), `job_profiles` (HCM-ORG-POSITIONS).
  - **Case (a), 4 `junction` rows** that each carry a 2–4 state lifecycle, so they are associative entities mis-filed as pure links, not junctions; re-class away from `junction` (workflow vs record per the entity's state machine): `recruiting_event_attendances` (ATS-CANDIDATE-CRM), `talent_pool_memberships` (ATS-TALENT-POOLS), `smp_app_owners` (SMP-DISCOVERY), `smp_license_seat_assignments` (SMP-OPTIMIZATION).
  - **Case (b), 2 `computed` masters** with no lifecycle where `has_personal_content` is meaningless (a computed entity has no writes to row-scope); clear the flag: `relationship_records` (INV-CRM-RELATIONSHIP-GRAPH), `work_user_workloads` (WORK-MGMT-TASK-EXEC).
  - Note: the earlier figure of "7" undercounted: it filtered on `has_submit_lock` only and missed `job_profiles` (a `catalog` row flagged via `has_single_approver`) plus all 6 `junction` / `computed` rows flagged via `has_personal_content`. The B15 `or=(…)` across all three flags is what surfaces the full set; do not narrow it to one flag.
