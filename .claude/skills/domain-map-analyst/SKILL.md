...
- **Corrective-additive work the agent executes without asking** — and ONLY *corrective* gap-fill on the domain's ALREADY-SCOPED footprint: fill an empty field, classify `entity_type`, author `record_status='new'` content (catalog text per Rule #20, aliases, lifecycle states, trigger_events, personas + RACI, APQC tags) on entities/modules that already exist, backfill module FKs. Everything lands at `record_status='new'`; the user reviews the result in the records, never via a chat gate. Rule #1 still holds: never stamp `approved`.
...
**What goes in the `q-` file.** Only items that need the user: every `b2` decision and every pending destructive approval (Rule #21). Open `b3` ideas go last under an "Optional (will not hold up the build)" heading. Nothing else (no `b1a`, no `b1b`). **`record_status` approval is NOT a q-file item.** Approving / rejecting records (setting any non-`new` `record_status`) is out of band: it never rides the `q-`/`a-` mechanism and requires the separate explicit, direct user request of Rule #1. Do not list `record_status` flips as q-file questions, and never treat an a-file answer (including a "go" rename) as authorization for one.
