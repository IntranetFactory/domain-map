...
### 2026-06-28 — `domain_module_host_domains` table dropped; host set derived from embeds; starters leave the module grid

**Context.** A user asked why "Real Estate Agent (solo / small firm bundle)" rendered as a module card under the CLM domain (and in `domain-map.json` under the CLM entry). Diagnosis: the `REAL-ESTATE-AGENT` starter declared CLM in `domain_module_host_domains`, and every emitter folded host-junction rows into a domain's module grid. The table was already DEPRECATED (2026-06-16, `references/deprecations.md`) but its readers were never rewired and the SKILL.md body still instructed authoring it. Live audit confirmed the table carried no non-derivable information: starter host rows equal the owner-domains of the starter's `embedded_master` entities (verified exact for REAL-ESTATE-AGENT and IT-OPS-STARTER), the 4 PAYROLL rows were self-host no-ops (host = own `domain_id`), and HVAC-SVC-MGMT carried an orphan FIN row backed by no entity.

**Decision (executed).** (1) Dropped the `domain_module_host_domains` table and all rows. (2) A module's host set is now DERIVED by `deriveHostDomains(dmdo, modules)` in `scripts/lib/catalog.ts`: for a STARTER, the domains that master its `embedded_master` entities, minus its own primary domain; FULL modules host nowhere. (3) `emit_domain_map.ts` now builds a domain's module grid from PRIMARY modules only, so a starter renders on its own bundle-domain page, not as a card on every market it embeds from. Cross-market relatedness survives via the existing master/touch `related_domains` rule (the bundle touches `legal_contracts`, CLM masters it → related). (4) The Semantius coverage score is now **primary-modules-only**: a starter that merely embeds one of a domain's entities is a CONSUMER of that market (Rule #19: starters never master, only embed), not a contributor, so its tools are no longer folded into the market's score. Dropped the host union from `coverage_rollup.ts` and `m11_rollup_probe.ts`, the F3/F5 queries, the §"Semantius score" definition, and `references/semantius-coverage-rollup.md` (prose + SQL). Effect: e.g. CLM's rollup denominator went 53→30 and the real-estate tools the REAL-ESTATE-AGENT starter dragged in (`syndicate_to_mls`, `query_real_estate_*`, …) are gone; the score is a stable property of the market. Analytics-only: emitted artifacts are unchanged (per-module blueprint coverage never used the host union; domain skill specs were already `module_kind='full'`-only). `deriveHostDomains` is retained for release gating (generate_blueprints/build_catalog) and the bundle's "markets it composes" display (emit_skill_spec) — those are about touched markets, not the score.

**Reasoning.** User direction, grounded in the live audit above. The table overloaded one column with two meanings (full-module "is also deployable here" vs starter "embeds from here"); only the starter meaning was ever used, and it is fully derivable from `embedded_master`. Cross-cutting FULL concepts are modeled as separate per-domain modules sharing a data_object (Knowledge = `ITSM-KNOWLEDGE` + `HRSD-KNOWLEDGE` + `CSM-KNOWLEDGE`), so no full module ever legitimately needed host rows.

**Removed/rewired surface.** Live: table `domain_module_host_domains` (entity + rows). Code: the `/domain_module_host_domains` fetch in `lib/catalog.ts` (now derives), the host-fold in `emit_domain_map.ts` (now primary-only grid), the independent fetches in `coverage_rollup.ts` / `m11_rollup_probe.ts` / `emit_skill_spec.ts` / `skill_grain_00_preflight.ts` (now `deriveHostDomains`) and the dead fetch in `skill_grain_04_per_domain_skills.ts`; `generate_blueprints.ts` / `build_catalog.ts` consume the derived `all.hostDomains` unchanged. Skill: SKILL.md Rule #14 cross-cutting + starter prose, Rule #19 persona-bundle prose, the at-a-glance entity count (33→32, 13→12 junctions), the `domain_modules` / `domain_module_data_objects` table rows, the §"Semantius score" tool-set definition, the Phase-A `domain_modules` step, the `<masters>` derivation, and the M1/M2/M8/M10/S1/S2/F3 audit queries (structural bands → primary modules; coverage bands → derived host); `references/modules.md` §3 + naming + anti-pattern, `references/module-shape.md` `domain_id` row + junction section (→ REMOVED), `references/semantius-coverage-rollup.md` prose + SQL (derive host in-SQL), `references/domain-audit-procedure.md` pull queries, `references/deprecations.md` (→ removal DONE).

**Scope.** Catalog-wide. Generated `domain-map.json` re-emitted (CLM grid now 5 real modules; REAL-ESTATE-AGENT renders on its own bundle domain; `related_domains` still links them). Blueprints: 0/56 drift. Generated `spec.json` / blueprints otherwise stale until re-emitted per domain.
