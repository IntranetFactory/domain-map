...
| `description` | multiline | yes | Analyst-facing. The richer prose that feeds the agent-facing domain skill spec (`emit_skill_spec.ts`) and the B13 entity_type keyword heuristic. Not the public surface. |
| `catalog_description` | text | no | Reader-facing short summary for the §2 Entity summary; the emitter renders this and falls back to `description` when empty. **Plural voice matching the plural Name, no leading article** ("Contracts with counterparties...", not "A contract..."), 1 to 2 sentences, jargon / field-lists / cross-domain asides stripped. Keep it short, especially when `data_object_name` is already self-describing. Empty by default; fill freely, never overwrite a non-empty value without explicit user approval (Rule #20 shape). |
...
| `has_personal_content` | boolean | yes | Default `false`. A **hint** that the data_object holds owner-scoped rows where only the owner / specific actors see specific rows (e.g. interview scorecards, performance reviews, employee notes), so a row-scoped ABAC rule (JsonLogic `select_rule` / `validation_rules`) should be authored for it. The flag itself enforces nothing; the emitter mints `view_all_*` / `manage_all_*` override permissions + an edit-scope rule as scaffolding, but the actual predicate (which column identifies the owner) must be added as a JsonLogic rule. The former `has_submit_lock` / `has_single_approver` flags were dropped 2026-06-26 (workflow-state concerns belong in the lifecycle / RACI layer, not on the entity). |
| `record_status` | enum | yes | Default `new` |
