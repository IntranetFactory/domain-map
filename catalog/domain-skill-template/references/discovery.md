...
Reconciles the HQ-emitted facts file against what's actually deployed in this tenant. Runs in two modes:

- **Full discovery** (cold start, or `state.discovered_against_major < facts_major`): every entity in the facts file is verified against the live deployment.
- **Incremental reconciliation** (`state.discovered_at < facts.emitted`, same major): diff facts against existing state; only verify what changed.
...
For each module listed in `facts.modules`:
...
For each `data_objects` entry in the facts file whose owning module is `present: true`, search the tenant's `entities` table by candidate names. The search order matters: catalog name first, then known aliases, then a fuzzy fallback if neither matches.
...
# Step 2b: if zero rows, try each alias from the facts file
...
3. **Fuzzy match** (2c hit): **ASK THE USER.** Surface the candidate match with the catalog description side-by-side: *"The facts file lists `suppliers` for {{DOMAIN_NAME}}. I found an entity called `vendors` with description matching closely. Treat them as the same?"* On yes, record as a rename. On no, treat as omitted plus a custom entity (Pass 4).
...
Compare against `facts.data_objects.<name>.lifecycle_states` (which implies a `status` or `state` field exists). The skill cares about field-level shape mainly for:

- Lifecycle fields the facts file implies (`status`, `state`, or whatever the tenant named it). If absent, record in `state.field_omissions` and note the lifecycle states can't be tracked.
...
Record in `state.custom_entities`. Ask the user for each: *"I found a custom entity `referral_bonuses` in your {{DOMAIN_NAME}} module that the catalog doesn't know about. Should I include it when reasoning about this domain? You can describe its role (master, log, reference data)."* Record the answer in `state.custom_entities.<name>`.

---

## Pass 5: relationship and handoff sanity

The facts file lists intra-domain relationships and outbound handoffs. The skill does not verify each one structurally during discovery; that's the catalog's job at emit time. The skill only flags relationships whose endpoints are missing from this tenant:
...
## Pass 6: write `state.yaml` and report

Write the state file with the structure shown in the parent [SKILL.md](../SKILL.md#whats-in-stateyaml). Then surface a one-screen summary to the user:

```
{{DOMAIN_NAME}} discovery complete.
  Modules: 3 of 4 deployed (ATS-INTERVIEW-MGMT not deployed)
...
When `state.discovered_against_major == facts_major` but `state.discovered_at < facts.emitted`:
...
3. Update `state.discovered_at` to the current date and `state.discovered_against_emitted` to `facts.emitted`.
