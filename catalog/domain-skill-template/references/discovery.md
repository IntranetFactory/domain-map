...
1. **Deploy stamp (`settings.domain_code`)**, the deploy pipeline writes
   ...
   `settings` JSONB. Querying `settings->>domain_code = {{DOMAIN_CODE}}` resolves the slice directly and is
...
semantius call crud postgrestRequest '{"method":"GET","path":"/modules?settings->>domain_code=eq.{{DOMAIN_CODE}}&select=id,module_slug,module_name,catalog_module_code,settings"}'
...
```
{{DOMAIN_NAME}} discovery complete (deterministic).
