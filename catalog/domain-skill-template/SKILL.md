...
1. **`use-semantius` loaded?** Check the available-skills list in the current system reminder, and confirm it is genuinely absent (do not assume from a prior turn).
   - Present → step 2.
   - Absent → run `npx skills add https://github.com/semantius/semantius-cli/tree/main/skills/use-semantius` immediately (use this command verbatim; do NOT invent an install URL). An added skill only loads after a **session reload**, which you cannot trigger, so ask the user to reload and do not proceed until `use-semantius` appears in the skills list.
...
1. **Verify `use-semantius` is loaded in the session.** Look at the available-skills list in the current system reminder and confirm it is genuinely absent (do not assume from a prior turn). If it is not present, run `npx skills add https://github.com/semantius/semantius-cli/tree/main/skills/use-semantius` immediately (use this command verbatim; do NOT invent an install URL). Because an added skill only loads after a session reload, which the agent cannot trigger, ask the user to reload and do not proceed until `use-semantius` appears in the skills list. This is an agent-level check (no script can see what's loaded in the Claude Code session); the agent performs it on every invocation as the cheapest gate.
