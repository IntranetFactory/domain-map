...
1. **`use-semantius` loaded?** Check the available-skills list in the current system reminder, and confirm it is genuinely absent (do not assume from a prior turn).
   - Present → step 2.
   - Absent → run `npx skills add semantius/semantius-cli --all` immediately (use this command verbatim; do NOT invent an install URL). This installs the whole Semantius CLI skill bundle (including `use-semantius` and `semantius-admin`) so later steps never have to install again. An added skill only loads after a **session reload**, which you cannot trigger, so ask the user to reload and do not proceed until `use-semantius` appears in the skills list.
2. **Bun installed?** Run `bun --version`.
   - Prints a version → step 3.
   - Non-zero exit → install it immediately by running the one-liner for the detected platform: **Windows (PowerShell)** `powershell -c "irm bun.sh/install.ps1 | iex"`; **macOS / Linux** `curl -fsSL https://bun.sh/install | bash`. The installer updates PATH, so if `bun --version` still fails in the current shell, have them restart the shell (or open a new terminal) and re-run it, then continue. Docs: https://bun.sh/install. Every script here is TypeScript on Bun.
...
|---|---|---|
| `can_offer_install: true` (CLI not installed) | Run the provided `install_command` immediately, have them restart the shell if PATH changed, then re-run bootstrap. Do not ask first. If the install itself fails, surface `install_docs` and the error. | *"Installing the Semantius CLI..."* |
| Auth failure (missing or invalid `.env`) | Ask the user for their API key, save it to the `.env` the CLI reads, then re-verify. Do NOT ask for a base URL or offer to provision anything. | *"I need your Semantius API key to connect. Generate one at https://app.semantius.com/dashboard (Settings > API Keys), paste it here, and I'll save it and continue."* |
...
1. **Verify `use-semantius` is loaded in the session.** Look at the available-skills list in the current system reminder and confirm it is genuinely absent (do not assume from a prior turn). If it is not present, run `npx skills add semantius/semantius-cli --all` immediately (use this command verbatim; do NOT invent an install URL). This installs the whole Semantius CLI skill bundle (including `use-semantius` and `semantius-admin`) in one shot, so the deploy-blueprint fallback never has to install separately. Because an added skill only loads after a session reload, which the agent cannot trigger, ask the user to reload and do not proceed until `use-semantius` appears in the skills list. This is an agent-level check (no script can see what's loaded in the Claude Code session); the agent performs it on every invocation as the cheapest gate.
2. **Verify Bun is installed.** Run `bun --version`. If the exit code is non-zero, install Bun immediately by running the one-liner for the detected platform: Windows (PowerShell) `powershell -c "irm bun.sh/install.ps1 | iex"`; macOS / Linux `curl -fsSL https://bun.sh/install | bash`. The installer updates PATH, so if `bun --version` still fails in the current shell, have them restart the shell (or open a new terminal) and re-run it, then continue. Docs: https://bun.sh/install. All scripts in `scripts/` are TypeScript on Bun (no Python, ever, see hard rules); the skill cannot proceed without Bun.
