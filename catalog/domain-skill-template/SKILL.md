...
This skill knows the **{{DOMAIN_NAME}}** domain as shipped from the catalog at HQ and as discovered in this deployment. It avoids re-discovering the domain shape on every conversation by persisting deployment-specific findings to local state.
...
- Entities: {{ENTITY_NOUNS_COMMA_LIST}}
...
UI base: `https://<org>.semantius.app/{{MODULE_SLUG}}/<table>`
