...
name: use-hrsd
description: >-
  Let employees raise HR questions and requests through a self-service portal, find answers in a knowledge base, and follow each case from intake to resolution. Triage and route incoming cases to the right specialist, escalate across payroll, benefits, and access teams, apply approvals and assignments, track against service levels, and handle reopens. Publish and maintain knowledge articles drawn from resolved cases to deflect repeat requests. Use for: HRSD, HR help desk, employee relations, tier 1 HR, tier 2 HR, case deflection, ticketing, SLA, queue, classification, agent assist, policy attestation, onboarding, FAQ, employee experience, shared services, HR portal, people operations.
---

# use-hrsd skill

This skill knows the **HR Service Delivery** domain as shipped from the DomainMap catalog and as discovered in this deployment. It avoids re-discovering the domain shape on every conversation by persisting deployment-specific findings to local state.
...
- Workflows / capabilities: AI Triage and Classification, Approval Workflow Orchestration, Knowledge Management, Self-Service Portal
...
3. **Alias**, an entity whose `catalog_entity_aliases` contains `{ alias_code: X, source_domain: HRSD }` (JSONB containment; resolve on the **pair**, never `alias_code` alone). Catches a reuse/merge that renamed `X` onto a differently-named host.
