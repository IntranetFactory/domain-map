...
name: use-ats
description: >-
  Open and route job requisitions for approval, post openings to job boards, and capture applications. Source candidates, build talent pools, and run outreach and referral campaigns. Move applicants through pipeline stages, schedule interviews, collect scorecards, and run assessments. Generate and version offer letters, route them for approval, and send them for e-signature. Order background and drug screenings with FCRA disclosures and adverse-action notices, then hand accepted hires to onboarding. Track EEO and OFCCP funnel analytics and disposition reasons. Use for: ATS, applicant tracking system, recruiting, recruitment, hiring, talent acquisition, sourcing, headhunting, resume, CV, careers page, candidate experience, nurture, recruiter CRM, interview kit, hiring panel, hiring manager, time-to-fill, offer management, salary, comp, drug test, EEOC, diversity, compliance, new hire, pre-employee, requisition approval.
---

# use-ats skill

This skill knows the **Applicant Tracking and Recruiting** domain as shipped from the DomainMap catalog and as discovered in this deployment. It avoids re-discovering the domain shape on every conversation by persisting deployment-specific findings to local state.
...
3. **Alias**, an entity whose `catalog_entity_aliases` contains `{ alias_code: X, source_domain: ATS }` (JSONB containment; resolve on the **pair**, never `alias_code` alone). Catches a reuse/merge that renamed `X` onto a differently-named host.
