...
name: use-smp
description: >-
  Inventory every SaaS application the company pays for, including unsanctioned tools employees signed up for on their own, and tie each one to an owner, a contract, and real login activity. Track subscriptions and seat assignments against actual usage, flag underused or duplicate licenses, and recommend or trigger reclamation to recover wasted spend. Run renewals on a schedule with reminders and negotiation prep, assess vendor risk, allocate cost to departments, and let employees request access through a self-service app catalog. Automate joiner and leaver provisioning and route approval workflows across the portfolio. Use for: SMP, SaaS management platform, shadow IT, SaaS spend management, software spend optimization, idle seats, renewal calendar, app rationalization, deprovisioning, offboarding, usage telemetry, sanctioned apps, SaaS sprawl, seat right-sizing, software asset visibility.
---

# use-smp skill

This skill knows the **SaaS Management Platforms** domain as shipped from the DomainMap catalog and as discovered in this deployment. It avoids re-discovering the domain shape on every conversation by persisting deployment-specific findings to local state.
...
- Workflows / capabilities: SaaS App Lifecycle Automation, SaaS Application Discovery, SaaS License Optimization, SaaS Renewal Management, Shadow IT Detection, SaaS Usage Analytics, SaaS Vendor Relationship Management
...
3. **Alias**, an entity whose `catalog_entity_aliases` contains `{ alias_code: X, source_domain: SMP }` (JSONB containment; resolve on the **pair**, never `alias_code` alone). Catches a reuse/merge that renamed `X` onto a differently-named host.
