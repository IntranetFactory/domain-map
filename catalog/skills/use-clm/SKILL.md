...
name: use-clm
description: >-
  Draft new contracts from a library of approved clauses and templates, route each agreement through legal and business review, and capture redlines as counterparties negotiate. Send for electronic signature, store the executed document in a searchable repository, then extract the obligations and milestones each contract creates and alert ahead of upcoming due dates and renewals. Use for: CLM, contract lifecycle management, e-sign, NDA, MSA, SOW, statement of work, master service agreement, non-disclosure agreement, evergreen, auto-renewal, expiration, breach, waiver, amendment, addendum, contract analytics, legal ops, deal desk, executed contract.
---

# use-clm skill

This skill knows the **Contract Lifecycle Management** domain as shipped from the DomainMap catalog and as discovered in this deployment. It avoids re-discovering the domain shape on every conversation by persisting deployment-specific findings to local state.
...
- Workflows / capabilities: Approval Workflow Orchestration, AI Clause-Risk Detection, Clause Library Management, Contract Authoring, Contract Repository, Obligation Extraction, Redlining and Negotiation, Renewal Management
...
3. **Alias**, an entity whose `catalog_entity_aliases` contains `{ alias_code: X, source_domain: CLM }` (JSONB containment; resolve on the **pair**, never `alias_code` alone). Catches a reuse/merge that renamed `X` onto a differently-named host.
