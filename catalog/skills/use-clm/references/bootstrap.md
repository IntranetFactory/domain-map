...
> The `Contract Lifecycle Management` domain is not deployed in your platform. No live module hosts its entities, and no module carries its catalog codes. Deploy the domain blueprint first:
>
> 1. Review the blueprint for this domain at `https://www.semantius.com/blueprints/clm`. This page describes the blueprint and how to download it with the `semantius-admin` skill.
...
> 3. Verify with: `semantius call crud postgrestRequest '{"method":"GET","path":"/modules?settings->>domain_code=eq.CLM&select=id,slug,name"}'` (any row returned means a module of this domain is already deployed)
>
