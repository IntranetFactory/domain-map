...
name: use-svcs-proc
description: >-
  Procure outsourced services under a statement of work end to end. Author the SOW that scopes what a firm will deliver, take in services proposals and bids, and stand up the engagement against the supplier and the contract. Amend scope through change orders, then break the work into milestones and deliverables and track each to formal acceptance. Settle on delivered outcomes with milestone-based invoices, and keep a record of who accepted what and when. The services firm stays the supplier and the SOW stays the contract; this runs the engagement in between. Use for: SOW management, services procurement, services engagement, milestone invoice, milestone-based billing, outcome-based services, services bid, service acceptance, professional services procurement.
---

# use-svcs-proc skill

This skill knows the **Services Procurement** domain as shipped from the DomainMap catalog and as discovered in this deployment. It avoids re-discovering the domain shape on every conversation by persisting deployment-specific findings to local state.
...
- Workflows / capabilities: Milestone and deliverable settlement, Services sourcing and bidding, SOW authoring and engagement execution, Services supplier performance
...
1. **`use-semantius` loaded?** Check the available-skills list in the current system reminder, and confirm it is genuinely absent (do not assume from a prior turn).
   - Present → step 2.
   - Absent → (a) say verbatim: *"This skill delegates all Semantius CLI mechanics to `use-semantius`. Install it and reload the session: `npx skills add https://github.com/semantius/semantius-cli/tree/main/skills/use-semantius`"* (do NOT invent an install URL); then (b) **offer to run that command for them** (*"Want me to install it now?"*), and run it on yes. Never install silently. An added skill only loads after a **session reload**, which you cannot trigger, so ask them to reload and do not proceed until `use-semantius` appears in the skills list.
2. **Bun installed?** Run `bun --version`.
   - Prints a version → step 3.
   - Non-zero exit → offer to install it (ask first; never install silently), then run the one-liner for their platform: **Windows (PowerShell)** `powershell -c "irm bun.sh/install.ps1 | iex"`; **macOS / Linux** `curl -fsSL https://bun.sh/install | bash`. The installer updates PATH, so if `bun --version` still fails in the current shell, have them restart the shell (or open a new terminal) and re-run it, then continue. Docs: https://bun.sh/install. Every script here is TypeScript on Bun.
...
| Domain not deployed (empty slice) | Stop; offer the blueprint deploy. | *"The Services Procurement domain isn't deployed on your platform yet. It can be added from the catalog. Want me to walk you through deploying it?"* |
...
1. **Verify `use-semantius` is loaded in the session.** Look at the available-skills list in the current system reminder and confirm it is genuinely absent (do not assume from a prior turn). If it is not present: first say this exact message (use the command below verbatim; do NOT invent an install URL): *"This skill delegates all Semantius CLI mechanics to `use-semantius`. Install it and reload the session: `npx skills add https://github.com/semantius/semantius-cli/tree/main/skills/use-semantius`"*; then **offer to run that command for the user** (ask first, it modifies their setup; never install silently). Because an added skill only loads after a session reload, which the agent cannot trigger, ask the user to reload and do not proceed until `use-semantius` appears in the skills list. This is an agent-level check (no script can see what's loaded in the Claude Code session); the agent performs it on every invocation as the cheapest gate.
2. **Verify Bun is installed.** Run `bun --version`. If the exit code is non-zero, OFFER to install Bun for the user (ask first, it modifies their system; never install silently), then run the one-liner for their platform: Windows (PowerShell) `powershell -c "irm bun.sh/install.ps1 | iex"`; macOS / Linux `curl -fsSL https://bun.sh/install | bash`. The installer updates PATH, so if `bun --version` still fails in the current shell, have them restart the shell (or open a new terminal) and re-run it, then continue. Docs: https://bun.sh/install. All scripts in `scripts/` are TypeScript on Bun (no Python, ever, see hard rules); the skill cannot proceed without Bun.
...
3. **Alias**, an entity whose `catalog_entity_aliases` contains `{ alias_code: X, source_domain: SVCS-PROC }` (JSONB containment; resolve on the **pair**, never `alias_code` alone). Catches a reuse/merge that renamed `X` onto a differently-named host.
