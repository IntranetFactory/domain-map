---
name: use-intranet-gov
description: >-
  Inventory the pages, hubs, libraries, and forms across the intranets an organization already runs, assign accountable owners to each one, and run recertification attestations so content is confirmed current or flagged stale. Score content health, check accessibility, manage review cadences, and turn findings into a prioritized improvement backlog. Govern the existing intranet platforms in place rather than replacing them. Use for: digital workplace, internal communications, content governance, content audit, governance, compliance, abandoned, orphaned, link rot, broken links, WCAG, ADA, Section 508, sunset, sprawl, taxonomy, findability, knowledge base hygiene, employee portal.
---

# use-intranet-gov skill

This skill knows the **Intranet Governance** domain as shipped from the DomainMap catalog and as discovered in this deployment. It avoids re-discovering the domain shape on every conversation by persisting deployment-specific findings to local state.
...
- Workflows / capabilities: Accessibility Governance, Ownership Attestation and Recertification, Source Connectors, Content Inventory and Mapping, Content Lifecycle and Review, Ownership Accountability, Improvement Planning and Backlog, Content Quality Scanning
...
3. **Alias**, an entity whose `catalog_entity_aliases` contains `{ alias_code: X, source_domain: INTRANET-GOV }` (JSONB containment; resolve on the **pair**, never `alias_code` alone). Catches a reuse/merge that renamed `X` onto a differently-named host.
