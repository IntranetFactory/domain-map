...
name: use-itsm
description: >-
  Log and resolve IT incidents and outages, fulfill service requests from a catalog, run problem investigations with root-cause analysis, and move changes and releases through approvals, CAB review, and collision checks. Prioritize and assign work by SLA timers and agent skills, deflect routine questions through a self-service portal and knowledge base, handle inbound events and alerts, and report on resolution times and service quality. Use for: help desk, helpdesk, ITIL, ticketing, ticket queue, password reset, MTTR, change advisory board, escalation, RCA, runbook, major incident, on-call.
---

# use-itsm skill

This skill knows the **IT Service Management** domain as shipped from the DomainMap catalog and as discovered in this deployment. It avoids re-discovering the domain shape on every conversation by persisting deployment-specific findings to local state.
...
3. **Alias**, an entity whose `catalog_entity_aliases` contains `{ alias_code: X, source_domain: ITSM }` (JSONB containment; resolve on the **pair**, never `alias_code` alone). Catches a reuse/merge that renamed `X` onto a differently-named host.
