...
>
> 1. Review the blueprint for this domain at `https://www.semantius.com/blueprints/itsm`. This page describes the blueprint and how to download it with the `semantius-admin` skill.
...
> 3. Verify with: `semantius call crud postgrestRequest '{"method":"GET","path":"/modules?settings->>domain_code=eq.ITSM&select=id,slug,name"}'` (any row returned means a module of this domain is already deployed)
>
