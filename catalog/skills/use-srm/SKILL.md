...
name: use-srm
description: >-
  Onboard and qualify suppliers, maintain the supplier master record and its documents, score supplier performance against delivery and quality targets, and assess supplier risk and compliance including sanctions screening, certifications, and ESG. Open corrective actions and re-qualify suppliers when risk or performance slips. Use for: supplier management, supplier relationship management, vendor management, supplier risk management, supplier onboarding, supplier qualification, supplier scorecards, supplier compliance, KYC, supplier diversity, supplier information management, SIM.
---

# use-srm skill

This skill knows the **Supplier Relationship Management** domain as shipped from the DomainMap catalog and as discovered in this deployment. It avoids re-discovering the domain shape on every conversation by persisting deployment-specific findings to local state.
...
- Workflows / capabilities: Supplier Compliance Screening, Supplier Corrective Action Management, Supplier Onboarding, Supplier Performance Management, Supplier Qualification, Supplier Risk Assessment, Supplier Segmentation, Supplier Information Management
...
| Domain not deployed (empty slice) | Stop; offer the blueprint deploy. | *"The Supplier Relationship Management domain isn't deployed on your platform yet. It can be added from the catalog. Want me to walk you through deploying it?"* |
...
3. **Alias**, an entity whose `catalog_entity_aliases` contains `{ alias_code: X, source_domain: SRM }` (JSONB containment; resolve on the **pair**, never `alias_code` alone). Catches a reuse/merge that renamed `X` onto a differently-named host.
