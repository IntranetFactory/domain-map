---
name: use-it-ops-starter
description: >-
  Manage IT for a small team from one place: log and resolve helpdesk tickets and service requests, track hardware and who has each device, inventory SaaS apps and their subscriptions, watch upcoming renewal dates, keep the contracts and on-prem licenses behind them, and alert owners before anything lapses. Look up a device, an app, a subscription, or a contract, record a new ticket or request, and flag renewals that are coming due. Use for: IT asset management, ITAM, ITSM, help desk, ticketing, incident, MSP, CMDB, asset register, software asset management, SAM, license compliance, vendor agreement, autorenewal, SaaS spend, shadow IT, seat count, end of life, warranty, procurement, sysadmin, IT operations.
---

# use-it-ops-starter skill

This skill knows the **IT Operations Starter** domain as shipped from the DomainMap catalog and as discovered in this deployment. It avoids re-discovering the domain shape on every conversation by persisting deployment-specific findings to local state.
...
3. **Alias**, an entity whose `catalog_entity_aliases` contains `{ alias_code: X, source_domain: IT-OPS-STARTER }` (JSONB containment; resolve on the **pair**, never `alias_code` alone). Catches a reuse/merge that renamed `X` onto a differently-named host.
