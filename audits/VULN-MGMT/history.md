...
- **Current footprint:** **0 modules** (no `domain_modules`, no `domain_module_host_domains`). **0 capabilities** (no `capability_domains`). **2 non-master domain_data_objects** unchanged from prior audit: `software_installations` (id 59, role `consumer`, necessity `required`, canonical master SAM-INST-MGMT) and `org_units` (id 34, role `embedded_master`, necessity `optional`, canonical master HCM-ORG-POSITIONS). **5 primary `solution_domains`** rows (ServiceNow Vulnerability Response 14, Tenable One 80, Qualys VMDR 81, Rapid7 InsightVM 82, Wiz Cloud Security Platform 83). **7 mandatory `domain_regulations`** (ISO/IEC 27001, SOC 2, NIST CSF, NIS2, DORA, NERC CIP, CMMC). **3 `business_function_domains`** rows (SOC owner 95, IT Operations contributor 96, Software Engineering contributor 97). **1 inbound handoff** 36 (SAM 52 -> VULN-MGMT 13, trigger 122 `software_install.detected`, payload 59 `software_installations`, integration_pattern `event_stream`, friction `low`, source_domain_module_id NULL, target_domain_module_id NULL). **0 outbound handoffs.** **0 intra-domain handoffs.** **0 `handoff_processes`** for handoff 36. **0 `trigger_events`** rooted on VULN-MGMT-mastered data_objects. **0 `data_object_aliases`** and **0 `domain_aliases`**. **0 `skills`** rooted on VULN-MGMT. **Empty `catalog_tagline`** and **empty `catalog_description`**. `parent_domain_id=11` (SECOPS). `record_status='new'`. Description "Identification, prioritization, and remediation of security vulnerabilities across endpoints, cloud, and applications." (American spelling, clean of em-dash). business_logic "Vulnerability scanning engines (network, host, web, container, code), CVE matching, and risk scoring: the algorithmic surface is the product." (no em-dash). Seven business-meta fields populated (`crud_percentage=35`, `min_org_size='30 m <2500'`, `cost_band='$$$'`, `certification_required=true`, `usa_market_size_usd_m=3500`, `market_size_source_year=2025`). **Semantius score: uncomputable** (no modules, no skills).
- **Bucket 1 (in-scope, agent fixable):** 5 items. Re-derived from the 2026-05-30 audit minus B1-V3 and B1-V4 (resolved via PATCH on 2026-05-31). All 5 remaining items unchanged in shape.
- **Bucket 2 (surface-for-user, judgment):** 6 items, unchanged from 2026-05-30. None have been answered.
- **Bucket 3 (Phase 0 pending, speculative):** 8 entity candidates + 5 regulation candidates + 2 modularization candidates, unchanged from 2026-05-30. All gated on B2-V1 (leadership-tier reconsideration).

### Structural pass bands (this audit run)

- **A1 PASS.** `domains.description` is American English ("prioritization") and clean of em-dash. `domains.business_logic` clean of em-dash. No vendor names in either column (Rule #18 OK).
- **A2 hard-fail.** 0 `capability_domains` rows. Vendor surface supports >=5 capabilities (asset discovery, scan orchestration, finding lifecycle, risk scoring, remediation routing). Rule #14's M2 floor will kick in once these load. Tracked as B1-V1 (capabilities are loaded together with modules per the prior audit's loader plan).
- **A4 hard-fail (Rule #20).** Both `catalog_tagline` and `catalog_description` empty. Tracked as B1-V2 (gated on B2-V5 wording approval).
- **M1 hard-fail (Rule #14).** 0 `domain_modules` rows of any `module_kind`. Even under the leadership-tier exception (which only waives B1, not M1), Rule #14 mandates >=1 `module_kind='full'`. Tracked as B1-V1 (gated on B2-V1 leadership-tier reconsideration and B2-V2 module split).
- **B1 pass-by-exception.** VULN-MGMT in SKILL.md leadership-tier list. Exception itself open to reconsideration; tracked as B2-V1.
- **B5 vacuous pass.** No masters -> embedded_master integrity check vacuously passes.
- **B7 vacuous pass.** No masters -> intra-domain relationship sufficiency vacuous.
- **B9 vacuous pass on outbound.** 0 outbound handoffs.
- **B9b pass.** 0 outbound handoffs to evaluate for outbound trigger-event direction.
- **B10b hard-fail on inbound.** Handoff 36 has both `source_domain_module_id` and `target_domain_module_id` NULL. Source side is owed by SAM (report-only for this audit). Target side is VULN-MGMT's own B10b but blocked by M1 (no module to point at). Tracked as B1-V5 (gated on B1-V1).
- **B11 vacuous pass.** No masters -> no `data_object_aliases` gap (those are master-keyed).
- **B12 vacuous pass.** No masters -> no `data_object_lifecycle_states` gap.
- **C1 pass.** 3 `business_function_domains` rows (SOC owner via business_function_id=64, IT Operations contributor via 27, Software Engineering contributor via 26). Shape is reasonable for the domain.
- **C2 vacuous pass.** No capabilities -> no capability-level responsibility override possible.
- **D pass.** 7 mandatory `domain_regulations` (ISO/IEC 27001, SOC 2, NIST CSF, NIS2, DORA, NERC CIP, CMMC). Additional regulation candidates (PCI-DSS 4.0, HIPAA Security Rule, FedRAMP / NIST 800-53 RA-5+SI-2, SOX 404 ITGC, GDPR Article 32) tracked in Bucket 3 unchanged.
- **E1 blocked-by-M1.** No modules -> no `role_modules` rows possible.
- **E2 blocked-by-M1.** No modules -> no module-scoped `permissions` rows possible.
- **E3 blocked-by-M1.** No modules -> no `role_permissions` for module-scoped permissions.
- **E4 blocked-by-M1.** No `data_object_lifecycle_states` rows -> no workflow-gate permissions to materialize (also blocked by absence of masters).
- **E5 blocked-by-M1.** No `permission_hierarchies` rows possible without permissions.
- **F1 blocked-by-M1.** No modules -> no skills slot to fill.
- **F2 blocked-by-M1.** No modules -> >=1 `skill_type='system'` skill per module check is uncomputable (0/0).
- **F3 blocked-by-M1.** No system skills -> no `skill_tools` rows possible.
- **F4 blocked-by-M1.** No tools -> no `operation_kind` <-> `data_object_id` pairing to validate.
- **F5 blocked-by-M1.** Semantius score uncomputable.
- **H1 partial-fail.** 0 of 1 cross-domain handoffs (handoff 36) carry `handoff_processes` rows. Quality headline: 0 approved. Process side-bar: 0 agent_curated. Tracked as B1-V6.
- **Rule #15 notes-pollution check, clean.** No notes columns have user-unapproved content on VULN-MGMT-scoped rows (`business_function_domains.notes` all empty, `domain_data_objects.notes` not queried for this audit but no writes were made by this audit).

### Bucket 1, In-scope confirmed gaps (5 items, all carried from 2026-05-30)

| ID | Band | Finding | Fix surface | Gating |
|---|---|---|---|---|
| B1-V1 | M1 + A2 | 0 `domain_modules` rows, 0 `capability_domains` rows. Proposed shape (per 2026-05-30 audit): 4 full modules (VULN-DISCOVERY-SCANNING, VULN-FINDINGS-MANAGEMENT, VULN-RISK-PRIORITIZATION, VULN-REMEDIATION-TRACKING) + 7 capabilities. | Loader at `.tmp_deploy/fix_vuln_mgmt_m1_modules.ts`. | B2-V1 + B2-V2 + B2-V3. |
| B1-V2 | A4 (Rule #20) | `catalog_tagline` and `catalog_description` both empty. Drafts surfaced in 2026-05-30 audit; require explicit user-approved wording per Rule #20. | PATCH `/domains?id=eq.13` after user approves text. | B2-V5. |
| B1-V5 | B10b | Handoff 36 `target_domain_module_id=NULL`. Once B1-V1 lands the target is VULN-DISCOVERY-SCANNING. Source side is SAM's owed work (report-only). | PATCH `/handoffs?id=eq.36`. | B1-V1. |
| B1-V6 | H1 (APQC tagging) | 0 of 1 cross-domain handoffs carry `handoff_processes`. Inbound 36 implements the IT-Operations / software-inventory leg whose closest cross-industry PCF parent is the "9.5 Manage information technology operations" family. Requires a pre-specified `process_id` from a PCF lookup at fix time. | INSERT `handoff_processes` `(handoff_id=36, process_id=<resolved>, proposal_source='agent_curated', record_status='new')` after lookup `/processes?process_name=ilike.*software inventory*&source_framework=eq.apqc_pcf_cross_industry`. If no clean L3 match, defer to Discover Pass 3. | independent. |
| B1-V7 | MISSING (`domain_aliases`) | 0 alias rows. Proposed slate (2026-05-30): RBVM (`risk-based vulnerability management`), TVM (`threat and vulnerability management`), VR (`vulnerability response`), bare RBVM. | INSERT `domain_aliases` rows. | B2-V6 alias slate confirmation. |

#### Bucket 1 count summary

| Finding type | Count |
|---|---|
| STRUCTURAL (M1 + A2 in B1-V1; A4 in B1-V2) | 2 |
| BOUNDARY / handoff direction (B1-V5) | 1 |
| MISSING (B1-V7 alias slate) | 1 |
| APQC TAGGING (B1-V6) | 1 |
| MODULARIZATION ISSUES | 0 (routed to Bucket 2) |
| **Bucket 1 total** | **5 items** |

### Bucket 2, Surface-for-user (judgment calls) (6 items, all carried from 2026-05-30)

All 6 items remain open exactly as surfaced in the 2026-05-30 narrative. The 2026-05-31 Continuation did not resolve any Bucket 2 item; it applied only the 2 mechanical B1 PATCHes (B1-V3, B1-V4). Carried verbatim:

| ID | Question (one-line) |
|---|---|
| B2-V1 | Reconsider VULN-MGMT leadership-tier inclusion (keep / remove / defer). |
| B2-V2 | Confirm 4-module split (as drafted / alt-a / alt-b / alt-c / defer). |
| B2-V3 | Capability slate prefixing (all prefixed / cross-cutting promote VULN-RISK-SCORING + VULN-THREAT-CONTEXT / defer). |
| B2-V4 | Description rewrite scope (mechanical fixes only -> now applied via B1-V3/B1-V4; remaining: full Phase 0 description rewrite or defer). |
| B2-V5 | Catalog UX field drafts (Rule #20 approval, edit, user-author, or defer). |
| B2-V6 | `domain_aliases` slate (approve 4 / extend / defer). |

Note on B2-V4: the original B2-V4 had three options where (a) was "2 minimal PATCHes." That option is now realized via the 2026-05-31 Continuation (B1-V3 + B1-V4 applied). B2-V4 reduces to (b) full Phase 0 description rewrite, or (c) defer.

### Bucket 3, Phase 0 pending (speculative)

All 2026-05-30 candidates carry forward unchanged. All gated on B2-V1.

- **8 entity candidates** (proposed masters): `vulnerability_scans`, `vulnerability_findings`, `cve_records`, `vulnerability_risk_scores`, `kev_advisories`, `remediation_tickets`, `exception_records`, `scan_targets`. Vendor evidence basis: Tenable One, Qualys VMDR, Rapid7 InsightVM, Wiz, ServiceNow VR, plus Microsoft Defender Vulnerability Management, CrowdStrike Falcon Spotlight, Nucleus Security, Vulcan Cyber, Brinqa for orchestration. Proposed-module mapping per the 4-module split in B1-V1.
- **5 regulation candidates**: PCI-DSS 4.0 (req 11.3), HIPAA Security Rule (164.308 / 164.312), FedRAMP / NIST 800-53 RA-5 + SI-2, SOX 404 ITGC, GDPR Article 32.
- **2 modularization candidates**: (a) promote VULN-RISK-PRIORITIZATION to cross-cutting EXPOSURE-MGMT module hosting on DSPM / CSPM / APP-SEC / EASM; (b) split DISCOVERY from SCANNING into two modules.
- **Candidate-domain queue (already in `audits/_missing-domains.md`):** EASM, PTAAS, CTEM, VULN-INTEL (added 2026-05-30). CSPM, CIEM, CAASM, PATCH-MGMT, EDR, PAM already pre-queued. This audit added no new candidates.

### Cross-bucket dependencies (unchanged from 2026-05-30)

- B2-V1 (leadership-tier reconsideration) gates all of Bucket 3 + B1-V1.
- B1-V1 gates B1-V5 (B10b target FK).
- B2-V2 (module split) gates B1-V1.
- B2-V3 (capability scope) feeds B1-V1 load shape.
- B2-V5 gates B1-V2 (Rule #20 wording).
- B2-V6 gates B1-V7 (alias slate).
- B1-V6 (APQC tag on handoff 36) is independent.
- B1-V7 (alias rows) is independent of M1 (domain-scoped).

### Report-only follow-ups (owed by other domains)

Carried from 2026-05-30 unchanged:

| Owing domain | Owed work |
|---|---|
| SAM | B10b: populate `source_domain_module_id` on outbound handoff 36; source module is SAM-INST-MGMT. Also consider publishing `software_install.eol_announced` and `software_install.uninstalled` events for VULN-MGMT scope-management. |
| CMDB | Expected outbound to VULN-MGMT not loaded: `configuration_item.discovered` / `.classified` / `.decommissioned`. |
| ITSM | Expected `change_request.scheduled` outbound + inbound from VULN-MGMT `vulnerability_finding.confirmed` once VULN-MGMT modularizes. |
| SOAR | Expected inbound from VULN-MGMT `vulnerability_finding.confirmed` -> playbook activation. |
| THREAT-INTEL | Expected outbound `cve.kev_added`, `cve.epss_changed`, `threat_actor.campaign_targeting_cve` once VULN-MGMT modularizes; partial absorption possible by candidate VULN-INTEL domain. |
| HCM | No active work owed. `org_units` consumed via optional embedded_master shell (Rule #16 compliant). |
...
_(none; this is a read-only Validate b1 audit pass)_
