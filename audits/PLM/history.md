...
| 1088 | PLM-ENG-CORE → FIN | `engineering_change_order.released` | `engineering_parts` | Manage bills of material (11742 L4) | confident L4 |
...
| 1090 | PLM-ENG-CORE → FIN | `engineering_part.released` | `engineering_parts` | Manage bills of material (11742 L4) | confident L4 |
...
| 1093 | PLM-COMPLIANCE → FIN | `product_compliance_declaration.approved` | `engineering_parts` | Manage regulatory compliance (16463 L3) | confident L3 |
...
No candidates queued via `append_missing_domain.ts`: the speculative cluster B3-1 is an internal-PLM gap, not a missing-domain candidate. All 5 cross-domain neighbors (MFG-OPS, FIN, S2P, FSM, PIM) are already in the catalog.
...
| B10b , derive `target_domain_module_id` for handoffs 1088, 1090, 1093 | FIN | All three target FIN with NULL `target_domain_module_id`; FIN to backfill once modularized. |
...
| B8 , add `consumer` DMDO on `engineering_parts` (796) | FIN | Receives `engineering_change_order.released` + `engineering_part.released` + `product_compliance_declaration.approved` payloads but does not declare consumer DMDO. |
...
Each of MFG-OPS, FIN, S2P, FSM should be added to the audit backlog so their per-domain b1 audits can backfill the missing DMDOs and target_domain_module_ids.
...
| B1-S7 (B10b NULL target FKs) | Symmetric side owned by MFG-OPS / FIN / S2P / FSM; not PLM's fix. |
...
- **Bucket 1b — blocked, no agent action this pass:**
  - B10b NULL `target_domain_module_id` on 8 handoffs (1087-1094): owed by MFG-OPS, FIN, S2P, FSM b1 audits.
...
| B10b NULL `target_domain_module_id` on handoffs 1088, 1090, 1093 | FIN | All three target FIN. |
...
| B8 missing `consumer` DMDOs on handoff payloads | MFG-OPS (engineering_parts, manufacturing_boms), FIN (engineering_parts), S2P (engineering_parts), FSM (cad_drawings) | None of these target domains declare a `consumer`/`contributor` DMDO row on the payload of the cross-domain handoff they receive. |
...
- **B1B-B10B-TARGET-MODULE-FKS:** 8 outbound handoffs (1087-1094) carry NULL
  `target_domain_module_id`; symmetric side owed by MFG-OPS / FIN / S2P /
  FSM audits.
