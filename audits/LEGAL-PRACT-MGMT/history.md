...
| 918 | LEGAL-TIME-BILLING to ERP-FIN | `client_invoice.paid` | `client_invoices` | "Process accounts receivable (AR)" 10744 L3 | 303 | confident L3 |
...
- B1-S5 (4 missing cross-domain relationships) gated on target-master research: ERP-FIN (general-ledger), KMS (knowledge-articles), GRC (compliance-issue). KMS and ERP-FIN sides have presumptive masters worth confirming; GRC defers to GRC's b1 audit.
...
- **ERP-FIN b1** owes: `target_domain_module_id` on handoffs 917 and 918; consumer DMDOs on `client_invoices` in the receiving ERP-FIN module; relationships from `client_invoices` to the ERP-FIN general-ledger and customer-payment masters.
