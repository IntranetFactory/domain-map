...
- **B10b , per-module attribution on handoffs.** **FAIL.** All 8 cross-domain handoffs (7 outbound + 1 inbound) have NULL `source_domain_module_id` and NULL `target_domain_module_id`. Outbound side cannot be attributed because INS-CLAIMS has no modules. Of the targets, only CSM has modules; FIN (id 65), GRC (id 15), and TELEMATICS (id 148) all have zero `domain_modules` rows, so target attribution is blocked on their side too. Routes to B1-M1 plus a cross-domain report-only note for FIN, GRC, and TELEMATICS.
...
|---|---|---|---|
| FIN | outbound | 3 (claim_payment.issued, claim_settlement.approved, salvage_recovery.posted) | 3 |
...
Pairwise reconciliation deep-dive (Pass 4) runs against neighbors at weight >= 3: **FIN** and **CSM**. GRC and TELEMATICS get a one-line summary in Pass 4 below.

### Pass 4 , Pairwise reconciliation

**INS-CLAIMS x FIN (weight 3).** FIN has zero `domain_modules` rows. None of the four-leg analysis can be completed structurally because the target side cannot be attributed. All three handoffs (claim_payment.issued, claim_settlement.approved, salvage_recovery.posted) carry NULL `target_domain_module_id` and the natural target (a GL-postings / journal-entries module on FIN) does not exist as a data_object in the catalog either. Verdict: **everything in this boundary is blocked on FIN's modularization.** Routes to a report-only entry against FIN. The relationship mirror `claim_payments posts_to journal_entries` is similarly blocked.
...
| 905 | INS-CLAIMS to FIN | `claim_payment.issued` | `claim_payments` | Post AR activity to the general ledger | 10803 | 1359 | medium (claim-payment posting is an AP-side journal entry in practice, but PCF's GL-posting activity is the closest cross-industry tag; high enough for a tag but flag to GL specialist on review) |
| 906 | INS-CLAIMS to FIN | `claim_settlement.approved` | `claim_settlements` | Post AR activity to the general ledger | 10803 | 1359 | medium (same rationale as 905; settlement approval triggers reserve release plus GL accrual) |
...
| 909 | INS-CLAIMS to FIN | `salvage_recovery.posted` | `salvage_recovery_records` | Post AR activity to the general ledger | 10803 | 1359 | low (salvage recovery posting is a recovery-side journal entry; PCF has no insurance-recovery activity; tag is for completeness but worth a reviewer's eye) |
...
| B1-B10b | B10b | Source-side PATCH gated on B1-M1; target-side FIN / GRC / TELEMATICS gated on those domains modularizing. No FKs derivable from existing modules (CSM target side blocked on CSM declaring consumer DMDOs, per the report-only follow-up). Defer. |
...
- No relationships were authored cross-domain into FIN, GRC, or TELEMATICS targets. Those remain blocked on the respective domain modularization (tracked under the "Report-only follow-ups" section above).
...
Structural Validate b1 audit re-running the 2026-05-30 baseline plus the 2026-05-31 technical residual continuation. Live state confirmed: domain id 44, 10 master `data_objects` (623 to 632), still 0 `domain_modules`, 0 `capabilities`, 0 `domain_module_data_objects`, 0 `data_object_lifecycle_states`, 0 `data_object_aliases`, 0 `handoff_processes` across all 8 cross-domain handoffs. The 21 `data_object_relationships` from the 2026-05-31 residual pass are in place (ids 464 to 465 from the original load, 1706 to 1725 from the 2026-05-31 continuation). Legacy domain-level system skill 71 (`ins-claims-system`) still standalone with 12 `skill_tools` rows (10 platform `query_*` + 1 platform `send_email` + 1 external `execute_payment`). 11 `domain_regulations` rows (all `applicability=mandatory`). 2 `business_function_domains` rows (Business Operations owner, Finance contributor). 5 `solution_domains` rows (3 primary, 2 secondary). `domains.business_logic` em-dash already cured (colon present). `catalog_tagline` and `catalog_description` still empty strings.

- Current footprint: 10 master `data_objects` across 0 modules + 21 intra and cross-domain relationships in place
- Market surface: prior audit's 12 MISSING entities still un-loaded
- Flagship vendors enumerated: Guidewire ClaimCenter, Duck Creek Claims, Sapiens IDIT, Insurity, Snapsheet, Shift Technology
- Bucket 1 (in-scope, agent fixable): 10 items pending
- Bucket 2 (surface-for-user, judgment): 8 items pending
- Bucket 3 (Phase 0 pending, speculative): 12 items pending

### Verification of 2026-05-31 continuation

Re-checked all 4 applied fixes from the 2026-05-31 residual pass:

| ID | Verified |
|---|---|
| B1-A1 | `domains.business_logic` reads `"Adjudication rules engine, fraud detection, and reserve calculations: the irreducible insurance kernel; FNOL and case workflow are declarative."`, no U+2014. Pass. |
| B1-B6 | All 9 intra-domain edges (ids 1706 to 1714) present with the proposed verbs. Pass. |
| B1-B7 | All 9 user-edge rows (ids 1715 to 1723) present, anchored at data_object_id=748. Pass. |
| B1-B8 | `notifies customer_cases` mirrors for `insurance_claims` (1724) and `insurance_policies` (1725) present alongside the existing `opens` rows (464, 465). Pass. |

No regressions in the 2026-05-30 baseline.

### Structural-pass status (re-confirmed against live state)

Hard fail still at **M1 (zero `domain_modules`)**, which cascades as before. The 2026-05-31 technical residual closed only the m-band-independent fixes; every other band gated on B1-M1 plus Bucket 2 #1 remains pending. No new findings produced by this audit pass; the open-items index is the residual of the 2026-05-30 audit minus the four 2026-05-31 fixes.

### Bucket 1, in-scope confirmed gaps (10 pending)

| ID | Band | Status | Gate |
|---|---|---|---|
| B1-A2 | A2 | new entities (8 capabilities + 8 capability_domains) | Bucket 2 #1 (module split) |
| B1-A3 | A3 | new entities (4 solutions + 4 solution_domains: Insurity, Snapsheet, Shift Technology, Origami Risk) | independent |
| B1-A4 | A4 | catalog_tagline + catalog_description copy | Rule #20, user copy |
| B1-M1 | M1 hard | 4 to 5 `domain_modules` | Bucket 2 #1 (split) |
| B1-B4 | B4 | pattern flag PATCHes (`siu_cases.has_personal_content`, `claim_settlements.has_single_approver`, `claim_payments.has_submit_lock`) | Bucket 2 #6 (config-shape exemptions) |
| B1-B10 | B10 | consumer DMDO on `dashcam_events` (378) | B1-M1 (INS-CLAIMS-INTAKE must exist) |
| B1-B10b | B10b | 8 handoffs all NULL source / target module FKs | B1-M1 for source side; CSM (DMDOs) / FIN / GRC / TELEMATICS modularization for target side |
| B1-B11 | B11 | ~30 `data_object_aliases` rows | independent in principle, audit needs to pre-specify per-master alias tuples |
| B1-B12 | B12 | lifecycle state machines for 5 to 7 workflow-bearing masters | B1-M1 (domain_module_id on every state) + Bucket 2 #6 |
| B1-F1 + B1-F7 | F1 / F7 | retire legacy skill 71, author module-level system skills, rewire `send_email` -> `notify_person` | B1-M1 |
| H1 (6 + 2) | H1 | `handoff_processes` agent_curated rows | Bucket 2 #5 (APQC strategy) |

### Bucket 2, surface-for-user (8 pending, unchanged from 2026-05-30)

1. **Modularization split**: 4-module vs 5-module vs 5-module with separate INS-CLAIMS-COMPLIANCE.
2. **Policy admin scope**: keep `insurance_policies` + `policy_coverages` on INS-CLAIMS, move to a stub INS-POLICY-ADMIN, or embed_master.
3. **Functional ownership review**: keep Business Operations owner + Finance contributor, or add Risk-and-Compliance contributor + Customer Service consumer.
4. **Regulation scope**: add NAIC Model #900 (Unfair Claims Settlement Practices), prune firm-level regulations not specifically claims-shaped (eIDAS, FATCA, SFDR).
5. **APQC tagging strategy**: tag with closest cross-industry PCF (a), defer to PCF Insurance vertical (b), or author custom processes (c).
6. **Lifecycle config-shape exemptions** for `policy_coverages`, `loss_incidents`, `claim_adjuster_assignments`, `salvage_recovery_records`.
7. **B12 verb override decisions** for `close_claim`, `approve_settlement`, `issue_payment`, `substantiate_siu_case`.
8. **Description rewrite** post-modularization (Rule #20).

### Bucket 3, Phase 0 pending (12 pending, unchanged from 2026-05-30)

`claim_reserves`, `claim_exposures`, `claim_activities`, `coverage_decisions`, `denial_letters`, `claim_notes`, `loss_estimates`, `claim_documents`, `claim_parties`, `regulatory_claim_reports`, `litigation_files`, `recovery_disbursements`. All slot into modules that only exist after Bucket 2 #1.

### Cross-bucket dependencies

- B2 #1 (modularization split) gates B1-M1, B1-A2, B1-B10, B1-B10b (source side), B1-B12, B1-F1, B1-F7 and every B3 candidate.
- B2 #6 (config-shape exemptions) gates B1-B4 and B1-B12 per-master.
- B2 #5 (APQC strategy) gates the 6 agent_curated rows + 2 Discover deferrals on H1.
- B1-B10b target side blocked separately on CSM (DMDOs), FIN, GRC, TELEMATICS modularization.

### Report-only follow-ups (owed by other domains, unchanged)

- CSM: declare consumer DMDOs on `insurance_claims` (623) and `insurance_policies` (624) under CSM-CASE-MGMT (112) to unblock handoff 904, 908, 910 target-side PATCH.
- FIN: modularize (zero modules today) to unblock handoff 905, 906, 909 target-side PATCH.
- GRC: modularize to unblock handoff 907 target-side PATCH.
- TELEMATICS: modularize to unblock handoff 315 source-side PATCH and the `dashcam_events` payload tagging.
...
_(None this pass, audit-only run; the 2026-05-31 continuation already closed the technical residuals.)_
...
- **b1b** (all blocked on the build B1A-BUILD or owed by other domains): B1B-A2-CAPABILITIES, B1B-M1-MODULES, B1B-B10-DASHCAM-CONSUMER, B1B-B10b-HANDOFF-MODULE-FKS (also owed by CSM / FIN / GRC / TELEMATICS), B1B-B11-ALIASES (gated on B2-B11-TUPLES), B1B-B12-LIFECYCLE (gated on build + B2-LFC-EXEMPT + B2-B12-VERBS), B1B-H1-APQC-TAGS (gated on B2-APQC-STRATEGY).
