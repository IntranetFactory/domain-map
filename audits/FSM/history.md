...
| B1-S5 | **B10b report-only (outbound NULLs owed by other domains)** | All 10 outbound FSM handoffs carry NULL `target_domain_module_id` on the receiving domain's side: 229 CSM, 230 CSM, 883 CSM, 884 FLEET-MGMT, 885 FIN, 1260 EAM, 1261 CRM, 1262 CRM, 292 REAL-EST (FSM is source), 318 FLEET-MGMT (FSM is source). Per B10b asymmetry, target module is the target domain's audit work. FSM's own side (`source_domain_module_id`) is populated on 8 of 10 (229, 230, 883, 884, 885, 1260, 1261, 1262); 292 and 318 have NULL on the source side too (legacy rows pre-modularization). FSM owes its source backfill on 292 and 318 (FSM-DISPATCH-OPS 161 for both, since both originate in the dispatch loop). | PATCH 292 set `source_domain_module_id=161`. PATCH 318 set `source_domain_module_id=161`. Schedule b1 audits on CSM, FLEET-MGMT, FIN, EAM, CRM, REAL-EST to populate the target side on the remaining 8 outbound rows. |
...
| 885 | FSM-DISPATCH-OPS → FIN | `service_work_order.completed` | `service_work_orders` | Invoice customer (10743 L3) | 302 | confident L3 |
...
- **FIN ↔ FSM (weight 1).** Outbound 885 to FIN with NULL target. No cross-relationship to FSM masters.
...
| B2-S6 | **Vertical home-services starter `HVAC-SVC-MGMT` scope.** The starter currently bundles 11 embedded masters from FSM + CRM (customers, crm_contacts), embeds `sales_quotes` (CPQ-mastered) and `customer_invoices` (FIN-mastered). Per Rule #19 invariant 2 each `embedded_master` row points at a data_object that has `role='master'` in some full module; that holds. But the starter's host junction is FSM-only (host_domain_id=31 via id 171's hosting on FSM as cross-cutting host). Question: should `HVAC-SVC-MGMT` also list CRM, CPQ, and FIN as additional `domain_module_host_domains` since it embeds shells of their masters? Today the starter is FSM-only by host metadata, which understates its cross-domain reach. | Editorial / host-junction discipline question. | (a) Add additional host_domain rows on CRM, CPQ, FIN. (b) Keep FSM-only since the home-services workflow centers there. |
...
- **B2-S6 (HVAC starter cross-domain host metadata):** add CRM / CPQ / FIN host rows or keep FSM-only?
...
| FIN | B10b: populate `target_domain_module_id` on 885. Add `consumer + optional` DMDO on `service_work_orders` (740). |
...
Structural Validate b1 pass against the live FSM catalog after the 2026-05-31 residual-technical loader landed. Verified live state: 3 full modules (FSM-DISPATCH-OPS 161, FSM-INSTALLED-BASE 162, FSM-SERVICE-CONTRACTS 163) + 1 starter (HVAC-SVC-MGMT 171, primary host NULL, hosted on FSM via `domain_module_host_domains`). 7 capabilities, 8 solutions (5 primary, 2 secondary, 1 in `IFS Cloud EAM` secondary). 7 FSM masters: `service_work_orders` (740), `field_visits` (261), `dispatch_records` (262), `installed_equipment` (819), `service_pm_schedules` (820), `customer_sites` (821), `service_contracts` (741). 35 lifecycle states across 7 masters. 23 aliases. 23 permissions (12 baseline incl. starter, 10 workflow-gate, 1 starter workflow-gate-shaped deferred). 4 system skills (one per full module + 1 starter), `skill_tools` populated. 3 FSM roles (Dispatcher, Technician, Manager) on 8 `role_modules` rows.

### Verified clean (Bucket 1 fixes applied 2026-05-31)

| Band | Status | Live evidence |
|---|---|---|
| B9 trigger_events.event_category | PASS | 999/1000/1001/1002/1003 all carry valid enum values (lifecycle, state_change, state_change, state_change, lifecycle). |
| B10b FSM-side `target_domain_module_id` | PASS | 1094 -> 162; 291/299/304/940/941/945 -> 161. |
| B10b FSM-side `source_domain_module_id` | PASS | 292/318 -> 161. |
| B14 Rule #15 notes pollution on handoff 1094 | PASS | `notes=''`. |
| Rule #18 em-dash sanitization on `domains.id=31 business_logic` | PASS | no U+2014; American-English ("optimization"). |
| H1 quantity progress | PARTIAL | 16 of 17 cross-domain handoffs carry agent_curated tags (1260 has two: 353 and 355). Quality headline (`record_status='approved'`) still 0/17 -> hard fail on the H1 quality measure. |

### Remaining structural bands

| Band | Status | Detail |
|---|---|---|
| A1 (Rule #8 domain metadata) | PASS | All 7 fields populated. |
| A4 (Rule #20 catalog UX on `domains`) | FAIL | `catalog_tagline=''`, `catalog_description=''` on id=31. |
| M1, M2, M4, M6 | PASS | 3 full + 1 starter; 7 caps with each realized; all 4 modules realize >=1 cap. |
| M7 (single-master + within-domain coherence) | HARD FAIL | 4 sibling `consumer` DMDOs still live (834, 835, 836, 837) on installed_equipment + customer_sites in FSM-DISPATCH-OPS and FSM-SERVICE-CONTRACTS. Gated on B2-S1 user judgment (DELETE vs PROMOTE), unresolved from prior audit. |
| M8 (Rule #20 catalog UX on `domain_modules`) | FAIL | All 4 FSM modules (161/162/163/171) have empty `catalog_tagline` + `catalog_description`. |
| B5 / B11 / B12 (lifecycle states + aliases on masters) | PASS | states present on all 7 masters; >=1 alias each. |
| B7 (`domain_data_objects` rollup) | PARTIAL FAIL | Rollup still missing 3 masters: installed_equipment (819), service_pm_schedules (820), customer_sites (821). Carried forward from prior audit; not in residual-technical scope. |
| B10b outbound NULL targets (owed by neighbors) | REPORT-ONLY | 229/230/884/885/1260 outbound still NULL target on neighbor side. CSM/FLEET-MGMT/FIN/EAM owe. |
| C (business_function_domains) | PASS | 2 rows: Customer Field Service owner, Supply Chain contributor. |
| D (business_function_capabilities) | INFO | No domain-divergent rows; not a blocker. |
| E1-E5 (roles + permissions) | PASS | 3 FSM roles, 8 `role_modules`, 23 permissions across 4 modules; workflow-gate counts match `requires_permission=true` lifecycle states (DISPATCH-OPS: 2/2; INSTALLED-BASE: 7/7; SERVICE-CONTRACTS: 1/1; starter HVAC-SVC-MGMT: baseline-only). |
| F1-F5 (system skills + tools) | PASS | 4 system skills (227/228/229/236); `skill_tools` populated; `operation_kind` pairing valid on sampled rows. |
| H1 quality (APQC `record_status='approved'`) | HARD FAIL | 0 of 17 cross-domain handoffs at `approved`; 16 at `agent_curated` `new`. Quality measure unmet until reviewer signs off. |
| H1 quantity (APQC tags) | NEAR-PASS | 16 of 17 tagged; 945 (UTIL-OPS `meter_read.anomalous`) and 1261 (CRM `service_pm_schedule.overdue`) remain untagged. Handoff 230 still carries the original `discovery_substring` row pointing at process 777 (Calculate and optimize destination dispatch plan, L4); the prior-audit REPLACE proposal was not applied. |
| Rule #20 (catalog_tagline/catalog_description) | FAIL on 5 surfaces | 1 `domains` row + 4 `domain_modules` rows all empty; backfill requires user-approved drafts. |

### Bucket 1, In-scope confirmed gaps

| ID | Band | Finding | Fix |
|---|---|---|---|
| B1-V1 | B7 missing `domain_data_objects` rollup | 3 of 7 masters absent from legacy rollup: installed_equipment 819, service_pm_schedules 820, customer_sites 821. | INSERT 3 `domain_data_objects` rows: (31, 819, master, required), (31, 820, master, required), (31, 821, master, required). |
| B1-V2 | Pairwise UTIL-OPS DMDO | FSM-DISPATCH-OPS (161) has no `consumer` DMDO on `utility_service_orders` (666), `utility_assets` (664), `meter_reads` (662) despite 3 inbound handoffs from UTIL-OPS landing on it. | INSERT 3 `domain_module_data_objects` rows: (161, 666, consumer, optional), (161, 664, consumer, optional), (161, 662, consumer, optional). |
| B1-V3 | H1 quantity untagged handoffs (2) | 945 (UTIL-OPS `meter_read.anomalous`) and 1261 (CRM `service_pm_schedule.overdue`) carry no `handoff_processes` row. | INSERT 2 `handoff_processes` rows. Candidate for 945: process 828 "Report maintenance issues" 10319 L4 (medium, alt: defer to Discover Pass 3). Candidate for 1261: process 196 "Manage customer service problems, requests, and inquiries" 10388 L3 (medium, overdue PM triggers CRM customer-care work). |
| B1-V4 | H1 handoff 230 REPLACE | Existing `discovery_substring` row points at process 777 (L4 dispatch-planning), weak fit for CSM-side service-failure receipt. Better fit is process 196 "Manage customer service problems, requests, and inquiries" L3. Destructive change deferred from prior audit. | DELETE existing `handoff_processes` row on (230, 777); INSERT (230, 196, agent_curated, new). |

### Bucket 2, Surface-for-user (judgment calls)

Carried forward unresolved from the 2026-05-30 audit (no Decisions section was populated; the 2026-05-31 residual loader did not act on Bucket 2).

| ID | Question | Options |
|---|---|---|
| B2-V1 | M7 architectural choice on 4 sibling consumer DMDOs (834, 835, 836, 837). DELETE drops the rows and lets sibling modules read by canonical reference. PROMOTE flips each to `embedded_master` to preserve standalone deployability of DISPATCH-OPS / SERVICE-CONTRACTS without INSTALLED-BASE. Agent default DELETE (standalone DISPATCH-OPS without an installed-base shell has no equipment to service; standalone SERVICE-CONTRACTS without an installed-base shell has nothing to cover). | (a) DELETE all 4 (b) PROMOTE all 4 to embedded_master (c) Mixed per row. |
| B2-V2 | Pattern-flag re-evaluation per Rule #12 on 6 candidates: `service_work_orders.has_submit_lock`, `field_visits.has_personal_content`, `dispatch_records.has_submit_lock`, `installed_equipment.has_personal_content`, `service_contracts.has_submit_lock`, `service_pm_schedules.has_submit_lock`. | Per-flag yes/no. |
| B2-V3 | Starter system skill 236 scope: self-sufficient with operational CRUD on FSM masters, or trim mutates so the starter delegates to the underlying full-module agents when co-deployed? | (a) Self-sufficient (current shape) (b) Delegate to full-module agents. |
| B2-V4 | Cross-domain role bridging FSM + CSM + CRM for service-failure / customer-care workflow. No FSM-CUSTOMER-SUCCESS or FSM-OPERATIONS-MANAGER role spans the boundary today. | (a) No cross-domain role (b) Add FSM-CUSTOMER-SUCCESS (c) Defer to CSM audit. |
| B2-V5 | Modularization: split FSM-DISPATCH-OPS into a dispatcher-side module and a mobile-tech module (paired with Bucket 3 mobile-tech entity candidates). | (a) Keep single DISPATCH-OPS (b) Split into DISPATCH-OPS + MOBILE-TECH (c) Defer until Bucket 3. |
| B2-V6 | Starter HVAC-SVC-MGMT host metadata: add CRM / CPQ / FIN host rows since it embeds shells of their masters (customers, crm_contacts, sales_quotes, customer_invoices), or keep FSM-only? | (a) Add cross-domain host rows (b) Keep FSM-only. |
| B2-V7 | Rule #20 catalog UX backfill on `domains.id=31` and 4 `domain_modules` rows. All 5 surfaces are empty; backfill requires user-approved buyer-voice drafts (`catalog_tagline` single sentence, `catalog_description` 1-3 paragraphs). | (a) Draft on next pass for review (b) Defer. |

### Bucket 3, Phase 0 pending (speculative)

Carried forward from 2026-05-30; no formal Phase 0 document for FSM has been produced. Vendor knowledge basis: ServiceNow FSM, Salesforce Field Service, MS Dynamics 365 Field Service, SAP FSM, IFS Cloud Field Service, Oracle Field Service Cloud, PTC ServiceMax, Praxedo, BigChange, ClickSoftware; vertical: ServiceTitan, Housecall Pro, Workiz, Jobber, FieldEdge.

| ID | Candidate cluster | Proposed module |
|---|---|---|
| B3-V1 | `time_entries` + `time_sheets` + `labor_rates` (technician time tracking, billable hours, rate cards). | new master in FSM-DISPATCH-OPS or sibling FSM-BILLING. |
| B3-V2 | `signature_records` + `service_photos` + `mobile_devices` (proof-of-completion artifacts; universal across all flagship FSM products). | new masters in FSM-DISPATCH-OPS (or FSM-MOBILE-TECH if B2-V5 splits). |
| B3-V3 | `safety_inspections` + `inspection_checklists` (OSHA, NEC, EPA technician safety). | new master in FSM-INSTALLED-BASE or new FSM-SAFETY module. |
| B3-V4 | `warranties` + `warranty_claims` (equipment warranty coverage, manufacturer claims; currently conflated with service_contracts). | new master in FSM-INSTALLED-BASE or sibling FSM-WARRANTY. |
| B3-V5 | `rate_cards` + `service_pricing_books` + `service_quotes` (pre-work pricing, parts markup). | new master in new FSM-PRICING module or extension of SERVICE-CONTRACTS. |
| B3-V6 | MODULARIZATION: FSM-MOBILE-TECH split (paired with B2-V5 and B3-V2). | new module. |
| B3-V7 | MODULARIZATION: FSM-WARRANTY split (paired with B3-V4). | new module. |
| B3-V8 | REGULATION: `OSHA 1910 safety standards` for US technician on-site work. FSM has zero `domain_regulations` rows. | new `domain_regulations` row. |
| B3-V9 | REGULATION: `EPA refrigerant handling` (Section 608, HVAC vertical). | new `domain_regulations` row. |
| B3-V10 | DOMAIN candidate: `TRADES-SVC` (Trades and Home-Services Field Operations) -- residential trade contractors, ServiceTitan / Housecall Pro / Workiz / Jobber / FieldEdge / BigChange as point-solution-market test vendors. | new `domains` row. |

### Cross-bucket dependencies

- B1-V1 (B7 rollup INSERT) and B1-V2 (Pairwise UTIL-OPS DMDO) are independent and agent-solvable.
- B1-V4 (handoff 230 REPLACE) is independent.
- B1-V3 (2 untagged handoffs) is independent; both candidates are medium confidence.
- B2-V1 (M7 DELETE vs PROMOTE) gates the M7 hard-fail resolution.
- B2-V5 (mobile-tech split) gates B3-V2 / B3-V6 module-placement.
- B2-V7 (Rule #20 backfill) requires user-approved wording; draft cannot be auto-loaded.

### Per-bucket prompts

- **Bucket 1, fix these now?** Reply `all`, list (e.g. `V1, V2, V3, V4`), or `skip`.
- **Bucket 2, what's your call on each?** B2-V1 through B2-V7; the agent waits for per-item decisions.
- **Bucket 3, vet via Phase 0 research or eyeball-mode?** If eyeball, name which of B3-V1..B3-V10 to treat as confirmed.

### Report-only follow-ups (owed by other domains)

| Owing domain | Owed work |
|---|---|
| CSM | Populate `target_domain_module_id` on inbound handoffs 229, 230, 883; add `consumer + optional` DMDO on `field_visits` (261), `dispatch_records` (262), `service_work_orders` (740). |
| FLEET-MGMT | Populate `target_domain_module_id` on outbound 884; add `consumer + optional` DMDO on `service_work_orders` (740). |
| FIN | Populate `target_domain_module_id` on 885; add `consumer + optional` DMDO on `service_work_orders` (740). |
| EAM | Populate `target_domain_module_id` on 1260; add `consumer + required` DMDO on `installed_equipment` (819). |
| REAL-EST | Populate `source_domain_module_id` on inbound 291; `target_domain_module_id` on outbound 292 already populated FSM-side. |
| RE-PROP-MGMT | Populate `source_domain_module_id` on inbound 299. |
| RE-CRE | Populate `source_domain_module_id` on inbound 304. |
| UTIL-OPS | Populate `source_domain_module_id` on inbound 940, 941, 945. |
...
_(none yet this audit; Bucket 1 items pending user approval)_

### Spot-check links

- https://tests.semantius.app/domain_map/domains?id=eq.31
- https://tests.semantius.app/domain_map/domain_modules?domain_id=eq.31
- https://tests.semantius.app/domain_map/domain_module_data_objects?domain_module_id=in.(161,162,163,171)
- https://tests.semantius.app/domain_map/domain_data_objects?domain_id=eq.31
- https://tests.semantius.app/domain_map/handoff_processes?handoff_id=in.(229,230,945,1261)
...
- **B2-STARTER-HOST-SCOPE.** Add CRM / CPQ / FIN rows to HVAC-SVC-MGMT (171) domain_module_host_domains since it embeds their shells, or keep FSM-only.
...
- **B1B blocked items kept:** B1B-M7 (gated on B2-M7), B1B-H1-APPROVED (gated on H1-review), and the report-only pairwise FK items owed by other domains' audits: B1B-CSM-PAIRWISE, B1B-FLEET-PAIRWISE, B1B-FIN-PAIRWISE, B1B-EAM-PAIRWISE, B1B-REAL-EST-PAIRWISE, B1B-RE-PROP-MGMT-PAIRWISE, B1B-RE-CRE-PAIRWISE, B1B-UTIL-OPS-PAIRWISE (FSM side of the UTIL-OPS pairwise is now fully complete: target FKs on 940/941/945 = 161 and consumer DMDOs 666/664/662 present; only the UTIL-OPS source FK remains owed by UTIL-OPS).
