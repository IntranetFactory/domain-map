...
| B10b outbound NULL targets (owed by neighbors) | REPORT-ONLY | 229/230/884/885/1260 outbound still NULL target on neighbor side. CSM/FLEET-MGMT/ERP-FIN/EAM owe. |
...
| B2-V6 | Starter HVAC-SVC-MGMT host metadata: add CRM / CPQ / ERP-FIN host rows since it embeds shells of their masters (customers, crm_contacts, sales_quotes, customer_invoices), or keep FSM-only? | (a) Add cross-domain host rows (b) Keep FSM-only. |
...
| ERP-FIN | Populate `target_domain_module_id` on 885; add `consumer + optional` DMDO on `service_work_orders` (740). |
