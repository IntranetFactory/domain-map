...
| FIN | 2 | 0 | 0 (no modules in target) | 0 | 2 | Lightweight |
...
The DMDO-consumer side of the boundary is empty for four of five neighbors because **those four neighbors have zero `domain_modules` rows themselves** (FOOD-TRACE, FSQM, TELEMATICS, FIN all return empty `/domain_modules?domain_id=eq.<id>`). Only FARMER-DIRECT-SALES is modularized: its `FDS-HARVEST-PLANNING` module (id 124) declares the only DMDO consumer row on any FMIS master (`harvest_records` → consumer + required). That row is the only fully-wired consumer FMIS has anywhere in the catalog.
...
| 970 | FMIS → FIN | `crop_plan.committed` | `crop_plans` | Create materials plan | 10223 (L3) | medium L3 (crop-plan-as-input-procurement-trigger fits the materials-plan shape) |
| 967 | FMIS → FIN | `ag_input_inventory.low` | `ag_input_inventory` | Order materials and services | 10279 (L3) | confident L3 |
...
| FIN | Zero `domain_modules` rows on this slice; 2 outbound FMIS handoffs target FIN (input-low, crop-plan-committed). | FIN M1 (for the procurement / cost-allocation slice that consumes farm input), B10b target-side. FIN is a large existing domain; the M1 finding here is specific to the ag-input slice not the whole FIN. |
...
- **B1-S9 (B8 cross-domain relationships).** Target-side masters on FOOD-TRACE / FSQM / TELEMATICS / FIN are unverified (those domains have zero modules); rows would point at nothing.
...
| B1-S9 | B8 (hard fail, partial gating) | 0 cross-domain outbound relationships. 6 plus payload-target mappings expected. Target-side masters on FOOD-TRACE / FSQM / TELEMATICS / FIN are unverified (those domains have zero modules); rows would point at nothing. | Defer rows whose targets are unmodularized; load the remainder. |
...
| FIN | Zero modules on the ag-input slice; 2 outbound handoffs (967, 970). | FIN M1 for the procurement / cost-allocation slice |
| FARMER-DIRECT-SALES | Has modules; 1 fully-wired consumer row on `harvest_records` (FDS-HARVEST-PLANNING, module 124). Clean. | n/a |

The cross-cluster modularization debt on FOOD-TRACE / FSQM / TELEMATICS / FIN is the largest single boundary integrity gap from FMIS's side; not an FMIS bug.
