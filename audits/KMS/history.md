...
- Live state confirmed: KMS still has 0 `domain_modules` rows and 0 `domain_module_host_domains` rows targeting KMS. M1 / M4 / M5 / F1 / F2 / F5 / E1 still FAIL, all rooted in the single unresolved Bucket 2 item 1 architectural decision (canonical-vs-derived ownership of `knowledge_articles`).
- Current footprint: 6 masters (`knowledge_base_articles` 410, `knowledge_categories` 411, `article_revisions` 412, `article_feedback` 413, `knowledge_search_queries` 414, `knowledge_collections` 415), all `role=master necessity=required` via the legacy `domain_data_objects` table only. 1 capability (`KNOWLEDGE-MGMT`), 9 solutions, 4 business_function_domains, 1 regulation (ISO/IEC 27001).
- Continuation block from earlier this date confirmed loaded: B1-S3 plural-label PATCH, 5 intra-KMS relationships (1505-1509), 4 outbound cross-domain relationships (1510-1513, CCAAS deferred), 6 `users` edges (1514-1519). B6 + B7 + B8 (outbound except CCAAS) now PASS.
- APQC `handoff_processes` now carries 17 rows across the 13 KMS-touching handoffs, all `proposal_source=agent_curated`, `record_status=new`. 0 approved. H1 process side-bar count satisfied; H1 catalog-quality headline (approved count) still 0.
- Bucket 1 (in-scope, agent fixable): 3 items (down from 5 last audit). 2 STRUCTURAL band failures pending (one new finding: domain.description em-dash violation; one continuation: catalog UX text awaiting Rule #20 approval). 1 supplementary B-band finding: trigger_events.event_category empty on all 8 KMS-master-source events.
- Bucket 2 (surface-for-user, judgment): 7 items, identical to last audit. None resolved.
- Bucket 3 (Phase 0 pending, speculative): 4 items, identical to last audit. None vetted.

### Pass 1, Structural delta against 2026-05-30 audit

| Band | Prior | Current | Delta |
|---|---|---|---|
| S1 | PARTIAL | PARTIAL | No change. `domain_modules` still 0. |
| A1 | PASS | PASS | All seven Rule #8 fields populated. |
| A2 | FAIL | FAIL | 1 capability, gated on Bucket 2 item 1. |
| A3 | PASS | PASS | 9 solutions across 3 coverage tiers. |
| A4 | FAIL | FAIL | `catalog_tagline=''`, `catalog_description=''`. Rule #20 awaits user approval. |
| A6 (new finding) | n/a | FAIL | `domains.description` contains a U+2014 em-dash: "internal and external audiences." preceded by " — vector embeddings". This violates the project's no-em-dash rule. Surgical PATCH proposed. |
| M1 | FAIL | FAIL | 0 modules. Gates Bucket 2 item 1. |
| M2 | VACUOUS | VACUOUS | Still 1 capability. |
| M4 | FAIL | FAIL | Orphan capability `KNOWLEDGE-MGMT`. |
| M5 | FAIL | FAIL | State 286 (`published`) `requires_permission=true permission_verb_override='publish_article' domain_module_id=NULL`. |
| M6 | N/A | N/A | No modules. |
| M7 | EVIDENCE | EVIDENCE | id 51 (`knowledge_articles`, ITSM-KNOWLEDGE) and id 410 (`knowledge_base_articles`, KMS) still dual canonical. Rule #9 substring collision. |
| B1 | PASS | PASS | 6 masters. |
| B2 | PARTIAL | PASS | `article_feedback.plural_label` PATCHed to "Article Feedback" in the continuation block. |
| B3 | PASS-BY-PREFIX | PASS-BY-PREFIX | No change. |
| B4 | PARTIAL | PARTIAL | Same pattern flags pending: `knowledge_search_queries.has_personal_content` decision (Bucket 2 item 6). |
| B5 | PASS | PASS | 0 embedded_master rows. |
| B6 | FAIL | PASS | 5 intra-KMS edges loaded in continuation block (rows 1505-1509). |
| B7 | FAIL | PASS | 6 `users` edges loaded in continuation block (rows 1514-1519). |
| B8 | PARTIAL | PARTIAL | 4 of 5 outbound cross-domain edges loaded (1510 HRSD, 1511 CSM, 1512 CONV-AI, 1513 CONV-AI). CCAAS `contact_center_agents` row still deferred (master not enumerated in catalog). |
| B9 | PASS | PARTIAL | 8 trigger_events all present, but every row has `event_category=''`. Enum permits `lifecycle | state_change | threshold | signal`. All 8 are lifecycle events naming a state transition on the source entity (`.published`, `.deprecated`, `.updated`, `.restructured`, `.approved`, `.negative`, `.no_result`). Proposed: 7 rows `event_category='lifecycle'`; event 759 `article_feedback.negative` is closer to `signal` (a quality signal, not a lifecycle transition). |
| B9b | VACUOUS | VACUOUS | <2 modules. |
| B10 | EVIDENCE | EVIDENCE | 8 inbound handoffs unchanged. |
| B10b | FAIL | FAIL | All 13 KMS-side legs still NULL. Resolves with M1. |
| B11 | FAIL | FAIL | 0 alias rows. Surfaced last audit, not yet loaded. |
| B12 | PARTIAL | PARTIAL | Same: states on 410 + 413; 4 masters unstated, pending Bucket 2 item 7. |
| C1 | PASS | PASS | 4 BF rows. |
| E2-E6, F2, F5 | N/A | N/A | No modules. |
| F1 | FAIL | FAIL | Skill 77 `kms-system` still `domain_module_id=NULL`. Resolves with M1. |
| F3 | PARTIAL | PARTIAL | Skill 77 carries 6 query tools (all `data_object_id` set, `operation_kind='query'`). |
| F4 | PASS | PASS | All 6 tool rows valid. |
| H1 | FAIL (1 approved baseline) | PARTIAL | 17 `handoff_processes` rows across the 13 KMS-touching handoffs, all `proposal_source=agent_curated record_status=new`. Quality headline (approved count) still 0. Process side-bar satisfied. |
...
| ID | Band | Finding | Fix |
|---|---|---|---|
| B1-S6 | A6 | `domains.description` (row 33) contains a U+2014 em-dash in the trailing clause "Search and RAG retrieval over the knowledge corpus  vector embeddings, ranking, and relevance models". Violates project no-em-dash rule. | PATCH `/domains?id=eq.33` `description` to replace the em-dash with a comma: "Search and RAG retrieval over the knowledge corpus, vector embeddings, ranking, and relevance models". Note: the same field also lives in `business_logic`; both need scrubbing. Surgical PATCH, no user wording approval needed (this is a typography fix, not new copy). |
| B1-S7 | B9 | All 8 KMS-master-source `trigger_events` rows have `event_category=''`. Enum requires `lifecycle | state_change | threshold | signal`. | PATCH each row: events 754, 755, 756, 757, 758, 761 to `lifecycle` (lifecycle transitions on the master); event 759 `article_feedback.negative` to `signal` (quality signal); event 760 `knowledge_search_query.no_result` to `signal` (gap signal from retrieval). Surgical PATCH, deterministic. |

#### Carried (deferred to user approval / Bucket 2)

| ID | Band | Why still open |
|---|---|---|
| B1-S1 | M1 / M4 / M5 / F1 / F2 / B10b | Gated on Bucket 2 item 1 module-shape decision. No agent action available. |
| B1-S2 | A4 | Rule #20 requires per-row user approval of `catalog_tagline` and `catalog_description` wording. Draft text is in the prior section; user owns acceptance. |

### Bucket 2, Surface-for-user (judgment calls)

All 7 items from 2026-05-30 carry forward unchanged. Item summaries (full text in prior section):

1. Canonical knowledge-article ownership: option (a) KMS canonical, (b) per-domain canonical, (c) hybrid two-tier. Gates B1-S1.
2. Catalog UX text approval (B1-S2 drafts). Depends on item 1.
3. Capabilities expansion: pick 5-8 from KM-AUTHORING, KM-TAXONOMY, KM-PUBLICATION-WORKFLOW, KM-VERIFICATION-CYCLES, KM-FEEDBACK-LOOP, KM-ANALYTICS, KM-GOVERNANCE, KM-AI-ASSIST. Subset depends on item 1.
4. Regulations scope: add ISO 30401, GDPR, HIPAA, SEC 17a-4, FERPA per applicability.
5. APQC tag override on handoff 826 (`discovery_substring` -> PCF 14095 "Document trade"). Continuation block added a competing `agent_curated` row (handoff 826 -> PCF 21679 "Deliver approved content"); user owns the retirement of the substring row. Independent of item 1.
6. Pattern flag `knowledge_search_queries.has_personal_content` true/false.
7. Lifecycle states on `article_revisions`, `knowledge_collections` (workflow), `knowledge_categories`, `knowledge_search_queries` (config-shape exemption).

### Bucket 3, Phase 0 pending (speculative)

All 4 candidates from 2026-05-30 carry forward unchanged:

1. `knowledge_owners` (per-article steward / SME).
2. `knowledge_subscriptions` (per-user follows).
3. `article_verification_cycles` (cadence-based accuracy review).
4. `search_synonyms` / `search_dictionaries` (likely routes to ENTERPRISE-SEARCH candidate domain).

### Cross-bucket dependencies (unchanged from 2026-05-30)

- Bucket 2 item 1 gates B1-S1, B1-S2 (buyer-voice routing), and Bucket 2 item 3 subset.
- Bucket 3 #4 depends on the ENTERPRISE-SEARCH candidate triage (`audits/_missing-domains.md`).
- Bucket 3 #1-3 conditional on Bucket 2 item 1 picking option (a) or (c).
- Bucket 2 item 5 independent.
- B1-S6 (em-dash fix) and B1-S7 (event_category fills) independent of Bucket 2 item 1.

### Report-only follow-ups (owed by other domains)

Unchanged from 2026-05-30 list. The 13 KMS-side handoff B10b legs cannot resolve until KMS M1 lands; partner-side (source on inbound) NULLs are owed by partner audits (CONV-AI 745, ECM 826, CSM 720, CCAAS 722, CONV-AI 723/724, WEB-CONTOPS 818/819).

### Per-bucket prompts

- **After Bucket 1:** "Approve the 2 STRUCTURAL fixes (B1-S6 em-dash PATCH on domain description and business_logic; B1-S7 trigger_events.event_category fills for 8 rows). B1-S1 and B1-S2 remain gated on Bucket 2 item 1."
- **After Bucket 2:** "Settle item 1 to unblock B1-S1 / B1-S2 / item 3 subset. Items 4, 5, 6, 7 are independent and can be answered in any order."
- **After Bucket 3:** "If item 1 routes to (a) or (c), pick vetted Phase 0 vendor research or eyeball-mode for the 4 candidate entities."
