...
| FIN (65) | 2 (`license.issued`, `regulatory_fee.assessed` to journal_entries) | 0 | 2 (regulatory_fees, license_records via journal_entries) | 4 | yes |
...
|---|---|---|---|---|---|---|---|
| 921 | PS-LIC → FIN | `regulatory_fee.assessed` | regulatory_fees | Process accounts receivable (AR) | 303 | 10744 | confident L3 |
...
| 925 | PS-LIC → FIN | `license.issued` | license_records | Post receivable entries (revenue / AR posting for the issuance fee) | 1353 | 10797 | confident L4 (parent PCF 303 also acceptable) |
...
| FIN (id 65) | M1 | FIN has zero `domain_modules` rows. Two PS-LIC outbound handoffs target FIN (handoffs 921, 925). Their `target_domain_module_id` blocks on FIN modularization. Cross-domain `data_object_relationships` 686 (`regulatory_fees` posts_to `journal_entries`) and 687 (`license_records` posts_to `journal_entries`) also depend on FIN's module shape. |
...
| GRC, FIN | B8 outbound to PS-LIC | These domains may publish events that PS-LIC consumes (e.g. FIN published payment-receipt event that updates PS-LIC `regulatory_fees.paid`; GRC published policy-update event that obsoletes a `code_violations` cause). Currently zero inbound handoffs to PS-LIC. Discover during those domains' B8 passes. |
| All neighbors | Pairwise reconciliation | Once PS-LIC modularizes per B1-S1, deferred pairwise diff against CSM (weight 6), FIN (weight 4), GRC (weight 4) for sections 1 to 5 of the four-leg analysis. Each will surface 1 to 3 missing handoff candidates plus the cross-domain DOR mirror checks. |
...
- **B1-S7 (residual):** source FKs on all 6 handoffs and target FKs on handoffs 921, 922, 925, 926 block on PS-LIC M1, GRC M1, FIN M1.
...
- **Bucket 1 blocked (b1b):** 5 items. PS-LIC M1 self-block, B12 lifecycle states (gate on M1), B10b residual handoff FKs (PS-LIC M1 + GRC M1 + FIN M1), B9 missing-handoff drafts (gate on M1), F2 module-level skills (gate on M1).
...
No b1a items remain after the 2026-05-31 Continuation. Every residual technical surface is parked in b1b behind PS-LIC M1, GRC M1, or FIN M1, or behind a Bucket 2 user pick.
...
- **FIN (id 65) M1.** Two PS-LIC outbound handoffs (921 `regulatory_fee.assessed`, 925 `license.issued`) target FIN. Their `target_domain_module_id` PATCH gates on FIN modularization.
- **GRC, FIN B8.** Potential inbound handoffs PS-LIC may consume (FIN payment-receipt -> regulatory_fee.paid; GRC policy-update -> code_violation invalidation). Surface during those domains' B8 passes.
...
- **All neighbors pairwise reconciliation.** Once PS-LIC modularizes, run pairwise diff against CSM (weight 6), FIN (weight 4), GRC (weight 4).
