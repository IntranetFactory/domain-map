...
- Bucket 3 (Phase 0 pending, speculative): 10 items (carry from prior audit).

### Bucket 1, In-scope confirmed gaps

#### MODULARIZATION (structural gate, unchanged from prior audit)

| ID | Finding | Proposed fix |
|---|---|---|
| B1A-MOD1 | M1 hard fail: zero `domain_modules` rows for AUDIT. Vacuously fails M2, M4, M5, M6, M8 and blocks E1-E6, F2-F5, F7, every B10b backfill, and per-module fact-sheet emission. Prior-audit proposal: 2-module split `AUDIT-PLAN-ENGAGE` and `AUDIT-FINDINGS-REPORTING`. Bucket 2 item 1 (2-module versus 3-module) is upstream and pending user input. | Blocked on Bucket 2 item 1 (B2-MOD-SHAPE). Once shape decided, author N `domain_modules` rows then load per-module DMDO, capabilities, skill, tools. |

#### STRUCTURAL (residual band failures from prior audit)
...
#### B10b BOUNDARY, per-module attribution on handoffs (blocked by MOD1)

| ID | Finding | Fix |
|---|---|---|
| B1B-B10b-OUT | 9 outbound handoffs (254, 255, 256, 257, 258, 592, 593, 594, 595) all have `source_domain_module_id=null`. Blocked by B1A-MOD1; once modules exist, run standard B10b derivation (the module that masters the trigger_event's `data_object_id`). Handoff 257 has `target_domain_module_id=38` (ITSM) already populated. | Blocked. After B1A-MOD1, backfill source-side FK deterministically. |
| B1B-B10b-IN | 31 inbound handoffs (189, 199, 247, 251, 275, 276, 531, 536, 538, 539, 541, 544, 550, 554, 558, 564, 710, 737, 738, 825, 840, 849, 852, 860, 914, 916, 920, 973, 974, 980, 1029) all have `target_domain_module_id=null`. Blocked by B1A-MOD1; for no-role payloads (B10b sub-case 2), Bucket 2 item 3 (B2-NOROLE-PAYLOADS) decides loader path per row. | Blocked. After B1A-MOD1 plus Bucket 2 item 3 resolution, backfill target-side FK and load any new `consumer` DMDO rows. |
...
Items 1-9 from the 2026-05-30 audit narrative remain open. No new Bucket 2 items this pass. Drafts the user still owes:

1. **B2-MOD-SHAPE**: 2-module versus 3-module split for AUDIT. Default: 2 modules.
2. **B2-AUDIT-VS-GRC**: scope of `management_responses`, `audit_committee_meetings`, `risk_assessments` (audit-side versus GRC-side). Affects MISSING entity placement in B3-* and prior-audit B1-M1..M7.
3. **B2-NOROLE-PAYLOADS**: 13 inbound handoffs carry payloads AUDIT does not currently model as DMDO; per-row decision (load `consumer` DMDO or accept as domain-level signal).
4. **B2-SKILL-RENAME**: rename legacy `audit-system` skill id 9 to `audit_agent` interim, or keep until per-module skills replace.
5. **B2-BAD-TAGS**: handoffs 256 (`Provide warranty-related recommendations`) and 825 (`Document trade`) carry semantically-wrong `discovery_substring` tags. Decide: DELETE versus leave for human reviewer.
6. **B2-ENG-LOCK**: `audit_engagements` `has_submit_lock=true` default decision pending.
7. **B2-SELF-LOOP**: `data_object_relationships` id=356 self-loop on `audit_findings` with verb `imports`. DELETE (default) or PATCH to `carries_forward_from`.
8. **B2-PAIRWISE**: defer pairwise reconciliation for 5 heavy neighbors or run inline.
9. **B2-DOMAIN-PEER**: confirm AUDIT remains as peer domain (default) or fold into GRC.

### Bucket 3, Phase 0 pending (speculative)

10 candidates from the 2026-05-30 audit narrative remain open: `audit_questionnaires`, `audit_walkthroughs`, `internal_control_narratives`, `audit_assertions`, `audit_sampling_methods`, `audit_time_entries`, `qar_assessments`, `audit_metrics`, `engagement_workflows`, `auditee_contacts`. Plus 7 MISSING items (B1-M1..M7 in prior audit's narrative): `audit_universe_entities`, `risk_assessments` (audit-scoped), `audit_evidence`, `audit_programs`, `audit_samples`, `management_responses`, `audit_committee_meetings`. These remain Bucket 3 / blocked because they require Bucket 2 items 1+2 resolved (module shape, AUDIT-vs-GRC scope) and Phase 0 vendor research vetting.

### Cross-bucket dependencies

- **B1A-MOD1 blocks B1A-A2 (capability authoring needs realizing modules per M4), B1A-B12 (lifecycle states need `domain_module_id`), B1B-B10b-OUT, B1B-B10b-IN, all prior-audit B1-S5 / B1-S6 / B1-S7 / B1-S8.**
- **B2-MOD-SHAPE upstream of B1A-MOD1**: shapes the module count and each MISSING entity's host.
- **B2-AUDIT-VS-GRC upstream of B1-M2 / B1-M6 / B1-M7 placement and Bucket 3 items 7-8 (`qar_assessments`, `audit_metrics`).**
- **Bucket 3 items independent of B2-NOROLE-PAYLOADS and B2-BAD-TAGS** (those are catalog-cleanup decisions, no Phase 0 dependency).
- **B1A-H1 independent of all other items.** Standalone fix; 4 inserts.

### Per-bucket prompts

- **Bucket 1.** 7 items, of which B1A-H1 (4 INSERTs into `handoff_processes`) is the only one not blocked on Bucket 2 input. Approve B1A-H1 now? Reply 'yes', 'show drafts first', or 'skip'. The other 6 items remain blocked on B2-MOD-SHAPE or are surfaced for confirmation only (B1A-B9, B1A-USR-EDGE informational).
- **Bucket 2.** 9 items, all carried from the 2026-05-30 pass. What's your call on each? B2-MOD-SHAPE is sequenced first.
- **Bucket 3.** 10 candidates plus 7 prior-audit MISSING. Vet via Phase 0, or eyeball-mode? Choose route.

### Decisions

_(empty; populated as user makes them)_

### Fixes applied

_(none this audit pass; report-only)_

### `domains.notes` pointer (if updated)

_(not yet written; requires user-approved wording per Rule #15)_
