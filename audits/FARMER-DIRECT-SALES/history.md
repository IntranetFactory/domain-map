...
| FIN | 3 | 0 | 0 | 0 (no revenue / AR entity wired) | 3 | Pairwise (full) |
...
| ID | Band | Finding | Fix |
|---|---|---|---|
| B1-S1 | **B10b report-only (outbound NULLs owed by target domains)** | 6 outbound handoffs carry NULL `target_domain_module_id`: 364 (FIN, `csa_subscription.charged`), 960 (FIN, `farmers_market_sale.recorded`), 961 (FIN, `wholesale_order.confirmed`), 963 (FOOD-TRACE, `butcher_order.ready`), and judgment calls for 365 / 962 / 964 (CRM, all populated as `target_domain_module_id=46` CRM-PIPELINE-MGT, which is probably wrong; see B2-S5 for that judgment). Strict NULL count is 4. | Schedule b1 audits for FIN and FOOD-TRACE to populate `target_domain_module_id` per the standard B10b backfill procedure. |
...
| B1-S6 | **Missing handoff (FDS → FIN at module-level)** | 3 of 3 FIN outbound handoffs (364, 960, 961) carry NULL `target_domain_module_id`. This is B10b report-only on the FIN side (covered by B1-S1) but additionally the trio implies a single revenue / AR landing module on FIN. FIN's b1 audit will determine the receiving module(s). No structural fix on FDS's side. | Covered by B1-S1 scheduling. |
...
| 364 | FDS-CSA-MGMT → FIN | `csa_subscription.charged` | `csa_memberships` | Process accounts receivable / Process customer payments | needs PCF lookup | confident L3 |
...
| 960 | FDS-MARKET-POS → FIN | `farmers_market_sale.recorded` | `farmers_market_sales` | Process revenue accounting / Record cash receipts | needs PCF lookup | confident L3 |
| 961 | FDS-WHOLESALE → FIN | `wholesale_order.confirmed` | `wholesale_orders` | Process accounts receivable / Generate invoices | needs PCF lookup | confident L3 |
...
| ID | Description |
|---|---|
| B1-S1 | Report-only, 4 outbound NULL target_module_id, schedule audits on FIN and FOOD-TRACE |
...
| B1-S6 | Report-only, FIN landing module unresolved (covered by B1-S1 scheduling) |
...
| Owed by | What | Why |
|---|---|---|
| FIN audit | Populate `target_domain_module_id` on outbound handoffs 364 (`csa_subscription.charged`), 960 (`farmers_market_sale.recorded`), 961 (`wholesale_order.confirmed`). Likely lands in an FIN revenue / AR module. | B1-S1 |
...
| FIN audit | Optional: declare `consumer` DMDO on FDS masters if FIN's revenue / AR module reads FDS payment / order records beyond the event payload. | Pairwise downstream |
...
**FIN ↔ FDS (weight 3).** Wired pairs: 3 (outbound only: 364, 960, 961). Section 2: all 3 outbound rows have NULL `target_domain_module_id` (FIN's B10b). Section 3: missing inbound from FIN to FDS could include: refund event on subscription cancellation that would update CSA membership state; payout-completion event for wholesale orders. Defer to FIN audit. Section 4: clean. Section 5: no cross-domain `data_object_relationships` between FDS and FIN entities (no `customer_payments` or `invoices` etc. linked to FDS masters at the relationship layer). This is probably a gap; FIN's audit will catch it.
...
- **B1-S1, B1-S2, B1-S6** (B10b NULL FK backfills): owed by FIN, FOOD-TRACE, FMIS audits; not derivable from FDS-side modules.
...
- **Current footprint:** 7 full modules (FDS-CSA-MGMT 118, FDS-ONLINE-STORE 119, FDS-DELIVERY-OPS 120, FDS-MARKET-POS 121, FDS-WHOLESALE 122, FDS-BUTCHER 123, FDS-HARVEST-PLANNING 124) + 1 starter (CSA-STARTER 172). 9 domain-owned masters across modules (csa_memberships 514, csa_share_packs 515, farmers_market_sales 516, wholesale_orders 517, delivery_routes 518, pickup_locations 519, butcher_orders 520, harvest_forecasts 521, farm_storefronts 522) + 1 contributor (customers 97, shared with CRM) + 1 consumer (harvest_records 490, mastered by FMIS). 13 capabilities (17 capability-to-module mappings; 4 to CSA-STARTER). 7 primary solutions (Barn2Door, Local Line, GrazeCart, Harvie, Open Food Network, CSAware, Local Food Marketplace). 12 trigger_events all carrying event_category. 14 cross-domain handoffs (8 outbound, 6 inbound) + 9 intra-domain lifecycle_progression handoffs (1272-1280). 28 data_object_relationships including 11 platform_builtin users edges. 20 aliases. 42 lifecycle states; 24 with requires_permission=true. 8 system skills (1 per module, F2 pass) with 53 skill_tools rows. 24 baseline + 24 workflow-gate FDS permissions + 3 starter baselines = 51 total.
- **Verification of prior continuation fixes:** B1-S3 confirmed in live state (data_objects.notes for IDs 514, 516, 517, 520, 521 all empty). B1-H1 confirmed (10 of 14 cross-domain handoffs now carry handoff_processes rows: 350 pre-existing, 363/364/960/961/1213/1214/1215/1216/1217 from continuation; all proposal_source=agent_curated, record_status=new).
- **Residual gaps after continuation:** 4 untagged handoffs (365, 962, 963, 964) gated on B2-S5. 19 skill_tools.notes rows still polluted per Rule #15. 5 B10b NULL FK boundary findings owed by neighbor domains. 7 Bucket 2 user-judgment items unresolved. 7 Bucket 3 speculative entities + regulation gaps unresolved.
...
| WRONG-OWNERSHIP | 0 |
| SCOPE-CREEP | 0 |
| STRUCTURAL (S/A/M/B/C/E/F band failures) | 1 (Rule #15 skill_tools.notes pollution, 19 rows; gated on B2-S2) |
| BOUNDARY (NULL FK or missing handoff) | 3 (B10b outbound, B10b inbound, missing CRM-to-OnlineStore) |
| **APQC TAGGING** | 4 deferred (365, 962, 963, 964; gated on B2-S5 / no clean PCF for 963) |
| MODULARIZATION ISSUES | 0 (routes to Bucket 2) |
| **Bucket 1 total** | 8 pending items |

#### STRUCTURAL band failures (still open)

| ID | Band | Finding | Fix |
|---|---|---|---|
| B1-S4 | F7 / Rule #15 (skill_tools.notes pollution, fully enumerated) | 19 skill_tools rows carry workflow-context, channel-justification, or read-purpose prose: skill 189 ids 1760, 1761, 1762, 1766, 1767; skill 190 ids 1773, 1774, 1775; skill 191 ids 1777, 1779, 1782; skill 192 ids 1785, 1786, 1787; skill 193 ids 1789, 1792; skill 194 ids 1797, 1798; skill 195 id 1801. Each text restates routing intent ("Pickup reminders, swap-deadline nudges, billing receipts.") or channel justification, which Rule #15 explicitly forbids. | Pending B2-S2 decision: REVERT (PATCH each notes to '') or APPROVE-PER-ROW (user supplies exact wording per row). |

#### BOUNDARY findings (still open, owed by other domains)

| ID | Band | Finding | Fix |
|---|---|---|---|
| B1-S1 | B10b report-only (outbound NULLs) | 4 outbound handoffs carry NULL target_domain_module_id: 364 (FDS-CSA-MGMT to FIN, csa_subscription.charged, csa_memberships), 960 (FDS-MARKET-POS to FIN, farmers_market_sale.recorded, farmers_market_sales), 961 (FDS-WHOLESALE to FIN, wholesale_order.confirmed, wholesale_orders), 963 (FDS-BUTCHER to FOOD-TRACE, butcher_order.ready, butcher_orders). | Schedule b1 audits for FIN and FOOD-TRACE to populate target_domain_module_id. |
| B1-S2 | B10b report-only (inbound NULL) | 1 inbound handoff 350 (FMIS to FDS-HARVEST-PLANNING, harvest_record.created, harvest_records, manual_handoff/high friction) carries NULL source_domain_module_id. FDS target is populated to 124. | Schedule b1 audit for FMIS to populate source_domain_module_id. |
| B1-S5 | Missing handoff (gated on B2-S4) | Candidate inbound CRM to FDS-ONLINE-STORE on crm_opportunity.closed_won (payload customers). Small-farm CRM coverage is uneven; only author if user confirms CRM is the system of record for DTC lead conversion. | Pending B2-S4 user choice. |

#### APQC TAGGING (residual)

| handoff_id | source then target | trigger_event | payload | Proposed PCF row | confidence | status |
|---|---|---|---|---|---|---|
| 365 | FDS-CSA-MGMT to CRM | csa_share_pack.delivered | csa_share_packs | Manage customer service requests / Service delivery | medium | deferred, target probably wrong (B2-S5) |
| 962 | FDS-DELIVERY-OPS to CRM | delivery_route.dispatched | delivery_routes | Manage customer service requests / Service quality | medium | deferred, target probably wrong (B2-S5) |
| 963 | FDS-BUTCHER to FOOD-TRACE | butcher_order.ready | butcher_orders | Manage product / service quality (16448 family resolved to risk-mgmt; no clean alternative) | low | deferred, defer to Discover Pass 3 custom-process authoring |
| 964 | FDS-HARVEST-PLANNING to CRM | harvest_forecast.updated | harvest_forecasts | Communicate with customers (10112 family) | medium | deferred, target probably wrong (B2-S5) |

### Bucket 2, Surface-for-user (judgment calls)

(All 7 items unchanged from 2026-05-30 audit; restated below for self-containment per state.yaml schema.)

- **B2-S1 (M7 architectural choice for FDS module deployability):** 5 sibling consumer DMDO rows (csa_share_packs in DELIVERY-OPS, wholesale_orders in DELIVERY-OPS, delivery_routes in CSA-MGMT and WHOLESALE, pickup_locations in CSA-MGMT, butcher_orders in DELIVERY-OPS). Architectural intent appears to be: each FDS module deploys separately, masters flow via 9 lifecycle_progression handoffs. Options: (a) KEEP all 5 by-design, (b) DELETE and rely solely on handoffs, (c) PROMOTE specific rows to embedded_master.
- **B2-S2 (Rule #15 notes-pollution revert authorization for skill_tools):** 19 skill_tools.notes rows polluted. Options: (a) REVERT all to empty string and log incident in references/skill-changelog.md, (b) APPROVE per row with user-supplied exact wording, (c) MIXED.
- **B2-S3 (FDS-DELIVERY-OPS as cross-cutting module candidate):** Module masters delivery_routes only but is destination of 4 cross-module lifecycle handoffs and consumes 4 sibling masters. Options: (a) leave as-is, (b) rename to FDS-FULFILLMENT and add explicit host junctions to FDS-CSA-MGMT, FDS-WHOLESALE, FDS-BUTCHER, (c) split into per-channel modules.
- **B2-S4 (B1-S5 missing inbound CRM purchase-intent handoff to FDS-ONLINE-STORE):** Should CRM seed DTC online-store purchase intent? Options: (a) author handoff (crm_opportunity.closed_won, payload customers), (b) skip (storefront-as-CRM), (c) defer to Bucket 3 vendor research.
- **B2-S5 (CRM target-module accuracy on 3 outbound handoffs):** Handoffs 365, 962, 964 all target CRM-PIPELINE-MGT (46). Payload+event suggests customer-marketing/engagement, not pipeline-mgmt. Options: (a) confirm CRM-PIPELINE-MGT is correct, (b) retarget to a different CRM module per upcoming CRM b1 audit, (c) retarget away from CRM entirely.
- **B2-S6 (E6 permission-bundle drift):** 24 workflow-gate permissions loaded 1:1 with requires_permission lifecycle states; role bindings not enumerated. Options: (a) confirm permission_hierarchy expands gates and leave bundles as-is, (b) add explicit gate grants on specific FDS roles, (c) leave drift.
- **B2-S7 (CSA-STARTER coverage):** Starter carries 4 embedded_masters (csa_memberships, csa_share_packs, pickup_locations, customers) + 1 platform_builtin consumer (users). Question: add delivery_routes and/or harvest_forecasts to starter? Options: (a) leave at 4 embeds, (b) add both, (c) add only one.

### Bucket 3, Phase 0 pending (speculative)

(All 7 items unchanged from 2026-05-30 audit; restated for self-containment.)

#### Entity candidates (5)

- **share_swaps** (or share_customizations): per-week member swap/pause/dietary substitution as distinct master, proposed FDS-CSA-MGMT. Vendor evidence: Harvie, Barn2Door, Local Line.
- **farmers_market_events**: parent event entity for market days (which market, which date, which booth, which vendors), proposed FDS-MARKET-POS. Vendor evidence: Open Food Network, Local Line, FoodKonnekt.
- **producer_payouts**: food-hub multi-farm split as structured payout records, proposed FDS-WHOLESALE. Vendor evidence: Open Food Network, Local Food Marketplace, GrownBy.
- **producers** (or aggregator_partners): partner producers as first-class entity for multi-farm aggregation, proposed FDS-WHOLESALE master or master-module promotion. Vendor evidence: Open Food Network, Local Food Marketplace.
- **cottage_food_disclosures** (or regulatory_disclosures): state cottage-food and sales-tax remittance disclosures, proposed FDS-MARKET-POS or new FDS-COMPLIANCE module. Vendor evidence: US state cottage-food / sales-tax regimes.

#### Modularization candidate

- **FDS-COMPLIANCE module candidate:** if FSMA / NOP / cottage food / sales tax entities get loaded, an 8th module makes more sense than overloading existing modules. Pending entity decisions.

#### Regulation candidates (zero domain_regulations rows currently loaded)

- FDA FSMA Produce Safety Rule, USDA National Organic Program (NOP), US state cottage food regulations, US state retail sales tax remittance, GAP/GHP certification, PCI-DSS.

### Cross-bucket dependencies

- B2-S2 is upstream of B1-S4 (revert authorization).
- B2-S5 is upstream of the 3 deferred APQC tags (365, 962, 964); 963 is a separate PCF-no-match problem independent of B2-S5.
- B2-S4 is upstream of B1-S5.
- B2-S1, B2-S3, B2-S6, B2-S7 are independent.
- Bucket 3 entity decisions can inform B2-S3 (FDS-FULFILLMENT discussion) and the FDS-COMPLIANCE module question.
...
_(none this pass; Validate b1 audit is read-only)_
...
- **B1B-S1 / B1B-S2** (B10b NULL FK backfills): blocked by FIN, FOOD-TRACE, FMIS b1 audits; not derivable on the FDS side.
