...
| FIN | 5 (131, 1127, 1128, 1025, 1019) | 0 | 0 | 0 | 5 | Pairwise (full) |
...
| B1-S5 | **B10b report-only (outbound NULLs owed by other domains)** | 9 outbound handoffs from PSA carry NULL `target_domain_module_id`, the receiving domain owes the fix: 131, 1127, 1128, 1025, 1019 (FIN); 1130 (PA); 1016, 1022, 1026 (HCM); 132 (S2P); 1021 (EPM). PSA's `source_domain_module_id` is populated on every outbound row. | Schedule b1 audits for FIN, PA, HCM, S2P, EPM to derive their `target_domain_module_id` per the standard B10b backfill procedure. |
...
| 1025 | PSA-PROJECT-DELIVERY -> FIN | `project_task.completed` | `project_tasks` | Manage project finances (10773) / Process invoices | confident L3 |
...
| 1019 | PSA-PROJECT-FINANCIALS -> FIN | `project_billing_milestone.reached` | `project_billing_milestones` | Process customer invoicing (10778 or child) | confident L3 |
...
**FIN <-> PSA (weight 5).** Wired pairs: 5 outbound, 0 inbound. Section 1: 5 handoffs (131, 1127, 1128, 1025, 1019). Section 2: every outbound has NULL `target_domain_module_id` (FIN's B10b). Section 3: candidate missing inbound FIN -> PSA on `customer_payment.received` to mark milestone paid; this is implied by tool 1032 `mark_project_billing_milestone_paid`. Section 4: clean. Section 5: no cross-relationship rows between PSA masters and FIN masters; the integration is event-stream, not data-object-mirror.
...
| B1-S5 | Report-only: 9 outbound NULL target_module_id; schedule FIN, PA, HCM, S2P, EPM audits. |
...
- **B1-S5 / B1-S6 (B10b report-only)** schedules audits on 9 other domains (FIN, PA, HCM, S2P, EPM, WFM, EXPENSE, VMS, AGENCY-MGMT); not PSA's load.
...
|---|---|
| FIN | B10b: populate `target_domain_module_id` on outbound handoffs 131, 1127, 1128, 1025, 1019. Add `consumer + required` DMDO on each relevant payload (`revenue_recognition_records` 109, `time_entries` 162, `expense_reports` 210, `project_tasks` 217, `project_billing_milestones` 219) in whichever FIN module receives. Note: 109 is SUB-MGMT-mastered, so FIN consumption joins the PSA contributor side as a secondary consumer. |
...
- **B1-S5 / B1-S6** (report-only): owed by FIN, PA, HCM, S2P, EPM, WFM, EXPENSE, VMS, AGENCY-MGMT, WORK-MGMT. Not PSA's load.
...
| B1-S5 (9 outbound NULL target_module, report-only) | Still owed by FIN / PA / HCM / S2P / EPM. Routed below. |
...
| Owing domain | Owed work |
|---|---|
| FIN | B10b: populate `target_domain_module_id` on outbound handoffs 131, 1127, 1128, 1025, 1019. |
...
- **b1b:** B1B-S5 (9 outbound NULL target_module owed by FIN/PA/HCM/S2P/EPM), B1B-S6
...
- **ORPHAN, owed by target domain (12, report-only):** the payload names work the receiving
  ...
  "Identify/select/assign resources" 5.4.1.6), 1025 -> 1409 (FIN "Record project
  transactions" 9.4.2.2), 1026 -> 1036 (HCM "Define competencies" 7.3.4.2), 1027 -> 887
  (SWP "Skills taxonomy" 5.2.2.1), 1127 -> 1409 (FIN), 1128 -> 59 (FIN "AP/expense"
...
- **B9d ORPHANs (12):** S2P (132), SWP (1018, 1027), EPM (1021), ATS (1023), FIN (1025,
