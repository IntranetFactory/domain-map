...
| FIN | 4 (pay_run.disbursed, pay_cycle.posted, payroll_journal.reversed, payment_run.executed) | 0 | 0 | 4 | Pairwise (full) |
...
| 99 | PAYROLL-RUN -> FIN | pay_cycle.posted | payroll_journal_entries | Process general accounting (10728 or child) | confident L3 |
...
| 1151 | PAYROLL-RUN -> FIN | pay_run.disbursed | payroll_journal_entries | Process general accounting (10728) | confident L3 |
| 1152 | PAYROLL-RUN -> FIN | payroll_journal.reversed | payroll_journal_entries | Process general accounting (10728) | confident L3 |
...
| 414 | PAYROLL -> FIN | payment_run.executed | payment_runs | Process accounts payable and expense reimbursements (10733) | medium |
...
**FIN <-> PAYROLL (weight 4).** Wired pairs: 4 outbound from PAYROLL (99, 1151, 1152, 414). Section 2: all 4 have NULL target_module_id (FIN owes). Section 3: missing handoff FIN -> PAYROLL on `bank_acknowledgement.received` (NACHA / SEPA ack for disbursement). Section 4: clean. Section 5: no cross-relationships between PAYROLL masters and FIN masters; expected `payroll_journal_entries posts_to journal_entries` (or wherever FIN masters them) is missing.
...
| B1-S7 | Report-only, 13 outbound NULL target_module_id, schedule audits on PA / FIN / EXPENSE / GRC / HCM |
...
- **S7 / S8 (B10b report-only)** schedules audits on PA / FIN / EXPENSE / GRC / HCM / WFM / VMS; not PAYROLL's fix.
...
| FIN | B10b: populate `target_domain_module_id` on outbound 99, 1151, 1152, 414. Add `consumer + required` DMDO on `payroll_journal_entries` (145) and `payment_runs` (205) on the receiving FIN module. |
...
- **Structural pass bands:**
  ...
  - **B5 pass** (every `embedded_master` data_object has a `master` row somewhere: `employees` mastered by HCM-CORE-WORKER 54, `hcm_positions` / `org_units` mastered by HCM, `cost_centers` mastered by FIN).
  ...
  - **H1 partial-progress** (catalog quality headline): 17 of 41 cross-domain handoffs carry `handoff_processes` rows (~41%). Of those 17: 9 `agent_curated` (100/108/110/188/417/1156/103/590/1118/416/426 - 11 rows actually), 5 `discovery_substring`/`discovery_override` (374/18/366 at L2 broad, 1126/101 at L2/L4, 7 at L5 mis-match `Create customer journey maps`). Zero rows at `record_status=approved`. 24 cross-domain handoffs remain completely untagged (411 ONBOARDING; 7 ONBOARDING weakly tagged at L5; 377/380/383 HCM employment_event/contract handoffs; 401/404 ATS background_check/candidate_referral; 25/102/1155 PAYROLL->PA; 1151/1152/99 PAYROLL->FIN; 415/1153 PAYROLL->GRC; 412/1154 PAYROLL->HCM; 1157/101 PAYROLL->EXPENSE second-pass; 187/423/105 COMP-MGMT->PAYROLL; 1140 COMP-MGMT->PAYROLL earning_code.created; 413 PAYROLL->BEN). Volume target per H1 (0.5N to 0.8N for N=41) = 21 to 33 `agent_curated` tags. Currently at 11 `agent_curated`, so the gap is 10 to 22 more rows.
...
| B1-S6r | **B10b PAYROLL still owes handoff 414** | Outbound 414 (`payment_run.executed` -> FIN, payload `payment_runs`) has NULL `source_domain_module_id` and NULL `target_domain_module_id`. Source side is PAYROLL's debt; target side is FIN's. The audit hedged in prior pass and surfaced for user. Proposed `source_domain_module_id=90` (PAYROLL-RUN; `pay_run.disbursed` 405 is the proximate cause of `payment_run.executed`). | PATCH handoff 414 `source_domain_module_id=90`. FIN-side target_module_id remains its B10b debt. |
| B1-S7 | **B10b report-only outbound (owed by other domains)** | 10 outbound handoffs with NULL `target_domain_module_id`: 25/102/1155 (PA), 99/1151/1152/414 (FIN), 101/1157 (EXPENSE), 1153 (GRC), 1154/412 (HCM), 415 (GRC). | Schedule b1 audits on PA, FIN, EXPENSE, GRC, HCM. |
...
| 99 | PAYROLL-RUN -> FIN | pay_cycle.posted | payroll_journal_entries | `Process general accounting` (10728 L2 family) | confident L2 |
| 1151 | PAYROLL-RUN -> FIN | pay_run.disbursed | payroll_journal_entries | `Process general accounting` | confident L2 |
| 1152 | PAYROLL-RUN -> FIN | payroll_journal.reversed | payroll_journal_entries | `Process general accounting` | confident L2 |
| 414 | PAYROLL -> FIN | payment_run.executed | payment_runs | `Process accounts payable and expense reimbursements` (10733) | medium |
...
| FIN | B10b populate `target_domain_module_id` on outbound 99, 1151, 1152, 414. Add `consumer + required` DMDO on `payroll_journal_entries` (145) and `payment_runs` (205). |
...
- **B1A-S6r** — PATCH handoff 414 `source_domain_module_id=90` (was NULL; pay_run.disbursed
  ...
  FIN-side `target_domain_module_id` remains FIN's debt.
