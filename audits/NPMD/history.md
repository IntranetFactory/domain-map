...
Structural Validate b1 pass against live state. Confirms the 2026-05-31 fix-loop deltas (3 user-edges, 5 APQC tags new + 1 already-tagged, notes revert on handoff 654), re-classifies the remaining open work under v2 schema buckets.

### Summary

- Current footprint (live PostgREST):
  - Domain row id 82, parent ITOM (2). A1 metadata complete (crud=20, business_logic non-empty, min_org_size `30 m <2500`, cost_band `$$$`, certification_required=false, usa_market_size_usd_m=2000, market_size_source_year=2025).
  - `catalog_tagline` / `catalog_description`: both empty (A4 fail unchanged).
  - `capability_domains`: 0 rows (A2 fail unchanged).
  - `solution_domains`: 0 rows; no NPM solutions in catalog (A3 fail unchanged).
  - `domain_modules`: 0 rows (M1 hard fail unchanged). `domain_module_host_domains`: 0.
  - `domain_data_objects`: 8 masters, all `master + required`: 538 `network_flow_records`, 539 `network_paths`, 540 `network_performance_metrics`, 541 `network_performance_alerts`, 542 `network_interfaces`, 543 `saas_application_performance`, 544 `network_topology_snapshots`, 545 `network_baseline_thresholds`.
  - `domain_module_data_objects`: 0 rows (downstream of M1).
  - `data_objects` flags / arbitration: all 8 carry `is_canonical_bare_word=false`, `naming_authority_rationale=''`, pattern flags all false-by-default (B3, B4 unchanged).
  - `data_object_relationships`: 3 rows present (the 2026-05-31 user-edges fix): id 1732 (541 -> 748 users, is_acknowledged_by), 1733 (545 -> 748, is_owned_by), 1734 (543 -> 748, is_owned_by). B7 now passes. B6 still 0 intra-domain master-to-master rows. B8 still 0 outbound cross-domain rows.
  - `data_object_aliases`: 0 rows (B11 fail unchanged).
  - `data_object_lifecycle_states`: 0 rows (B12 fail unchanged).
  - `trigger_events`: 9 rows. Three events have no handoff: 657 `network_path.degraded`, 661 `network_interface.flapping`, 664 `network_baseline_threshold.recalculated` (leaf, acceptable). All 9 carry `event_category=''` (Rule #13 vocabulary unset; cleanup item).
  - `handoffs` outbound (source 82): 6 rows. All 6 `source_domain_module_id IS NULL` (B10b fail unchanged, downstream of M1). Target attribution: ITSM 650 / 652 -> module 38 set; CMDB 654 -> module 109 set; AIOPS 649, OBS 651, DEM 653 target NULL (owed by neighbor M1). All 6 carry `notes=''` (Rule #15 revert held).
  - `handoffs` inbound (target 82): 0 rows (B10 report-only; NPMD declares no consumer/contributor/embedded_master dependencies).
  - `handoff_processes`: 7 rows across the 6 outbound handoffs. Coverage: 649 -> 1304, 650 -> 1299, 651 -> 1304, 652 -> 1299, 653 -> 1304 + 1137 (dual tag, both confident L4), 654 -> 1309. H1 PASSES (6/6 handoffs tagged, all `proposal_source=agent_curated`, `record_status=new`).
  - `business_function_domains`: 1 row (IT Infrastructure as owner; C1 passes). 0 contributors / consumers loaded (Bucket 2 item carried).
  - `domain_regulations`: 0 rows (no regulation loaded; Bucket 2 item carried).
  - `skills`: 1 row, `npmd-system` (id 87), `skill_type=system`, `domain_module_id IS NULL` (F1 fail unchanged). 8 `skill_tools` rows, all `query_<entity>` (operation_kind=query, data_object_id set), all `coverage_tier=platform`, all `requirement_level=required`. F4 invariant satisfied on every row; F7 vacuously passes (no channel primitives).
  - `roles` / `role_modules` / `role_permissions`: 0 / 0 / 0 (E1-E5 vacuously pass while M1 fails).

- Bucket 1 (in-scope, agent fixable): 15 items (5 agent-solvable, 10 blocked on user judgment or prerequisite).
- Bucket 2 (surface-for-user, judgment): 9 items.
- Bucket 3 (Phase 0 pending, speculative): 7 items.

Pass-level verdict: the 2026-05-31 technical fix-loop closed B7 (user-edges) and H1 (APQC tags). Notes pollution on handoff 654 was reverted. The structural backbone remains gated on M1 (modularization decision in Bucket 2 #1) and on Phase 0 vendor research (Bucket 3). Once the user picks a module split, B1-S1 unblocks B1-S15 (per-module skills) and B1-S11 (B10b backfill on the NPMD side). Capabilities (A2) and solutions (A3) are independent and agent-authorable from the proposed list pending user approval.

### Vendor surface basis

Carried verbatim from 2026-05-30. Flagship leader quadrant (pure-play NPM specialists chosen over diversified suites): Cisco ThousandEyes (acquired 2020; SaaS / internet path / BGP), Kentik (NetFlow + peering + CDN), Catchpoint (synthetic + last-mile + ISP/CDN observability), NETSCOUT nGeniusONE (packet broker + application-aware, telco-grade), Broadcom DX NetOps (large-enterprise device + flow), LiveAction LiveNX (SD-WAN-aware enterprise NPM with packet capture). No formal Phase 0 doc exists for NPMD; vendor surface inferred from public product documentation.
...
| `solution_domains` | domain_id | 0 | yes | **A3 fail** |
| `business_function_domains` | domain_id | 1 | yes | C1 passes |
| `domain_regulations` | domain_id | 0 | sometimes | Bucket 2 |
| `handoffs` source_domain_id | source | 6 | yes | partial B9 pass |
| `handoffs` target_domain_id | target | 0 | usually | B10 report-only |
| `skills` (legacy domain_id) | domain_id | 1 | no (target = 0) | **F1 fail** |
| `domains.parent_domain_id` | parent | 1 (ITOM) | sometimes | no finding |

S2 and S3 cannot run meaningfully (0 modules; lifecycle / event_category / alias counts ~zero across the board for every master and route to B9 / B11 / B12).

#### A. Phase A, Market shape

- **A1** PASS. All seven `domains` business-metadata fields populated.
- **A2** FAIL. 0 `capability_domains` rows.
- **A3** FAIL. 0 `solution_domains` rows. No NPM solutions in catalog.
...
- **M2 - M7** vacuously fail downstream of M1. M7 catalog-wide single-master check still clean (all 8 masters live only in `domain_data_objects`, no DMDO rows anywhere; no double-master hazard).
- **M8** vacuously fail downstream of M1.

Proposed module split (carried from 2026-05-30; gated on Bucket 2 #1):
...
- **B7** PASS (3 user-edges loaded 2026-05-31: 1732, 1733, 1734 cover the audit-specified set on 541 / 543 / 545).
- **B3** FAIL. None of the 8 masters has been arbitrated under Rule #9. Collision risk on `network_interfaces` (bare-word `interfaces`) and `saas_application_performance` (overlap with APM domain headline). Prefix path is open; needs positive review.
- **B4** FAIL (audit obligation). Pattern flags all false-by-default; no positive review has been recorded. Expected post-review state is `false` across all three on all 8 (signal-stream masters carry none of these patterns) but the review must happen.
- **B6** FAIL. 0 intra-domain `data_object_relationships` rows between masters. Six audit-pre-specified edges still owed: `network_paths traverses network_interfaces`, `network_performance_alerts raised_from network_performance_metrics`, `network_performance_metrics derived_from network_flow_records`, `network_baseline_thresholds applies_to network_performance_metrics`, `network_topology_snapshots contains network_interfaces`, `saas_application_performance correlated_with network_paths`.
- **B8** FAIL (outbound only). 0 cross-domain outbound `data_object_relationships` rows. Six edges expected to mirror the six outbound handoffs; some target masters (`aiops_signals`, `telemetry_streams`, `digital_experience_sessions`) may not exist yet and have to be existence-gated before the load.
- **B9** PARTIAL. 9 trigger events; 6 handoffs. Missing: 657 `network_path.degraded` and 661 `network_interface.flapping` carry no handoff. 664 `network_baseline_threshold.recalculated` acceptable as leaf.
- **B9b** N/A while module count < 2.
- **B10** REPORT-ONLY. 0 inbound handoffs; NPMD declares no consumer dependencies, so the discovery query yields zero owed inbound rows. Re-confirmed.
- **B10b** FAIL. All 6 outbound handoffs have `source_domain_module_id IS NULL` (downstream of M1). Target-side NULLs on AIOPS (649), OBS (651), DEM (653) are report-only on those domains' M1 backlog. ITSM (650, 652) and CMDB (654) target FKs are set.
- **B11** FAIL. 0 aliases on 6 non-self-explanatory masters (carried alias targets from 2026-05-30 unchanged).
- **B12** FAIL. 0 lifecycle states. Only `network_performance_alerts` (541) has a real workflow (`open -> acknowledged -> resolved -> closed` with `requires_permission=true` on acknowledge and resolve). Other 7 masters are signal-stream / config-shape and need a user-approved exemption decision (not auto-written per Rule #15).
- **Cleanup item.** All 9 `trigger_events` rows have `event_category=''`. Rule #13 vocabulary is `lifecycle|state_change|threshold|signal`. Proposed: anomaly / threshold-breach events use `threshold`; lifecycle events (`raised`, `down`, `flapping`, `updated`, `recalculated`, `degraded`) use `state_change` or `lifecycle` per event semantics. Default empty is accepted by the DB but is a quiet vocabulary gap; agent-authorable PATCH on confirmation.

#### C. Phase C, Functional ownership

- **C1** PASS. `IT Infrastructure` is owner. Bucket 2 #9 carries the contributor (`IT Operations / NOC`) and consumer (`Security`) question.
- **C2** vacuously passes (no capabilities loaded; routes to A2).

#### D. UI spot-check

Not run as part of this audit pass.

#### E. Roles and permission bundling

E1-E5 vacuously pass while M1 fails (2-module floor blocks role authoring).

#### F. Skill-layer integrity

- **F1** FAIL. Legacy `npmd-system` (id 87) with `domain_module_id IS NULL` persists. Will become obsolete once per-module skills exist; DELETE conditional on B1-S15 landing.
- **F2 - F5** vacuously fail downstream of M1.
- **F4** invariant holds on every linked tool row (8 `query` rows, all with `data_object_id` set).
- **F7** vacuously passes (no channel primitives linked).

#### H. APQC tagging

- **H1** PASS. All 6 NPMD outbound handoffs carry at least one `handoff_processes` row (7 rows total: handoff 653 dual-tagged with both 1304 and 1137). All are `proposal_source=agent_curated`, `record_status=new`. Volume target (3-5 confident) exceeded.

### Pass 2, Market audit findings

Carried from 2026-05-30 vendor surface enumeration; no fresh subagent re-run this pass (focus is structural; the 2026-05-30 market surface is recent and unchanged in catalog state).

Highest-confidence MISSING: `synthetic_tests`, `synthetic_test_runs`, `bgp_route_observations` (universal across flagship vendors), `packet_captures` (NETSCOUT / LiveAction differentiator). Lower-confidence: `network_devices`, `network_probes`, `network_sites`, `wan_circuits`, `cloud_provider_regions`, `traffic_classifications`, `network_kpi_definitions`. SCOPE-CREEP candidate: `saas_application_performance` partial overlap with DEM (Bucket 2 #2). WRONG-OWNERSHIP: none observable (modularization not started).

### Pass 3, Neighbor discovery

Edge weights via outbound handoffs (inbound is 0); unchanged from 2026-05-30:

| Neighbor | Edge weight | Source |
| --- | --- | --- |
| ITSM | 2 | 650, 652 -> ITSM-INCIDENT-MGMT (38) |
| AIOPS | 1 | 649 |
| OBS | 1 | 651 |
| CMDB | 1 | 654 -> CMDB-SERVICE-MAPPING (109) |
| DEM | 1 | 653 |

No neighbor reaches edge weight >= 3; one-line summaries only. Cross-domain `data_object_relationships` rows still missing for all five (carried under Bucket 1 B1-S9).

### Pass 4, Pairwise reconciliation

No neighbor at weight >= 3; full 5-section diff skipped per procedure.

### Bucket 1, In-scope confirmed gaps (15 items: 5 agent-solvable b1a, 10 blocked b1b)

#### STRUCTURAL (band failures)

| ID | Band | Finding | Fix |
|---|---|---|---|
| B1-M1 | M1 | Zero `domain_modules` rows. Until modules exist, M2-M8, B-band DMDO writes, B9b, E1-E5, F2-F5 cannot pass. | Author module split (default 4-module proposal) and load via a Phase-A loader. Pending Bucket 2 #1. |
| B1-A2 | A2 | Zero `capability_domains` rows. | Author 5-8 capabilities and link via `capability_domains` + `domain_module_capabilities`. |
| B1-A3 | A3 | Zero `solution_domains` rows; no NPM solutions in catalog. | Add `vendors` + `solutions` for Cisco ThousandEyes, Kentik, Catchpoint, NETSCOUT nGeniusONE, Broadcom DX NetOps, LiveAction LiveNX, plus `solution_domains` with coverage_level. |
| B1-A4 | A4 | `catalog_tagline` and `catalog_description` empty. | Draft per Rule #20 buyer voice; user approves verbatim before write. |
| B1-B3 | B3 | None of 8 masters has been arbitrated under Rule #9. | Per-master: stamp `is_canonical_bare_word=false` after positive review and capture per-master evidence outside `notes`. |
| B1-B4 | B4 | Pattern flags default-false on all 8 masters with no positive review. | Record positive review per master; expected post-review state is `false` across all flags on all 8. |
| B1-B6 | B6 | 0 intra-domain `data_object_relationships` edges between masters. | Author 6 edges per the carried list. |
| B1-B8 | B8 | 0 cross-domain (outbound) `data_object_relationships` for 6 outbound handoffs. | Author up to 6 edges; existence-gate against target masters; route missing targets to those domains' audits. |
| B1-B9 | B9 | 2 trigger events without `handoffs` rows: 657 `network_path.degraded`, 661 `network_interface.flapping`. | Author both: `network_path.degraded` -> AIOPS + ITSM; `network_interface.flapping` -> ITSM. 664 acceptable as leaf. |
| B1-B10B | B10b | All 6 outbound handoffs have `source_domain_module_id IS NULL`. | After M1 fix, run module-attribution backfill (pattern: `backfill_ats_handoff_modules_2026_05_23.ts`). Target NULLs for AIOPS / OBS / DEM are report-only on those domains. |
| B1-B11 | B11 | 0 `data_object_aliases` for 8 masters. | Author 12+ alias rows pending user approval (alias_name is freeform, Bucket 2 #5 carries refinement). |
| B1-B12 | B12 | 0 lifecycle states. Only `network_performance_alerts` has a real workflow; other 7 masters need config-shape exemption decision (not auto-written). | Load 4 lifecycle states for `network_performance_alerts` with `requires_permission=true` on acknowledge and resolve; surface exemption decision for the other 7 to user. Pending Bucket 2 #4. |
| B1-EVT | B-band cleanup | All 9 `trigger_events` rows carry `event_category=''`. | PATCH per event: anomaly / threshold-breach events `threshold`; published-state events (`raised`, `down`, `flapping`, `updated`, `recalculated`, `degraded`) `state_change` or `lifecycle` per semantics. Agent-authorable on user confirmation. |
| B1-F1 | F1 | Legacy `npmd-system` skill (id 87) persists with `domain_module_id IS NULL`. | DELETE after per-module skills authored. Migrate the 8 existing `skill_tools` rows to the per-module skills. Pending B1-M1 + B1-F2. |
| B1-F2 | F2/F3 | Once modules ship, each needs exactly 1 `skill_type=system` skill with >=1 `skill_tools`. | Author per-module system skills (`npmd_flow_analysis_agent`, etc.). Pending B1-M1. |

#### APQC TAGGING

H1 PASSES. No new tags owed in this pass.

### Bucket 2, Surface-for-user (judgment calls) (9 items)

1. Modularization split (4-module proposal vs 2-module merge). Independent of Bucket 3. Bucket 1 default: 4-module.
2. `saas_application_performance` scope vs DEM (keep in NPMD, move to DEM, or split). Affects B1-B8 target master and Bucket 3.
3. `network_devices` master (NPMD masters, NPMD `embedded_master`, or NPMD consumes CMDB `configuration_items`). Affects future synthetic-test and path-tracing references.
4. B12 config-shape exemption for 7 of 8 masters (signal-stream / write-once). Confirm exemption decision (audit surfaces; agent does not auto-annotate).
5. Aliases approve list. Bucket 1 B1-B11 needs user-approved exact alias_name strings.
6. Catalog-tagline / catalog-description voice. Rule #20: agent drafts, user approves verbatim before write.
7. Regulations, intentional zero or add PCI-DSS scope-of-monitoring.
8. F1 legacy skill 87, migrate-then-delete vs keep as transitional. Bucket 1 default: migrate then DELETE.
9. C1 contributors and consumers (`IT Operations / NOC` contributor, `Security` consumer).

### Bucket 3, Phase 0 pending (7 items)

Universal vendor entities surfaced by the semantic pass; Phase 0 vetting would confirm or filter. Carried verbatim from 2026-05-30.

| Candidate | Proposed module | Vendor evidence |
|---|---|---|
| `synthetic_tests` | NPMD-SAAS-INTERNET-PERF | Universal (ThousandEyes, Catchpoint, NETSCOUT, Kentik) |
| `synthetic_test_runs` | NPMD-SAAS-INTERNET-PERF | Universal |
| `bgp_route_observations` | NPMD-PATH-TRACING | ThousandEyes Internet Insights, Kentik peering |
| `packet_captures` | NPMD-FLOW-ANALYSIS or new NPMD-PACKET-CAPTURE | NETSCOUT, LiveAction, Riverbed |
| `network_probes` | NPMD-SAAS-INTERNET-PERF | ThousandEyes Enterprise / Cloud / Endpoint agents, Catchpoint nodes |
| `traffic_classifications` | NPMD-FLOW-ANALYSIS | LiveAction, NETSCOUT, Cisco (DPI / NBAR-based) |
| `network_kpi_definitions` | NPMD-METRICS-ALERTS | Broadcom DX NetOps, NETSCOUT |

### Cross-bucket dependencies

- Bucket 2 #2 (SaaS vs DEM) depends on Bucket 3 vendor research; if user picks the vetted route for Bucket 3, hold Bucket 2 #2 until research lands.
- Bucket 1 B1-B8 depends on Bucket 2 #2 (target master for the SaaS-degradation edge).
- Bucket 1 B1-M1 depends on Bucket 2 #1 (4-module vs 2-module split).
- Bucket 1 B1-F1 / B1-F2 depend on Bucket 1 B1-M1.
- Bucket 1 B1-B10B depends on B1-M1 (NPMD side) and on AIOPS / OBS / DEM authoring their own modules (target side, report-only).
- Bucket 1 B1-A2 / B1-A3 / B1-EVT independent of each other and of Bucket 2 / 3.

### Report-only follow-ups (owed by other domains)

- **AIOPS M1** (id 6) owes a `domain_modules` row + `target_domain_module_id` backfill on handoff 649.
- **OBS M1** (id 7) owes a `domain_modules` row + `target_domain_module_id` backfill on handoff 651.
- **DEM M1** (id 83) owes a `domain_modules` row + `target_domain_module_id` backfill on handoff 653. Tightly coupled to Bucket 2 #2.
- **AIOPS / OBS / DEM B8 inbound** owe the symmetric `data_object_relationship` rows once NPMD authors B1-B8.
- **CMDB B8 inbound** (id 4): handoff 654 target module already set; CMDB owes a `configuration_items reconciled_by network_topology_snapshots` row.
- **ITSM B8 inbound** (id 1): handoffs 650 / 652 target module already set; ITSM owes 2 symmetric relationship rows on `service_incidents`.

### Per-bucket prompts

- **Bucket 1 (15 items, 5 b1a + 10 b1b):** Fix these now? Reply 'all', a subset (e.g. 'B1A-A2, B1A-A3, B1A-EVT'), or 'skip'. b1a items (A2, A3, EVT, B6, B9) are agent-solvable today; b1b items are gated either on Bucket 2 user decisions or on prerequisite Bucket 1 items (e.g. B1B-F2 waits on B1B-M1, B1B-B10B waits on B1B-M1).
- **Bucket 2 (9 items):** What's your call on each? Per-item decisions needed before acting. Items 5 (aliases) and 6 (catalog voice) need exact text before writing per Rule #15 / Rule #20.
- **Bucket 3 (7 items):** Vet via Phase 0 research (recommended for `synthetic_tests`, `synthetic_test_runs`, `bgp_route_observations`; vendor-universal but no formal Phase 0 ran for NPMD), or eyeball-mode? If eyeball, name which candidates to treat as confirmed.
