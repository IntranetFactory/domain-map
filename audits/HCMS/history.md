...
- **Current footprint:** **0 `domain_modules` rows** (M1 hard-fail persists). 6 masters + 1 contributor (`content_types` 131, `content_entries` 132, `content_releases` 133, `editorial_workflows` 134, `content_locales` 135, `content_environments` 136 as masters; `digital_assets` 137 as contributor). 14 trigger_events (ids 33, 34, 35, 36, 46, 886-894), now all carrying a valid `event_category` (per 2026-05-31 continuation pass: B9 closes for HCMS). 15 cross-domain handoffs (12 outbound to DXP 77, B2C-COMM 71, MA 70, DAM 92, LSD 25, WEB-CONTOPS 128; 3 inbound from DAM 92 and WEB-CONTOPS 128). 0 intra-domain handoffs. 15 aliases across 6 masters. 0 lifecycle states. 33 data_object relationships. 8 capabilities (HCMS-MODEL, HCMS-DELIVERY, HCMS-AUTHORING, HCMS-I18N, HCMS-DAM, HCMS-VERSIONING, HCMS-WEBHOOKS, HCMS-VISUAL). 11 solutions (10 primary + 1 partial). 0 regulations. 1 orphan `skills` row (`hcms-system` id 67, `domain_module_id=NULL`) with 6 `skill_tools` rows (all query primitives). 0 `role_modules`. 2 `handoff_processes` rows (98 to PCF 1763; 806 to PCF 428) of 15 cross-domain handoffs.
- **Validate b1 verdict:** **M1 still hard-fails** (Rule #14 floor: zero modules on a domain with 8 capabilities and 6 masters). Every downstream M / B (except B9) / C / D / E / F band that depends on a `domain_modules` row remains unauditable until B1-S1 closes. Continuation pass landed B9 (event_category PATCHes) and partial H1 (2 of expected 8-12 tags). Everything else carried 2026-05-30 remains open: B1-S1 (modules), B1-S3 (skill orphan), B1-S4 (mutate tools), B1-S5/S6 (B10b NULLs owed by neighbors), B1-S7 (3 missing pairwise handoffs), B1-H1 (11 remaining APQC tags + 2 deferred), B2-S1/S2/S3/S4 (judgment calls), Bucket 3 (Phase 0 pending).
- **Structural bands re-evaluated:** A pass (Rule #8 7-field metadata populated on row 93). S unauditable (no modules to scaffold). M1 hard-fail. B5/B7 unauditable (no DMDO rows, only legacy `domain_data_objects`). **B9 pass** (all 14 events now carry valid `event_category`: lifecycle/state_change/signal). B9b unauditable (intra-domain cross-module handoffs require modules). B10b 15 of 15 cross-domain handoffs carry NULL on both endpoints (cluster-wide M1 cascade across HCMS + 6 neighbors). B11/B12 unauditable. C1/C2 unauditable. D1 unauditable. E1-E5 unauditable (E-band gated on M-band). F1 pass (live aliases populated). F2 hard-fail (skill 67 orphan). F3 conditional-pass (6 query tools on skill 67). F4 pass (all 6 tools `query` + `data_object_id` populated, valid pairing). F5 uncomputable. H1 partial: 2/15 cross-domain handoffs tagged (13.3% coverage), short of 53-80% target (8-12 tags); 11 untagged + 2 explicitly deferred for no-PCF-match (816, 817). **Rule #15 clean** across data_objects, aliases, relationships, lifecycle, skill_tools, handoffs, solutions. **Rule #18 clean** on `domains.description` / `business_logic` for row 93.
- **Bucket 1 (in-scope, agent fixable):** 7 items carried (1 resolved 2026-05-31: B1-S2 event_category PATCH; H1 partially advanced).
- **Bucket 2 (surface-for-user, judgment):** 4 items carried unchanged.
- **Bucket 3 (Phase 0 pending, speculative):** 10 entity candidates + 2 modularization + 2 regulation candidates + 2 queued candidate domains, all carried.

### Resolved since 2026-05-30

| ID | Resolution | Evidence |
|---|---|---|
| B1-S2 | **CLOSED.** All 14 HCMS trigger_events (33, 34, 35, 36, 46, 886-894) carry a valid `event_category` per Rule #13 enum (`lifecycle`/`state_change`/`signal`). Verified live: events 886-894 carry the PATCH values from the 2026-05-31 continuation pass. | Live `/trigger_events?data_object_id=in.(131..137)` returns 14 rows, all with non-empty `event_category`. |
| B1-H1 (partial) | **2 of 13 candidate tags landed.** Handoff 98 (DAM → HCMS `digital_asset.published`) → `handoff_processes` row with `process_id=1763`, `proposal_source='agent_curated'`, `record_status='new'`. Handoff 806 (HCMS → DAM `content_entry.created`) → `process_id=428`, same source/status. 11 candidate tags still pending; 2 deferred (816, 817) remain deferred. | Live `/handoff_processes?handoff_id=in.(92..837)` returns 2 rows. |

### Bucket 1, In-scope confirmed gaps (carried forward)

| ID | Band | Status | Action |
|---|---|---|---|
| B1-S1 | M1 hard-fail, zero modules | Gated on B2-S4 (module-split granularity). | Author N `domain_modules` rows once user picks the split count + names; migrate `domain_data_objects` to per-module DMDO; attach scaffold (3 permissions, 3 roles, 6 FK columns). |
| B1-S3 | F2 hard-fail, orphan system skill 67 | Gated on B1-S1 + B2-S3 (1 vs N skills). | Either PATCH skill 67 `domain_module_id`, or DELETE 67 + INSERT N system skills (one per module per Rule #17) and repartition the 6 skill_tools. |
| B1-S4 | F3 / F5, missing mutate tools | Gated on B1-S1 + B1-S3. | INSERT ~8 mutate tools (`create_content_entry`, `publish_content_entry`, `schedule_content_entry`, `create_content_release`, `add_locale`, `promote_environment`, `deprecate_content_type`, `request_review`) + skill_tools. |
| B1-S5 | B10b report-only, outbound NULLs | Sequential on neighbor M1s (DXP 6 handoffs, B2C-COMM 2, MA 1, DAM 1, LSD 1, WEB-CONTOPS 1). | Schedule b1 audits on DXP, B2C-COMM, MA, DAM, LSD, WEB-CONTOPS; each neighbor's M1 fix unblocks its slice of the 12 outbound `target_domain_module_id` NULLs. |
| B1-S6 | B10b report-only, inbound NULLs | Sequential on DAM + WEB-CONTOPS M1. | Schedule b1 audits on DAM (1: 98) and WEB-CONTOPS (2: 816, 817). |
| B1-S7 | Pairwise, 3 missing candidate handoffs | Gated on B1-S1. | Author 3 `handoffs` once modules exist: HCMS → DXP on `content_release.failed` (890) with `content_releases` payload; HCMS → DXP on `content_entry.unpublished` (888) with `cdn_cache_invalidations` payload; HCMS → WEB-CONTOPS on `content_entry.published` (33) with `web_pages` payload. |
| B1-H1 | APQC tagging, 11 remaining | Independent of B1-S1; per-handoff PCF picks pending. | INSERT 11 `handoff_processes` rows for handoffs 92, 93, 94, 95, 96, 97, 803, 804, 805, 807, 837 plus catch deferral notes on 816, 817. PCF picks live in the 13.6.x branch (Manage Content 83 / Develop and manage content 428 / Deliver approved content 429); pick judgment per handoff. |

#### Bucket 1 count summary (this audit)

| Finding type | Count |
|---|---|
| MISSING | 0 (in Bucket 1; entity gaps live in Bucket 3) |
| WRONG-OWNERSHIP | 0 |
| SCOPE-CREEP | 0 |
| STRUCTURAL (M1 + F2 + F3 + B10b x 2 + Pairwise) | 6 |
| BOUNDARY | 0 net new |
| APQC TAGGING | 1 (11 remaining + 2 deferred) |
| MODULARIZATION ISSUES | 0 in Bucket 1 |
| **Bucket 1 total** | **7 items** |

### Bucket 2 (carried forward unchanged)

| ID | Question |
|---|---|
| B2-S1 | Lifecycle states authoring (entries + releases only / all 6 / defer). |
| B2-S2 | Pattern flag positive re-evaluation (per-flag yes/no on `has_submit_lock` for entries + releases, `has_personal_content` for entries, `has_single_approver` for editorial_workflows). |
| B2-S3 | Single system skill vs per-module split (gated on B2-S4). |
| B2-S4 | Module split granularity (3-module / 2-module / 4-module incl. HCMS-LOCALIZATION / 1-module Rule #14 waiver). |

### Bucket 3 (carried forward unchanged)

10 MISSING entity candidates (B3-E1 through B3-E10), 2 MODULARIZATION candidates (HCMS-LOCALIZATION as own module, HCMS-VISUAL as own module), 2 regulation candidates (GDPR, WCAG 2.x), 2 queued candidate domains (TMS, DIGITAL-PERSONALIZATION via `_missing-domains.md`).

### Cross-bucket dependencies

- **B2-S4 → B1-S1** is the master gate. B1-S3, B1-S4, B1-S7, B2-S1, B2-S2, B2-S3, and every B3-E* entity gap cascade off B1-S1.
- **B1-S5 / B1-S6** are sequential on neighbor M1 fixes (DXP, B2C-COMM, MA, DAM, LSD, WEB-CONTOPS).
- **B1-H1 (11 remaining APQC tags)** is independent of B1-S1.
- **B3 candidate domains** route via `audits/_missing-domains.md` triage, independent.

### Per-bucket prompts

**Bucket 1, fix these now?** Reply with: `all`, or a list (e.g. `H1`), or `skip`.

- **S1 (M1 hard fail)** depends on B2-S4 module-split decision.
- **S3 (F2 skill reparent)** depends on S1 + B2-S3.
- **S4 (mutate tools)** depends on S1 + S3.
- **S5 + S6 (B10b report-only)** scheduled across 6 neighbor audit waves.
- **S7 (3 candidate handoffs)** depends on S1.
- **H1 (11 remaining APQC tags + 2 deferred)** independent; load now or batch?

**Bucket 2, what's your call on each?** Per-item decisions; B2-S4 unblocks the rest.
