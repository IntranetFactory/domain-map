...
- **B10b report-only.** 10 outbound handoffs still carry `target_domain_module_id=NULL`. Owed by target domains (S2P, OMS, ERP-FIN, CSM, GRC, B2C-COMM).
...
| B1-H1 | APQC tagging residual | 3 of 11 cross-domain handoffs untagged (1054 INV to ERP-FIN on transfer.received, 1055 INV to CSM on expiry_warning, 1058 INV to CSM on warranty_activated). Each has 2 PCF candidates the user must pick from. |
