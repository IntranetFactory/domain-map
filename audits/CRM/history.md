...
| 472 | CRM-PIPELINE → FIN | `crm_opportunity.closed_won` | `crm_opportunities` | Process revenue accounting (10796 or child) | confident L3 |
...
- **FIN ↔ CRM (weight 1).** 1 outbound (472). NULL target_module_id (FIN's B10b, already known from baseline b2 audit, Phase B never run on FIN).
...
| FIN | B10b: populate `target_domain_module_id` on 472. Phase B may still be pending per the 2026-05-29 b2 baseline audit (no DMDO data). |
...
Validate b1 re-run. Five of the nine 2026-05-30 Bucket 1 items closed by the 2026-05-31 technical-fix continuation (S2, S4, S7, S8, S10). Four B1 items remain open (S1, S3, S9, H1 residual). All seven Bucket 2 items remain open. Bucket 3 unchanged.

- **Current footprint:** 5 full modules (46-50) plus 2 starters (153, 171) hosted via host junctions; 6 CRM masters (97, 98, 99, 100, 101, 102); 33 trigger_events on CRM masters; 24 lifecycle states across 5 of 6 masters (`pipeline_stages` 101 still 0 states, config-shape exemption pending B2-S2); 5 module system skills (157, 158, 159, 160, 161) only (legacy skill 41 confirmed gone); 60 skill_tools across those 5 system skills; 23 role_modules rows (no duplicates remain); 77 cross-domain handoffs plus 4 intra-domain (1191-1194).
- **Catalog-quality APQC headline:** 40 of 77 cross-domain handoffs carry `handoff_processes` rows (was 14 at last audit), split as 8 `discovery_substring`, 3 `discovery_override`, 29 `agent_curated`. **Still zero `record_status='approved'`.** Volume floor (0.5 * 77 = 38.5) just met; soft ceiling (0.8 * 77 = 61.6) still 21.6 short. Process-health side-bar: 29 `agent_curated` rows means the layered ownership is firing.
- **Bucket 1 (in-scope, agent fixable):** 4 items still PENDING (S1, S3, S9, H1 residual).
- **Bucket 2 (surface-for-user, judgment):** 7 items, all unchanged.
- **Bucket 3 (Phase 0 pending, speculative):** 13 candidates unchanged (10 entities, 2 modularization, 6 regulations).

### Structural Validate b1 bands

| Band | Status | Notes |
|---|---|---|
| A1-A3 | PASS | domain 69 carries all 7 metadata fields populated (crud 88, business_logic populated, min_org_size populated, cost_band populated, certification_required false, usa_market_size 35000, source_year 2025). 9 capabilities including cross-cutting CUSTOMER-360 (310). 25 solution_domains rows with coverage_level on every row. |
| M1, M2, M4, M5, M6 | PASS | 5 full modules, 2 starters hosted via junction. Capability CUSTOMER-360 realized by CRM-ACCT-MGT (assumed). |
| M7 (within-domain incoherence) | **HARD-FAIL** unchanged | 6 contributor rows on CRM-AI-COPILOT (50) against the canonical masters in modules 46-49 remain. B1-S1 unresolved, gated on B2-S1 architectural choice. |
| B5 (trigger_events per master) | PASS | 33 events on 6 masters, every master has at least one event. |
| B7 (relationships) | PASS | carry-over verdict, no regression observed. |
| B9 (event_category populated) | PASS | every CRM trigger_event has a non-empty `event_category` value. 462 confirmed `signal`. |
| B9b (intra-domain handoffs) | **PARTIAL-FAIL** unchanged | only 4 intra-domain rows (1191-1194 from `crm_lead.converted` and `crm_opportunity.closed_won`). B1-S3 unresolved. |
| B10b (FK populated on CRM side) | PASS for CRM side | all CRM-side `target_domain_module_id` populated on inbound; all CRM-side `source_domain_module_id` populated on outbound. 17 outbound NULL-target + 38 inbound NULL-source rows remain other-domain debt. |
| B11 (aliases) | PASS | 12 alias rows across 5 of 6 masters. |
| B12 (lifecycle + pattern flags) | **PARTIAL-FAIL** unchanged | `pipeline_stages` (101) carries 0 lifecycle states; config-shape exemption decision pending B2-S2 (Rule #15 notes wording on row 101 still polluted, 1 row). Pattern-flag re-evaluation pending B2-S4 (4 flag re-eval candidates from 2026-05-30 still open). |
| C1-C2 (handoff coverage) | PASS / PARTIAL | 77 cross-domain handoffs consistent with 2026-05-30 inventory; 14 candidate boundary handoffs from 2026-05-30 pairwise still pending other-domain audits. |
| D (handoff_processes catalog-quality) | **HARD-FAIL** | 0 of 77 at `record_status='approved'`. H1 partial progress: 40 tagged (up from 14), still 37 untagged. Volume floor just met on raw row count, but quality floor still zero. B1-H1 residual continues. |
| E1-E5 | PASS / advisory | E2 fixed (5 duplicate role_modules cleaned in 2026-05-31 continuation; current 23 rows have no duplicates). E3 PASS (no NULL interaction_level). E6 permission-bundle drift surfaced as B2-S5. |
| F1 | PASS | legacy `crm-system` skill 41 gone; 5 module system skills remain. |
| F2 | PASS | each of 5 modules has exactly 1 `skill_type='system'` skill. |
| F3 | PASS | every system skill has >=1 `skill_tools` row (157: 15; 158: 9; 159: 11; 160: 8; 161: 17). |
| F4 | PASS | every skill_tool `operation_kind` <-> `data_object_id` pairing valid (query/mutate non-null; side_effect/compute null). |
| F5 | PASS in shape | Semantius score (strict, CRM proper, 60 skill_tools rows): 51 platform / 60 total = 85%. 9 external rows on `classify_text` (158, 161), `detect_sentiment` (159, 161), `generate_text` (159, 161), `transcribe_audio` (160), `create_calendar_event` (160). |
| F7 (channel-primitive justification) | **PARTIAL-FAIL** unchanged | `send_email` (1465) and `create_calendar_event` (1466) on CRM-ACTIVITY skill 160 still carry empty notes. `send_email` (1483) and `generate_text` (1482) on CRM-AI-COPILOT skill 161 also empty. B1-S9 unresolved, gated on B2-S3. |
| H1 (APQC catalog-quality) | **HARD-FAIL** | 0 approved; raw volume floor met (40/77 = 52%, 0.5N met). B1-H1 residual: roughly 22 more `agent_curated` proposals needed to hit 0.8N soft ceiling; plus all 40 existing rows need reviewer approval before quality headline moves off zero. |

### Bucket 1 residual (still in-scope, agent fixable)

- **B1-S1** (M7 hard-fail, CRM-AI-COPILOT contributor rows). Unchanged. Still 6 rows on module 50 against data_objects (97, 98, 99, 100, 101, 102), all `role='contributor', necessity='required'`. Gated on B2-S1.
- **B1-S3** (B9b under-modeled intra-domain handoffs). Unchanged. 5 candidate rows pre-specified in 2026-05-30; (b) carries an `or` choice (event 70 vs event 160), surfaces as user pick.
- **B1-S9** (F7 channel-primitive justification, CRM-ACTIVITY skill 160 + CRM-AI-COPILOT skill 161 send_email rows). Unchanged. Gated on B2-S3 wording.
- **B1-H1 residual** (APQC tagging). 26 cross-domain handoffs still untagged (the 11 `discovery_*` rows count toward volume but not quality). The structural pass already proposed PCF candidates in 2026-05-30; deferral was on PCF id resolution. Route forward: enumerate the 26 untagged handoffs, run `/processes?process_name=ilike.*<term>*&source_framework=eq.apqc_pcf_cross_industry` per handoff, INSERT 26 more `agent_curated` rows. Separately, all 40 existing rows still need reviewer approval before catalog-quality moves off zero.

### Bucket 2 unchanged

All 7 items from the 2026-05-30 audit still open: B2-S1 (AI-COPILOT architectural choice gating B1-S1), B2-S2 (pipeline_stages 101 notes audit confirms the `notes` field still populated with the auto-loaded B12 exemption text, contradicts Rule #15), B2-S3 (F7 wording for skill 160 send_email + create_calendar_event), B2-S4 (pattern-flag re-eval), B2-S5 (permission-bundle drift), B2-S6 (REAL-ESTATE-AGENT + HVAC-SVC-MGMT starter intent), B2-S7 (CUSTOMER-360 cross-cutting promotion).

### Bucket 3 unchanged

Phase 0 verification pending on the 10 entity candidates (`sales_territories`, `sales_quotas`, `sales_forecasts`, `sales_playbooks`, `competitors`, `competitor_intelligence`, `lead_assignment_rules`, `account_hierarchies`, `partner_relationships`, `commission_records`), 2 modularization candidates (CRM-FORECAST split, CRM-MARKETING-LIST), and 6 regulation candidates (LGPD, PIPEDA, TCPA, CAN-SPAM, DPDP, US state laws).

### Cross-bucket dependencies (carryover)

- B1-S1 gated on B2-S1 (DELETE vs PROMOTE).
- B1-S9 gated on B2-S3 (channel wording or PATCH to notify_person).
- B2-S7 may be informed by B3 (the CUSTOMER-360 promotion would interact with CRM-FORECAST + CRM-MARKETING-LIST modularization).
- All other buckets independent.
...
No change versus 2026-05-30 routing table (23 owing-domain rows). The 17 outbound + 38 inbound NULL-FK rows remain CRM neighbors' B10b debt, not CRM's.
