...
- **Current footprint:** Same shape as 2026-05-30 plus the prior continuation's fixes. `domains` row 88 still intact (A1 pass: 7 business columns populated; A3 pass after em-dash backfill). 0 `domain_modules` (M1 unchanged), 0 `capabilities` linked via `capability_domains` (A2 unchanged), 0 `domain_regulations`, 12 `data_objects` masters at the domain-rollup level with the 3 stale duplicates removed (M-band confirms only `master` rollups remain for ids 232 / 252 / 254). 20 DCG-related `trigger_events` all carry `event_category` (B9 hard cured). 0 `data_object_lifecycle_states` across all 12 masters (B12 unchanged). 0 `data_object_aliases` (B5 unchanged). All 12 master `data_objects` pattern flags remain `has_personal_content=false`, `has_submit_lock=false`, `has_single_approver=false` (B7 unchanged). 4 `data_object_relationships` touch DCG masters (id 219 application_interfaces consumes data_products, 348 audit_findings reviews data_lineage_relationships, 1587 users authors metric_definitions, 1588 users certifies metric_definitions). 2 external `consumer + optional` DMDOs on DCG masters (APM-PORTFOLIO-REGISTRY on data_products 232, IGA-ACCESS-REQUEST on data_access_policies 307). 30 cross-domain handoffs touching DCG (one new since 2026-05-30: handoff 1341 BI -> DCG `bi_dashboard.published`). Only handoff 263 has a populated module FK (target_domain_module_id=144 IGA-ACCESS-REQUEST); the other 29 still NULL DCG-side. 25 of 30 cross-domain handoffs now carry a `handoff_processes` row (handoff 1341 ingests new + 4 still untagged: 683, 685, 698, 699 per the Discover Pass 3 deferral). 23 of the 25 tagged are `proposal_source='agent_curated'`; 2 still `discovery_substring` (152 was reclassified to agent_curated; 220 stayed discovery_substring per the prior continuation's "decide" deferral; 263 stayed discovery_substring; 264 stayed discovery_substring). 0 `record_status='approved'` across all 25 tags.
...
- **A1 pass.** All 7 business-meaningful columns on `domains` id 88 populated (crud_percentage=75, cost_band=$$$, min_org_size=`30 m <2500`, business_logic non-empty and em-dash-free, usa_market_size_usd_m=2500, market_size_source_year=2025, certification_required=false).
- **A2 hard-fail.** Zero `capability_domains` rows for domain_id=88. Rule #14 cascade unchanged.
- **A3 pass.** No em-dash in `domains.description` or `domains.business_logic` (B1-S8 cured 2026-05-31 continuation).
- **M1 hard-fail.** Zero `domain_modules` rows for domain_id=88. Zero rows in `domain_module_host_domains` pointing at DCG. M2-M7 unevaluable.
- **B5 advisory.** Zero `data_object_aliases` on any DCG master. Carries forward as Bucket 3 (vendor terminology mapping requires Phase 0 vendor research).
- **B7 advisory.** All 12 DCG masters carry all three pattern flags false. B4-style re-evaluation still pending user judgment (carries forward as B2-S5).
- **B9 pass.** All 20 DCG-related trigger_events carry a populated `event_category` value. 7 DCG-owned events were patched in the 2026-05-31 continuation; the other 13 belong to other domains and already had values.
- **B9b not evaluable.** Intra-domain cross-module handoffs cannot exist; no modules.
- **B10b hard-fail.** Of 30 cross-domain handoffs touching DCG, 29 have DCG-side module FK NULL (the lone exception is handoff 263 where target_domain_module_id=144 IGA-ACCESS-REQUEST). The cascade is structurally blocked on M1.
- **B11 not evaluable.** Module-level adjacency requires modules.
- **B12 hard-fail (Rule #12 violation).** Zero `data_object_lifecycle_states` rows across all 12 masters. 9 of 12 are workflow-bearing per the 2026-05-30 audit's enumeration; 3 are config-shape candidates pending B2-S4.
- **C1 pass.** Domain has 12 master data_objects.
- **C2 pass.** All 12 master data_objects exist with `kind=domain_owned`.
- **C-regulations advisory.** Zero `domain_regulations` rows; carries forward as B2-S6.
- **D1 pass.** All 12 master data_objects exist in `data_objects` (not just rollup).
- **E1-E5 not evaluable.** No modules = no module-scoped roles, role_modules, role_permissions, permissions, or permission_hierarchy to audit.
- **F1-F5 not evaluable.** No modules = no DCG-owned `skills` (zero rows where `skills.domain_module_id` resolves to a DCG module). F2-F5 unevaluable.
- **H1 partial-fail.** 25 / 30 (83.3 %) cross-domain handoffs carry a `handoff_processes` tag; 23 / 30 (76.7 %) are `proposal_source='agent_curated'` (above the 0.5N to 0.8N target). 0 / 30 (0 %) at `record_status='approved'` (catalog-quality headline). 5 still untagged: 683 (ml_model.deployed), 685 (feature_set.published), 698 / 699 (KG entity events), 1341 (bi_dashboard.published, new since prior audit). The first 4 are the audit's own Discover Pass 3 deferrals; 1341 is fresh and is the only Bucket 1 H-band item this pass.

### Bucket 1, In-scope confirmed gaps

#### STRUCTURAL band failures (carryover)

| ID | Band | Finding | Fix |
|---|---|---|---|
| B1-S1 | M1 + Rule #14 | Carryover. DCG still has zero `domain_modules` rows; gated on B2-S1 architectural choice (6-module recommendation outlined 2026-05-30). | Same loader plan; gated on B2-S1. |
| B1-S2 | A2 hard-fail | Carryover. Zero `capabilities` linked. | Gated on B2-S2. |
| B1-S4 | Rule #12 | Carryover. Zero lifecycle states on the 12 masters. | Gated on B1-S1 and B2-S4. |
| B1-S6 | B10b cascade | 29 of 30 cross-domain handoffs still NULL on DCG-side module FK (only 263 populated). | Gated on B1-S1. |
| B1-S7 | Rule #18 wording | Carryover. Handoffs 158, 223, 265 still carry the wording flagged 2026-05-30 ("the platform's native RBAC layer", "(Knowledge Graph Platform)", "Data Catalog and Governance platforms maintain..."). | Gated on B2-S3 wording. |
| B1-S9 | Pairwise consumer DMDOs | Carryover. Still only 2 external consumer DMDOs on DCG masters (APM-PORTFOLIO-REGISTRY on 232; IGA-ACCESS-REQUEST on 307); 9 neighbor domains (DI, KGP, DLP, DSPM, BI, AUDIT, MDM, METRICS-LAYER, DATA-AI-PLAT) still owe consumer DMDOs on DCG masters in their own b1 audits. | Report-only; gated on B1-S1 then routes to neighbors. |

#### APQC TAGGING

| ID | Band | Finding | Fix |
|---|---|---|---|
| B1-H1 | H-band | Handoff 1341 (BI -> DCG `bi_dashboard.published`, payload `bi_dashboards` 692) is untagged. Proposed: process_id 277 "Manage business information" (L3, external 20779), confidence confident L3 (matches the pattern used for sibling 688 BI -> DCG `bi_report.published`). 4 carryover Discover Pass 3 candidates (683, 685, 698, 699) remain deferred per the prior audit's licensed decision. | INSERT 1 `handoff_processes` row with `proposal_source='agent_curated'`, `record_status` defaulted to `new`. Surgical CLI sufficient. |
...
| WRONG-OWNERSHIP | 0 |
| SCOPE-CREEP | 0 |
| STRUCTURAL (carryover) | 6 (S1, S2, S4, S6, S7, S9) |
| BOUNDARY | 0 (subsumed by B1-S6) |
| APQC TAGGING | 1 (H1, +4 Discover Pass 3 deferrals tracked under B3) |
| MODULARIZATION ISSUES | 0 (routes to Bucket 2) |
| **Bucket 1 total** | 7 line items rolled into 6 audit IDs (S1, S2, S4, S6, S7, S9, H1) |

### Bucket 2, Surface-for-user (judgment calls)

Carryover from 2026-05-30; no Decisions captured in history yet.

| ID | Question | Why agent can't answer | Options |
|---|---|---|---|
| B2-S1 | DCG module split (6-module recommendation: DCG-CATALOG, DCG-LINEAGE, DCG-GLOSSARY, DCG-STEWARDSHIP, DCG-ACCESS-GOV, DCG-DATA-PRODUCTS). | Architectural decision; depends on whether DCG deploys as a unified governance stack with DSPM / DLP (favors fewer modules) or as a discrete catalog product (favors more). | (a) 6-module agent recommendation, (b) 3-module tight split, (c) 4-module split, (d) 7-module split, (e) other. |
| B2-S2 | Capability list (10 candidates: technical-metadata-harvesting, business-glossary-management, data-lineage, automated-data-classification, data-stewardship-workflow, data-quality-scorecards, certification-workflow, data-product-publishing, access-request-and-policy-authoring, regulatory-mapping). | Market-definition call. | (a) all 10, (b) tighter 6-7, (c) different (specify). |
| B2-S3 | Rule #18 wording on handoffs 158, 223, 265. | Rule #15 cousin: wording must be user-approved per row. | (a) approve agent-proposed rewrite text, (b) supply your own, (c) confirm leave-as-is. |
| B2-S4 | Rule #12 config-shape exemption for `data_domains` (304), `glossary_terms` (302), `data_usage_metrics` (308). | Exemption is user-decided. | Per master: (i) exempt, (ii) author lifecycle states. |
| B2-S5 | Pattern-flag re-evaluation (`data_access_policies` 307 submit_lock + single_approver; `data_stewardship_assignments` 305 personal_content; `data_certifications` 306 single_approver; `data_classifications` 303 submit_lock). | Workflow-shape judgment. | Per-flag yes / no. |
| B2-S6 | Regulation scoping (GDPR Article 30, CCPA / CPRA, HIPAA, SOX ICFR, BCBS 239, GLBA, EU AI Act, SR 11-7, US state-level privacy laws). | Market-scoping. | (a) all 9, (b) subset, (c) none, (d) defer to Phase 0. |

### Bucket 3, Phase 0 pending (speculative)

Reframed from 2026-05-30. Audit candidates consolidated for the next Phase 0 pass.

| ID | Candidate | Vendor evidence | Proposed module / disposition |
|---|---|---|---|
| B3-ENTITIES | 10 missing-entity candidates (data_contracts, data_quality_rules DCG-scoped, data_access_requests, data_marketplace_listings, lineage_pipelines, query_lineage_records, data_impact_analyses, metadata_collection_jobs, data_sensitivity_labels, data_collaboration_threads). | Alation, Atlan, Collibra, data.world, Informatica EDC + AXON, Microsoft Purview, Open Metadata, Acryl DataHub, Apache Atlas, Apache Polaris, Databricks Unity Catalog. | Per-entity disposition per 2026-05-30 inventory; final routing depends on B2-S1 module split. |
| B3-MODULARIZATION | 2 candidates: DCG-LINEAGE possibly subsumed by catalog UI integration; DCG-DATA-PRODUCTS overlaps DATA-CONTRACTS market. | Atlan / data.world / Castor (lineage-as-tab); Gable.ai / Datacontract.com / Confluent SR / PayPal DataContract CLI / Buf SR (DATA-CONTRACTS). | Feeds B2-S1 module split. |
| B3-REGULATIONS | 8 regulation rows missing (GDPR Article 30, CCPA / CPRA, HIPAA, SOX ICFR, BCBS 239, GLBA, EU AI Act, SR 11-7). | Statutory anchors per flagship-vendor compliance product surfaces. | Feeds B2-S6. |
| B3-CANDIDATE-DOMAINS | 2 new domain-tier candidates (DATA-OBSERVABILITY: Monte Carlo, Acceldata, Bigeye, Anomalo, Soda, Sifflet, Metaplane, Datafold; DATA-CONTRACTS: Gable.ai, Datacontract.com, PayPal Data Contract CLI, Confluent SR, Buf SR). | Independent point-solution markets adjacent to DCG / DQ. | Queue in `_missing-domains.md` (route via separate audits when promoted). |

### Cross-bucket dependencies

- B1-S1 gated on B2-S1; B1-S2 gated on B2-S2; B1-S4 gated on B1-S1 + B2-S4; B1-S6 gated on B1-S1; B1-S7 gated on B2-S3.
- B1-S9 routes to 9 neighbor b1 audits once B1-S1 lands.
- B3-ENTITIES + B3-MODULARIZATION feed back into B2-S1 (accepted entities change the module split shape).
- B2-S6 independent of all other items.
- 4 carryover Discover Pass 3 deferrals (handoff_processes for 683, 685, 698, 699) feed Discover Pass 3 custom-process authoring, not a DCG audit blocker.
...
_(none this pass; this audit run produced state-tracking only)_
