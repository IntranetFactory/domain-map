...
- **Current footprint:** 3 full modules (ONB-JOURNEY-MGMT 35, ONB-DOCUMENT-INTAKE 36, ONB-WELCOME-EXPERIENCE 37), no starters, no cross-domain hosts. 8 masters (onboarding_plans 15, onboarding_journeys 16, onboarding_stages 17, onboarding_tasks 18, buddy_assignments 19, onboarding_cohorts 20, welcome_communications 21, onboarding_document_collections 22). 6 capabilities (ONBOARDING-JOURNEY-DESIGN, ONBOARDING-PROVISIONING-ORCHESTRATION, ONBOARDING-DOCUMENT-COLLECTION, ONBOARDING-WELCOME-EXPERIENCE, ONBOARDING-BUDDY-MENTORING, ONBOARDING-MILESTONE-TRACKING). 23 domain_module_data_objects rows. 22 handoffs touching ONBOARDING: 5 intra-domain lifecycle_progression (1227, 1228, 1229, 1230, 1232), 13 outbound cross-domain (4, 5, 6, 7, 8, 9, 407, 408, 409, 410, 411, 1231, 1233), 4 inbound cross-domain (2, 3, 394, 1074); 17 cross-domain total. 12 trigger_events on ONBOARDING masters (all event_category=state_change). 36 lifecycle states across 8 masters; 22 carry requires_permission=true, all with empty permission_verb_override. 16 aliases on all 8 masters. 9 permissions (3 baseline-read, 3 baseline-manage, 3 baseline-admin), 0 workflow-gate. 4 ONBOARDING-touching roles (Hiring Manager 10007, Onboarding Coordinator 10011, Onboarding HR Partner 10012, Onboarding Program Manager 10013). 3 system skills (one per module, Rule #17 satisfied): onb_journey_mgmt_agent (203, 16 tools), onb_document_intake_agent (204, 6 tools), onb_welcome_experience_agent (205, 9 tools); 31 skill_tools rows. 11 handoff_processes rows across 11 of 17 cross-domain handoffs (8 discovery_override, 1 discovery_substring, 2 agent_curated; 0 record_status=approved).

- **Bucket 1 (in-scope, agent fixable):** 4 items.
- **Bucket 2 (surface-for-user, judgment):** 4 items.
- **Bucket 3 (Phase 0 pending, speculative):** 0 new (prior Bucket 3 candidates referenced by id B3-PRIOR).

### Structural pass bands

- **A1-A3 pass.** domains.crud_percentage=92, min_org_size="20 s <500", cost_band="$$", certification_required=false, usa_market_size_usd_m=500, market_size_source_year=2024, business_logic populated.
- **A4 / M8 fail.** All 3 modules carry empty catalog_tagline and empty catalog_description. Per Rule #20 these must not be authored without user-approved wording. Surfaced as B2-S4.
- **M1-M7 pass.** 3 full modules on a 6-capability domain (Rule #14 floor of >=2 satisfied). Every module has >=1 master.
- **B1 pass.** 8 masters.
- **B2 pass.** Every master carries singular_label and plural_label.
- **B3 pass.** All 8 master names are prefixed; is_canonical_bare_word=false across the board.
- **B4 partial.** Pattern flags partially populated: onboarding_plans has_single_approver=true; onboarding_journeys / onboarding_tasks / buddy_assignments / welcome_communications has_personal_content=true; onboarding_document_collections has all three flags true. Gaps surfaced as B2-S2.
- **B5 pass.** Every embedded_master row resolves to a canonical master: employees / hcm_positions / org_units -> HCM (54), locations -> IWMS (23). Consumers: candidates -> ATS (56), hardware_assets -> HAM (51). Contributors: service_requests -> ITSM (1), asset_lifecycle_events -> ITAM (3).
- **B7 pass.** 9 user (id 748 platform_builtin) edges authored on all 8 masters: onboarding_plans (maintains), onboarding_journeys (onboards, owns), onboarding_tasks (performs, created), buddy_assignments (mentors), onboarding_cohorts (manages), welcome_communications (sends), onboarding_document_collections (approves).
- **B9 pass.** All 12 trigger_events on ONBOARDING masters carry event_category=state_change.
- **B9b pass.** 5 intra-domain lifecycle_progression rows cover the multi-module cross-product (1227 ONB-JOURNEY-MGMT->ONB-DOCUMENT-INTAKE on plan.activated, 1228 ONB-JOURNEY-MGMT->ONB-WELCOME-EXPERIENCE on plan.activated, 1229 ONB-DOCUMENT-INTAKE->ONB-JOURNEY-MGMT on doc_collection.completed, 1230 ONB-WELCOME-EXPERIENCE->ONB-JOURNEY-MGMT on welcome.sent, 1232 ONB-WELCOME-EXPERIENCE->ONB-JOURNEY-MGMT on buddy.activated).
- **B10b pass.** All 17 cross-domain handoffs carry populated source_domain_module_id and target_domain_module_id. No NULL FK on either side.
- **B11 pass.** 16 aliases distributed across all 8 masters (2 per master).
- **B12 partial-fail.** 22 of 36 lifecycle states carry requires_permission=true but ALL 22 have empty permission_verb_override AND zero materialized workflow-gate permissions exist on modules 35/36/37. Same root cause as E6. Surfaced as B1-S2.
- **C1 pass.** Owner = Employee Onboarding business function; contributor = IT Operations.
- **D1 deferred.** UI spot-check not part of structural pass.
- **E1-E5 partial.** 5 roles attached via role_modules with interaction_level populated (E3 clean). Two-module floor (E2) satisfied for Hiring Manager (2), Onboarding Coordinator (2), Onboarding HR Partner (2), Onboarding Program Manager (3). E4 satisfied. E5 clean.
- **E6 hard-fail.** Zero workflow-gate permissions despite 22 requires_permission=true lifecycle states. Surfaced as B1-S2.
- **F1 pass.** No legacy domain-level system skills.
- **F2 pass.** Every domain_modules row has exactly one skill_type=system skill.
- **F3 pass.** 16 / 6 / 9 skill_tools rows per skill.
- **F4 pass.** Tool operation_kind invariants hold across all 31 skill_tools rows.
- **F5 pass.** Semantius score computable. Strict: 27 of 31 skill_tools on coverage_tier=platform (~87%). External: notify_team 914 (twice), sign_document 42 (once), one notify_team external instance counted.

### Bucket 1, in-scope confirmed gaps

| ID | Band | Finding | Fix |
|---|---|---|---|
| B1-S1 | **Rule #15 pollution carry-over: 6 domain_data_objects rows + 16 skill_tools rows + 9 handoffs rows** | Prior audit (2026-05-30) flagged 6 DDO rows (51 employees, 92 service_requests, 98 asset_lifecycle_events, 879 hcm_positions, 880 org_units, 881 hardware_assets) and 16 skill_tools rows. Live re-check confirms all 22 still polluted. New finding: handoffs notes populated on row 9 ("B10b resolved 2026-05-26..."), row 1074 ("cross | ATS->ONBOARDING | ..." cluster-draft prefix), and intra-domain rows 1227, 1228, 1229, 1230, 1231, 1232, 1233 (all carry sentence-shaped commentary). Per Rule #15 every notes column is empty by default. | Surface per-row to user (B2-S1). Default on auto-populated: PATCH all 31 rows notes to empty string and log Rule #15 incident. |
| B1-S2 | **B12 + E6 hard-fail, materialize workflow-gate permissions for 22 lifecycle states** | 22 of 36 lifecycle states across all 8 masters carry requires_permission=true with empty permission_verb_override. Zero workflow-gate permissions exist on modules 35/36/37. Per Rule #12 each warrants a <module_code>:<verb>_<entity> permission. | Blocked by B2-S3 (per-state verb authoring requires user judgment to avoid the ATS B12 stale-permission bug pattern). Once verbs approved, INSERT 22 workflow-gate permissions + role_permission mappings via focused loader. |
| B1-S3 | **H1 hard-fail, 6 untagged + 1 wrong + 3 coarse APQC tags** | Untagged: handoff 2 (ATS->ONB job_offer.accepted), 410 (ONB->HCM doc_collection.completed), 411 (ONB->PAYROLL doc_collection.completed), 1074 (ATS->ONB job_offer.rescinded), 1231 (ONB->EMP-EXP stage.completed), 1233 (ONB->LMS cohort.activated). Wrong: handoff 7 (ONB->PAYROLL journey.day_one_reached) tagged to PCF 1814 "Create customer journey maps" L5, a CRM-mapping process unrelated to HR onboarding (substring matcher hit "journey" without HR context). Coarse: handoff 3 (HCM->ONB employee.created) tagged to PCF 41 L2 (20599) instead of L3/L4; handoff 394 (ATS->ONB candidate.hired) tagged to "Recruit/Source candidates" PCF 220 (10440) which is upstream of the receiving-side onboarding activity. Target PCFs to investigate at fix time: Manage employee onboarding (224, 10469), Provide workspace and facilities (345), Manage IT user identity and authorization (273), Develop, conduct, and manage employee training programs (1039), Administer Payroll (236), Conduct employee engagement surveys (250), Manage employee inquiry process (242, 10523, already mapped to h9). | Per-handoff PCF classification pass: INSERT 6 new agent_curated rows for untagged handoffs; REPLACE 4 rows (h7 wrong, h3 / h394 coarse, h8 review-confirm). Total candidates: 10 rows after this audit; 2 already agent_curated (h5, h409). Reaches ~12 agent_curated post-fix, within H1 volume target (0.5N-0.8N = 9-14 for N=17). |
| B1-S4 | **B10 report-only, consumer DMDOs on downstream domains** | ONBOARDING-outbound handoffs imply consumer DMDOs on target side that may not exist. Pattern: task.* events to ITSM (4, 408), IGA (5, 407), IWMS (6), HRSD (9); journey.day_one_reached to PAYROLL (7), EMP-EXP (409); cohort.activated to LMS (1233); stage.completed to EMP-EXP (1231); doc_collection.completed to HCM (410), PAYROLL (411). | Schedule b1 audits on ITSM / IGA / HRSD / IWMS / LMS / PAYROLL / EMP-EXP so they declare the consumer DMDOs. Not ONBOARDING fix. HCM b1 (2026-05-30) already covered. |
...
| WRONG-OWNERSHIP | 0 |
| SCOPE-CREEP | 0 |
| STRUCTURAL (B12+E6 workflow-gate permissions) | 1 (B1-S2) |
| BOUNDARY (NULL FK or missing handoff) | 0 (B10b clean) |
| Rule #15 incident (notes pollution) | 1 (B1-S1, 31 rows across 3 tables; surfaced via B2-S1) |
| **APQC TAGGING** | 1 (B1-S3, 10 rows: 6 INSERT + 4 REPLACE) |
| Report-only follow-ups | 1 (B1-S4) |
| **Bucket 1 total** | **4 line items** |

### Bucket 2, surface-for-user (judgment calls)

| ID | Question | Why agent cannot answer | Options |
|---|---|---|---|
| B2-S1 | **Rule #15 notes pollution: 31 rows across domain_data_objects (6), skill_tools (16), handoffs (9).** Were these user-approved at load time, or auto-populated? | Load-time approval status not recoverable. Carry-over from 2026-05-30 audit's B2-S1 / B2-S2; no resolution recorded in Continuation. | (a) Confirm user-approved; leave in place. (b) Confirm auto-populated; PATCH all 31 rows notes to empty string and log incident in references/skill-changelog.md. (c) Per-row decision. |
| B2-S2 | **B4 pattern-flag positive re-evaluation.** Proposed flips: onboarding_journeys.has_submit_lock=true (state machine append-only after in_progress), onboarding_journeys.has_single_approver=true (HR Partner signs completion). onboarding_tasks.has_submit_lock=true (completed terminal). | Workflow-shape judgment user owns. | Per-flag yes/no. |
| B2-S3 | **Workflow-gate permission verbs for 22 lifecycle states.** Per-module candidate verbs: ONB-JOURNEY-MGMT (12) approve_onboarding_plan, activate_onboarding_plan, retire_onboarding_plan, complete_onboarding_journey, cancel_onboarding_journey, complete_onboarding_stage, skip_onboarding_stage, complete_onboarding_task, skip_onboarding_task, cancel_onboarding_task, activate_onboarding_cohort, complete_onboarding_cohort, cancel_onboarding_cohort; ONB-WELCOME-EXPERIENCE (6) activate_buddy_assignment, complete_buddy_assignment, cancel_buddy_assignment, mark_welcome_sent, mark_welcome_delivered, mark_welcome_failed, cancel_welcome_communication; ONB-DOCUMENT-INTAKE (4) submit_onboarding_document_collection, approve_onboarding_document_collection, reject_onboarding_document_collection, archive_onboarding_document_collection. User picks which to materialize and which to collapse to baseline tier. | Editorial verb authoring; cannot be invented by agent per ATS B12 lesson. | (a) Materialize all 22-23 explicit verbs. (b) Collapse subset to baseline-manage / baseline-admin; only approval gates (plan.approved, document_collection.approved / rejected, journey.completed) get explicit verbs. (c) Per-state decision. |
| B2-S4 | **M8 / A4, populate catalog_tagline and catalog_description on modules 35, 36, 37.** All three carry empty values. Per Rule #20 these are buyer-voice fields that must not be authored without explicit user-approved wording. | Marketing copy is content the user owns. | (a) User supplies wording per module. (b) Defer to marketing pass; M8 stays open. |

### Bucket 3, Phase 0 pending

No new candidates this run. Prior 2026-05-30 audit Bucket 3 entries remain unresolved and are referenced by id B3-PRIOR (onboarding_documents per-doc records, sub-typed provisioning_tasks, onboarding_check_ins / pulse_check_ins, manager_actions / manager_tasks, background_check_records, equipment_orders, onboarding_buddies_program; compliance regulation candidates W-4 / W-2, OSHA safety training, state-specific new-hire reporting).

### Per-bucket prompts

- **Bucket 1:** Reply "all" / "just S2, S3" / "skip". S2 blocked by B2-S3 verbs. S3 (APQC) directly applicable once user approves.
- **Bucket 2:** Reply per item. For B2-S1 must NOT proceed without explicit per-row approval or blanket "revert all to empty string". For B2-S4 must NOT author taglines without user-supplied text (Rule #20).
- **Bucket 3:** Eyeball-mode (name which B3-PRIOR candidates ring true) or formal Phase 0 vendor research.

### Cross-bucket dependencies

- B1-S2 blocked by B2-S3 (per-state verb decision before workflow-gate permission INSERTs).
- B1-S1 independent of B1-S2 / B1-S3.
- B2-S4 independent of all other items.
