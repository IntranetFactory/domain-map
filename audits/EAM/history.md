...
| B1-S7 | B9 | Six trigger_events exist on EAM masters; only 3 have `handoffs` rows. Untagged events: `industrial_asset.commissioned` (962), `industrial_asset.retired` (963), `equipment_pm_schedule.updated` (966). | Confirm whether each is a genuine leaf or has at least one cross-domain subscriber: `industrial_asset.commissioned` likely fires to FIN `fixed_assets` capitalization + GRC compliance_obligations; `industrial_asset.retired` likely fires to FIN disposal + GRC; `equipment_pm_schedule.updated` is plausibly intra-domain only (config change). Draft 4 handoffs (2 per asset event x 2 targets), defer schedule.updated as intra-domain (intra-module handoff under B9b once modules ship). |
...
- Current footprint: 3 masters (`industrial_assets` 475, `eam_work_orders` 476, `equipment_pm_schedules` 477), 2 embedded_masters (`org_units`, `locations`), 0 `domain_modules`, 0 `capabilities`, 5 solutions (4 primary, 1 secondary), 0 regulations, 6 `trigger_events` on the 3 masters, 3 outbound + 5 inbound cross-domain handoffs (8 total), 0 EAM-anchored roles, 1 legacy domain-level system skill (`eam-system` id 53) with 4 skill_tools (3 query, 1 send_email). `catalog_tagline` and `catalog_description` still empty.
- This is a structural-pass-only Validate b1 audit; the semantic market pass was already run on 2026-05-30 and remains the basis for missing-entity findings.
- Bucket 1 (in-scope, agent fixable): 7 STRUCTURAL + 2 BOUNDARY + 1 APQC TAGGING = 10 items.
- Bucket 2 (surface-for-user, judgment): 5 items (carried forward from 2026-05-30 verbatim, all still open).
- Bucket 3 (Phase 0 pending, speculative): 1 consolidated candidate list (carried forward).
- next_action_by: agent (b1a non-empty, blocked on Bucket 2 #1 module-split decision for full execution).

### S-band coverage snapshot

| Table | FK column | EAM rows | Expected non-zero? | Routes to |
| --- | --- | --- | --- | --- |
| business_function_domains | domain_id | 2 | yes | C1 pass |
| capability_domains | domain_id | 0 | yes | A2 fail |
| domain_data_objects | domain_id | 5 | yes | B1 pass |
| domain_modules | domain_id | 0 | yes | M1 fail (structural gate) |
| domain_module_host_domains | domain_id | 0 | optional | n/a |
| domain_regulations | domain_id | 0 | usually yes | A1-regs fail (carried as B2 #5 industry-scope judgment + B1-S4 deferred) |
| solution_domains | domain_id | 5 | yes | A3 pass |
| skills | domain_id | 1 (legacy) | yes | F1 fail / F2 blocked |
| handoffs.source_domain_id | | 3 | yes (non-leaf) | B9 partial |
| handoffs.target_domain_id | | 5 | yes (non-leaf) | inbound coverage |

S2 (per-module DMDO/capabilities): vacuously empty (no modules).
S3 (per-master indirect): lifecycle_states 0 across all 3 masters (B12 fail); trigger_events 2/2/2 (B9 partial); aliases 3/3/2 (B11 pass).
...
| ID | Band | Finding | Fix |
|---|---|---|---|
| B1-S1 | M1 | Zero `domain_modules` rows for EAM. Catastrophic structural gate failure. | Author 2+ full `module_kind='full'` modules per Bucket 2 #1 decision (recommended `EAM-ASSETS` + `EAM-MAINTENANCE-OPS`). |
| B1-S2 | A2 | Zero `capability_domains` rows. | Load 6 capabilities, link via `capability_domains` and `domain_module_capabilities` once modules ship. |
| B1-S3 | A4 | `catalog_tagline` and `catalog_description` both empty (Rule #20). | Draft buyer-voice strings, surface to user for review BEFORE writing per Rule #20. |
| B1-S4 | A1 / regulations | Zero `domain_regulations` rows. ISO 55000, OSHA 29 CFR 1910.147 (LOTO), OSHA Process Safety Management, FDA 21 CFR Part 11 (if pharma/biotech) are all material. | Add 3-4 `regulations` rows + `domain_regulations` links. Industry scope is a B2 question. |
| B1-S5 | F1 / F2 | Legacy `eam-system` skill (id 53) sits at `domain_id=53`, `domain_module_id=NULL`. F1 fails once any module-level skill exists. F2 (one system skill per module) is fully blocked by M1. | Once B1-S1 modules ship: DELETE legacy skill 53, author `eam_assets_agent` + `eam_maintenance_ops_agent`. Re-link 4 existing skill_tools to the right module-skill. |
| B1-S6 | B12 | Zero `data_object_lifecycle_states` rows on any of the 3 masters. `eam_work_orders` and `industrial_assets` both have observable workflows; `equipment_pm_schedules` is config-shape candidate per Rule #12 exemption. | Author state machines per the 2026-05-30 audit's proposed shapes; surface PM schedules exemption to user for explicit confirmation (Rule #15 forbids notes auto-write). |
| B1-S7 | B9 / B7 | 3 of 6 trigger_events on EAM masters have no `handoffs` row (962 industrial_asset.commissioned, 963 industrial_asset.retired, 966 equipment_pm_schedule.updated). Separately, zero `users` edges on any of the 3 masters (B7 fail). | Author 4 cross-domain handoffs for asset.commissioned / .retired (FIN fixed_assets, GRC compliance_obligations). Defer schedule.updated as intra-domain pending modularization. Author `users` edges for at least asset owner, work-order assignee, PM schedule maintainer (Rule #10). |

#### BOUNDARY

| ID | Finding | Fix |
|---|---|---|
| B1-B1 | B10b, all 3 outbound handoffs (866, 867, 868) have NULL `source_domain_module_id`. Cause: M1. | After B1-S1 modules ship, PATCH `source_domain_module_id` per the role-strength resolution rule. |
| B1-B2 | B10b, 5 inbound handoffs (294, 320, 949, 953, 1260) have NULL `target_domain_module_id`. Cause: M1 plus missing receiving DMDO rows. | After B1-S1 modules + B1-S2 consumer DMDOs on the 5 payloads ship, PATCH `target_domain_module_id`. |

#### APQC TAGGING

H1 coverage now stands at 7/8 cross-domain handoffs carrying an `agent_curated` tag (866, 867, 868, 949 x2, 1260 x2, 320, 294 has both discovery_substring + agent_curated 1412). One handoff still has only a `discovery_substring` tag with no agent_curated companion.

| ID | handoff_id | source -> target | trigger_event | payload | Proposed PCF row | PCF id | confidence |
|---|---|---|---|---|---|---|---|
| B1-H1 | 953 | MFG-OPS -> EAM | production_schedule.published | production_schedules | Schedule maintenance work | (best fit, L4) | confident L4 |

Coverage headline: 0 `record_status='approved'` rows on EAM cross-domain handoffs; 12 rows present across the 8 handoffs (mix of agent_curated + discovery_substring), all `new`. Reviewer approval is the next H-band lift, not new tagging.

### Bucket 2, Surface-for-user (judgment calls)

Carried forward verbatim from 2026-05-30 Bucket 2 (none answered yet):

1. **Module split shape.** 2-module (`EAM-ASSETS` + `EAM-MAINTENANCE-OPS`) vs 3-module (add `EAM-PM-PLANNING`).
2. **M7 cross-catalog watch on `eam_work_orders escalates_to service_incidents` (rel 636).** Keep or downgrade?
3. **Bare-word claim on `work_orders`.** Does any of EAM / FLEET-MAINT / REAL-EST / FSM own the unprefixed name (Rule #9)?
4. **`fixed_assets` embedding direction.** EAM-ASSETS embedded_master `fixed_assets`, or stay decoupled to FIN?
5. **Three queued candidate domains** (`APM-RELIABILITY`, `IIOT-PLATFORM`, `PTW-LOTO`), promote vs fold for each.

All independent of each other except #1 gating Bucket 1 cascade.

### Bucket 3, Phase 0 pending

Carried forward as one consolidated item: 16 candidate entities (`condition_monitoring_readings`, `failure_codes`, FMEA, `asset_warranties`, `vendor_service_contracts`, `technician_certifications`, `crews`, `inspection_checklists`, `oil_samples`, `regulatory_compliance_logs`, `as_built_drawings`, `asset_meter_groups`, `pm_task_libraries`, `failure_root_causes`, `mro_parts`, `tool_crib_inventory`) drawn from cross-vendor recall (Maximo, Hexagon, IFS, SAP PM, AVEVA APM). Choose vetted Phase 0 vs eyeball-mode.

### Cross-bucket dependencies

- B1-S1 / S2 / S5 + B1-B1 / B1-B2 are blocked on Bucket 2 #1 module-split decision.
- B1-S6 PM-schedules exemption is blocked on Bucket 2 #1 (state attribution) + Rule #15 user wording.
- B1-S3 catalog UX is Rule #20 user review only.
- B1-S4 regulation scope is partly Bucket 2 #5 (PTW-LOTO promotion).
- Bucket 3 vetting is independent.
...
_(none, awaiting user)_

### Fixes applied

_(none, read-only audit pass)_

### `domains.notes` pointer (if updated)

_(none, requires Rule #15 user-approved wording)_
...
- **B2-FIXED-ASSETS**: embedding direction for `fixed_assets` (embedded_master in EAM-ASSETS vs decoupled to FIN). Catalog-wide, safe to defer.
