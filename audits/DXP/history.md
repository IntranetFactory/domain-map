...
- **Current footprint:** **0 `domain_modules` rows** (M1 still hard-fail). 7 masters (`web_pages`, `content_components`, `digital_experiences`, `personalization_rules`, `ab_tests`, `segments_dxp`, `journey_steps`) in legacy `domain_data_objects` only. 6 capabilities (DXP-WCM, DXP-PERSONALIZE, DXP-PORTAL, DXP-AB-TEST, DXP-MULTISITE, DXP-FORM-JOURNEY). 12 solutions (9 primary + 3 secondary). 2 regulations (ADA, Section 508). 10 `trigger_events` all with `event_category` set (8 `state_change`, 2 `signal`) per 2026-05-31 continuation. 8 outbound + 11 inbound cross-domain handoffs (19 total). 8 `users` edges per 2026-05-31 continuation covering all 7 masters. 0 intra-domain master-to-master `data_object_relationships`. 0 outbound cross-domain `data_object_relationships`. 0 `data_object_lifecycle_states`. 0 `data_object_aliases`. 0 pattern flags flipped on any master. 1 legacy domain-level system skill (`dxp-system`, id 52) with 7 query-only `skill_tools`; `domain_module_id=null`. 0 roles. 2 of 19 cross-domain handoffs carry `handoff_processes` rows (handoffs 1009, 1011, both `agent_curated` + `record_status=new`, authored on PROD-MGMT's side); 0 are `approved`.
- **Bucket 1 (in-scope, agent fixable):** 9 items.
- **Bucket 2 (surface-for-user, judgment):** 4 items.
- **Bucket 3 (Phase 0 pending, speculative):** 7 items.

Net change vs 2026-05-30 audit: B1-S4 (users edges) and B1-S7 (event_category enum) resolved by the 2026-05-31 continuation; everything else remains pending in the same shape. The structural blocker is unchanged: **0 modules** means M1 / M2 / M4 / M5 / M6 / M8 hard-fail and cascade into F1 / F2 / F3 / F5 (uncomputable Semantius score) and B10b (every cross-domain handoff has NULL `target_domain_module_id` on DXP's side).

### Structural pass bands

- **A1:** pass (all 7 metadata fields populated). **A2:** pass (6 capabilities). **A3:** pass (12 solutions, 9 primary). **A4:** hard-fail (`catalog_tagline` and `catalog_description` both empty; routes to Bucket 2 since Rule #20 requires explicit user approval).
- **M1 / M2 / M4 / M5 / M6 / M8:** hard-fail (zero modules). **M7:** vacuously passes (no DMDO rows exist; legacy `domain_data_objects` rollup carries masters and the single-master rule is satisfied as a side effect).
- **B1 / B2 / B3:** pass via legacy rollup. **B4:** hard-fail (no positive re-evaluation; all flags still false-by-default across 7 masters). **B5:** pass (no embedded_master rows; no orphans). **B6:** hard-fail (zero intra-domain master-to-master relationships). **B7:** pass (8 `users` edges authored, all 7 masters covered, `web_pages` has both author + approver). **B8:** hard-fail (zero outbound cross-domain payload-target relationships mirroring the 8 outbound handoffs). **B9:** partial pass (10 events with correct categories; coverage gap on workflow-gate state events that depend on B1-S2 / B12). **B9b:** vacuously passes (zero modules to host intra-domain handoffs; condition cannot be checked). **B10b:** hard-fail (every outbound handoff has NULL `source_domain_module_id`, cascade of M1; every inbound also has NULL `target_domain_module_id` on the DXP side). **B11:** hard-fail (zero aliases). **B12:** hard-fail (zero lifecycle states for all 7 masters).
- **C1:** pass (Marketing owner + Software Engineering contributor). **C2:** vacuously passes.
- **D1:** spot-check skipped this run (no UI write since the prior audit's continuation).
- **E1 / E2 / E3 / E4 / E5:** vacuously pass (single-module-equivalent shape because there are zero modules; no roles authored).
- **F1:** fails (legacy `dxp-system` row remains; will become a violation once a module-anchored skill is authored). **F2:** hard-fail (no module-anchored system skills). **F3:** hard-fail (no module-anchored skill_tools rows; the legacy skill's 7 tools are query-only with zero mutate or workflow-gate tools). **F4:** pass on the 7 existing tools (all query with `data_object_id` set per Rule #17 invariant). **F5:** uncomputable (cascades from F2).
- **H1:** hard-fail. 2 of 19 cross-domain handoffs have any `handoff_processes` row (10.5% coverage, zero approved). The 2 covered rows are PROD-MGMT-side (handoffs 1009 `product_release.shipped`, 1011 `product_roadmap.published`) and were not authored by DXP's pass.

Semantius score is **still uncomputable** for DXP because F2 fails.

### Bucket 1, In-scope confirmed gaps

#### STRUCTURAL band failures (still pending)

| ID | Band | Finding | Fix |
|---|---|---|---|
| B1-S1 | M1 / M2 / M4 / M5 / M6 / M8 hard fail | Zero `domain_modules` rows for DXP. 6 capabilities so the >=3 -> >=2 modules floor applies. Cascade: M5 cannot anchor lifecycle states, M6 leaves every capability orphaned, M8 has no rows to PATCH, F2 / F3 / F5 are blocked. | Resolve B2-1 first; then author 2-4 `domain_modules` rows + capability junction rows + per-master DMDO rows. |
| B1-S2 | B12 hard fail | Zero `data_object_lifecycle_states` for all 7 masters; each has an obvious workflow ladder. | Author lifecycle state rows after B1-S1 lands so `domain_module_id` can be set on workflow gates. |
| B1-S3 | B4 hard fail | Pattern flags still false-by-default on all 7 masters (no positive re-evaluation). | PATCH per B2-2 answers; no `notes` annotation (Rule #15). |
| B1-S5 | B6 hard fail | Zero intra-domain master-to-master `data_object_relationships`. Expected edges: `web_pages embeds content_components`; `digital_experiences spans web_pages`; `personalization_rules targets segments_dxp`; `ab_tests runs_on web_pages`; `ab_tests evaluates personalization_rules`; `journey_steps composes digital_experiences`; `journey_steps references web_pages`. | Author 6-8 relationship rows after B1-S1 and B1-S2 land. |
| B1-S6 | B11 hard fail | Zero `data_object_aliases` across 7 masters. Vendor terminology rendering blocked. | Author 14-20 alias rows after B1-S1. |
| B1-S8 | B9 coverage gap | Workflow-gate state transitions (`web_page.archived`, `web_page.draft`, `ab_test.archived`, `personalization_rule.paused`, `personalization_rule.retired`, `segment_dxp.retired`, `digital_experience.ended`) will lack matching `trigger_events` rows once lifecycle states land. | Insert 5-7 additional `trigger_events` rows after B1-S2 lands. |
| B1-S9 | B8 hard fail | Zero outbound cross-domain `data_object_relationships` from DXP masters; 8 outbound handoffs imply 4-6 loadable rows where target masters exist. | Surface to user; load the 4-6 where target masters already exist (CDP, MA, WEB-CONTOPS where applicable); defer the rest. |
| B1-S10 | F1 / F2 / F3 hard fail | Legacy `dxp-system` skill only (7 query tools, zero mutate, zero workflow-gate). No module-anchored skill. | After B1-S1, author one system skill per new module with 4-8 tools per skill mixing query / mutate / side_effect / inbound. Then DELETE legacy `dxp-system`. |
| B1-S11 | H1 hard fail | 2 of 19 cross-domain handoffs carry `handoff_processes` rows (handoffs 1009, 1011, agent_curated, new status). 0 approved. Volume expectation: 10-15 `agent_curated` tags for the remaining 17 untagged handoffs. | Author per the PCF lookup table from the 2026-05-30 audit's B1-S11 sub-section; verify each via `/processes?process_name=ilike.*<term>*&source_framework=eq.apqc_pcf_cross_industry`; insert with `proposal_source='agent_curated'`, `record_status='new'`. |
...
| ID | Question | Why agent can't answer | Options |
|---|---|---|---|
| B2-1 | **Module split shape** (carried from 2026-05-30). Alternatives: (a) 4-module split (DXP-AUTHORING / DXP-PERSONALIZATION-EXPERIMENTATION / DXP-PORTAL / DXP-FORMS-JOURNEYS); (b) 2-module split (DXP-AUTHORING + DXP-AUDIENCE-OPS); (c) 3-module split (DXP-WCM / DXP-PERSONALIZE-EXPERIMENT / DXP-PORTAL with forms / journeys folded in); (d) defer until Phase 0 ratifies the surface. | Module split is a design call the user owns; depends on Bucket 3 resolutions (whether portal and personalization spin out as separate domains). | (a) / (b) / (c) / (d), user picks. |
| B2-2 | **Pattern flag positive re-evaluation** (carried). Per-flag yes / no on: `web_pages.has_submit_lock`; `ab_tests.has_submit_lock`; `personalization_rules.has_single_approver`; `journey_steps.has_personal_content`; `digital_experiences.has_submit_lock`; `segments_dxp.has_submit_lock`; `content_components.has_submit_lock`. | Pattern-flag judgments are workflow-shape decisions the user owns; false-by-default doesn't establish review. | Per-flag yes / no from user. |
| B2-3 | **A4 / Rule #20 catalog UX fields.** `catalog_tagline` and `catalog_description` are both empty on domain 77. Rule #20 forbids the agent from authoring these without explicit user approval of the wording. | Marketing voice + per-row approval required per Rule #20. | (a) supply exact wording for both, (b) ask agent for a buyer-voice draft to approve / rewrite, (c) defer. |
| B2-4 | **`segments_dxp` naming arbitration** (carried). Future DIGITAL-PERSONALIZATION promotion may relocate the canonical segment master. | Future-domain promotion timing is the user's call. | (a) leave as-is, (b) rename to `personalization_segments`, (c) wait on DIGITAL-PERSONALIZATION. |

### Bucket 3, Phase 0 pending (speculative)

Carried verbatim from 2026-05-30; no new Phase 0 work has landed.

| ID | Finding | Type | Vendor knowledge basis |
|---|---|---|---|
| B3-1 | `site_definitions` (multi-site / multi-brand master backing DXP-MULTISITE) | MISSING | All 7 flagship vendors (AEM Sites, Sitecore, Drupal, Optimizely, Liferay, Salesforce Experience Cloud, Bloomreach) |
| B3-2 | `page_templates`, `layouts`, `content_blocks` (authoring substrate beyond `web_pages`) | MISSING (3 entities) | All 7 vendors |
| B3-3 | Experiment / Variant / Result split (current `ab_tests` collapses all 3) | MISSING (2 entities) | Optimizely, Adobe Target, Sitecore Personalize, AB Tasty |
| B3-4 | `forms`, `form_fields`, `form_submissions` backing DXP-FORM-JOURNEY | MISSING (3 entities) | AEM Forms, Sitecore Forms, Drupal Webform |
| B3-5 | `portal_pages`, `portal_widgets`, `portal_user_profiles` backing DXP-PORTAL (or PORTAL-FRAMEWORK promotion) | MISSING (3-5 entities) | Liferay, Salesforce Experience Cloud, IBM HCL Digital Experience, Backbase |
| B3-6 | Module split needs Phase 0 ratification (informs B2-1) | MODULARIZATION | All flagship vendors |
| B3-7 | `segments_dxp` / `personalization_rules` may belong to a future DIGITAL-PERSONALIZATION domain | WRONG-OWNERSHIP candidate | Adobe Target, Sitecore Personalize, Optimizely Personalization point solutions |

### Cross-bucket dependencies (unchanged)

- B1-S1 gates B1-S2 / S5 / S6 / S8 / S10 and is itself shaped by B2-1.
- B1-S3 depends on B2-2; independent of B2-1.
- B1-S9 and B1-S11 are independent of B1-S1; both can ship in parallel.
- B3-5 / B3-7 (Bucket 3) inform B2-1; recommend running Phase 0 before committing to the module split.

### Per-bucket prompts

- **Bucket 1, fix these now?** Reply `all`, list (e.g. `S9, S11`), or `skip`. S9 and S11 can ship without resolving B2-1 / B2-2; everything else is gated.
- **Bucket 2, what's your call on each?** I will wait for per-item decisions. B2-3 (Rule #20) needs your exact wording; I will not draft `catalog_tagline` / `catalog_description` without explicit per-row approval.
- **Bucket 3, vet via formal Phase 0 vendor research or eyeball-mode?** B3-1 (`site_definitions`) and B3-2 (`page_templates` / `layouts` / `content_blocks`) are the highest-leverage clusters and land in DXP-AUTHORING regardless of which split (a / b / c) wins. Recommend formal Phase 0 before committing to B2-1.
...
| Owed by | Check | Detail |
|---|---|---|
| HCMS | B10b `source_domain_module_id` | 6 inbound HCMS->DXP handoffs (92, 95, 97, 803, 804, 805) NULL on source side. |
| WEB-CONTOPS | B10b `source_domain_module_id` | 2 inbound WCO->DXP handoffs (815, 820) NULL on source side. |
| PROD-MGMT | B10b `target_domain_module_id` | Only the DXP-target NULL remains; cannot be fixed until B1-S1 lands. |
| MA | B8 + B10b | MA owes consumer DMDOs on `digital_experiences`, `journey_steps` and `target_domain_module_id` on outbound DXP handoffs 808, 811. |
| CDP | B8 + B10b | CDP owes consumer DMDOs on `segments_dxp`, `journey_steps` and `target_domain_module_id` on 809, 810. |
| B2C-COMM | B8 + B10b | B2C-COMM owes consumer DMDO on `personalization_rules` and `target_domain_module_id` on 812. |
| WEB-CONTOPS | B8 + B10b | WCO owes consumer DMDO on `web_pages` and `target_domain_module_id` on 814, 838. |

UI for spot-checking when fixes land:
