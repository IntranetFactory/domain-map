...
3. **Phase B — Data-object footprint.** Loads `data_objects`, `domain_module_data_objects`, `handoffs`, `data_object_relationships`, `data_object_aliases`, `data_object_lifecycle_states`. Every Phase 0 matrix entity loads or carries a one-line skip justification. Naming-collision check against existing domain entities per Rule #9. **When the analyst knows the APQC PCF activity a cross-domain handoff implements, they also draft a `handoff_processes` row** (all rows `record_status='new'` per Rule #1; user approves in review). PCF lookup is opt-in: untagged handoffs are fine, Discover's substring matcher will fill the gap later. The cost of asking authors to look up PCF rows for every handoff is real friction; the value of capturing the analyst's context when they happen to know it is high — so the workflow is *capture-when-you-know*, not *mandatory*. User reviews draft.
