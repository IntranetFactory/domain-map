...
4. **Pairwise reconciliation with each neighbor — TWO phases; only Phase 2 decides reconciled.** For each neighbor surfaced in pass 3, walk the boundary in two phases. **Phase 1 (structural gate; NECESSARY, NOT reconciliation):** the four legs (producer master + lifecycle, trigger event, handoff with both module FKs, consumer DMDO) and the structural diff categories (existing-fully-wired, NULL-module-FK, missing-handoff-implied, boundary-integrity, relationship-mirror). Passing Phase 1 only makes the boundary *eligible* to be reconciled. **Phase 2 (semantic reconciliation; THE verdict):** classify every handoff's APQC payload against the realized set (a process that is gated via `data_object_lifecycle_states.process_id` AND carries `process_raci`), hierarchy- and category-aware, as RESOLVED / ROLL-UP / ORPHAN / MIS-TAG. A boundary is RECONCILED only when every payload is RESOLVED, every ROLL-UP re-point is approved, and every ORPHAN/MIS-TAG is closed; otherwise it is RECONCILABLE, not reconciled. A boundary that is fully wired (Phase 1 green) but carries an unrealized, mis-categorized, or wrong-grain payload is NOT reconciled. User approves which neighbors to deep-dive (default: all with edge weight ≥3); lighter neighbors get a condensed one-line-per-payload Phase-2 verdict, but Phase 2 still runs on every payload. A structure-only boundary result is never acceptable.
...
| **RECONCILIATION: Phase-2 payload verdict** | Per handoff payload (Pairwise reconciliation Phase 2 / band B9d): **ROLL-UP** → re-point `handoff_processes.process_id` to the realized child/parent code (`record_status='new'`). **ORPHAN** → realize on the owning domain (gate `process_id` + `process_raci`) or report `owed by <owner>`. **MIS-TAG** → re-point or delete the `handoff_processes` row on sign-off. A boundary with any open ORPHAN/MIS-TAG payload is not reconciled. |
...
After fixes load, **re-run Validate**. Acceptance: zero MISSING / WRONG-OWNERSHIP / SCOPE-CREEP findings, zero structural failures (modulo documented exceptions in B1, B12), no NULL-FK handoffs on this domain's side, **AND every neighbor boundary RECONCILED in the Phase-2 sense (every payload RESOLVED, every ROLL-UP re-point approved, zero ORPHAN/MIS-TAG). A structurally-green boundary that still carries an ORPHAN or MIS-TAG payload does NOT pass.** MODULARIZATION findings may legitimately remain pending refactor design — they don't block Validate from passing the structural bar; they're a parking-lot item for the user.
...
**Output:** the discover-pass section is **appended** to `audits/_discover.md` (catalog-wide append-only file; one new dated section per run; sits next to the per-domain `audits/<DOMAIN>.md` files). The ranked candidate table from Pass 2 stays in `.tmp_deploy/` (ephemeral, regenerable).
...
|---|---|---|
| **Build the whole catalog folder (released-scoped)** | `bun run scripts/build_catalog.ts` (preview) / `--yes` (write) | The umbrella command: refreshes all three moving parts of `catalog/` from live state, scoped to **released** domains (`domains.catalog_release != null`, any date). Runs `emit_domain_map --released-any`, then `generate_blueprints --released`, then `emit_skill_spec --released --yes`, in that order (so the JSON's fresh DB read warms the shared cache for the blueprint pass). No-arg run previews the released scope and writes nothing; `--yes` regenerates in place; `--check` regenerates then fails if `catalog/` changed (CI drift gate); `--no-cache` forces a fresh DB read. Steps are failure-isolated: an upstream data gap (e.g. a B13 unclassified `entity_type`) fails only its own blueprint, the rest still refresh, and the command exits non-zero with a summary. |
| **Create the domain-map JSON snapshot** | `bun run scripts/emit_domain_map.ts` | Emits four files into [`catalog/domain-map/`](catalog/domain-map/): [`domain-map.json`](catalog/domain-map/domain-map.json) (domains + modules, each with `personas[]` and tagged `processes[]`), [`personas.json`](catalog/domain-map/personas.json) (every persona + its reach), [`processes.json`](catalog/domain-map/processes.json) (only processes in use), and [`business-functions.json`](catalog/domain-map/business-functions.json) (the function-centric inverse). Released-only by default (`--released-any` to include future-scheduled dates, `--all` for every domain). A full catalog snapshot for external tooling. Regenerable from live state. Git-tracked. |
...
| **Regenerate released-domain blueprints** | `bun run scripts/generate_blueprints.ts --released` | Emits a blueprint for every module hosted on a released domain (`catalog_release != null`), creating files for released modules that have none yet. The blueprint slice of `build_catalog`. |
...
- [`catalog/`](catalog/) — everything the catalog publisher consumes to produce the public site and installable skills. Contents:
  ...
  - [`catalog/domain-map/`](catalog/domain-map/) — the four emitted JSON snapshots of the catalog (`domain-map.json`, `personas.json`, `processes.json`, `business-functions.json`), gitted but regenerable in one pass via [`scripts/emit_domain_map.ts`](scripts/emit_domain_map.ts). `domain-map.json` carries per-domain / per-module `personas[]` + tagged `processes[]`; the sibling files are the full persona list, the in-use process list, and the function-centric RACI inverse.
